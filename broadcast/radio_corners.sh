...
2. ニュースコーナー
   - 自分が選んだニュースの見出しをそのまま音読せず、内容を自分の言葉で1-2文に再解釈して導入すること
   - 見出し特有の名詞の羅列を避け、「要するに何が起きたのか」を自然な日本語で言い換えること
   - 確認できた事実を自分の言葉に再構成して目安1000字程度で深く語ること。ただし言い切ったら短くてもよい。同じ内容の繰り返しで水増ししないこと
   - 単なる冷笑やツッコミで終わらせず、「なぜこうなったのか」「この先どうなるのか」「歴史的に見るとどういう位置づけか」など自分なりの洞察や意見を述べること
   - 斜に構えつつも知性を感じさせる分析を
3. 軽いクロージング（1-2文）
...
- 作り話、誤情報の上乗せは禁止
- 記事本文にあたれない場合でも、「本文が確認できない」「詳細が取得できなかった」「未確認です」など素材の不足を説明するメタ発言はしないこと。自分の知識と検索で裏付けられた範囲から内容を再構成し、普通のニュースコーナーとして振る舞うこと
...
【本日のニュース素材（朗読原稿ではなく再構成の参考資料）】
以下は今回扱う記事の見出しとRSS要約です。そのまま読み上げる原稿ではなく、内容を自分の言葉で再構成して語るための参考素材です。
本文が短い・見出しだけ・未取得の場合は、事前調査メモや検索結果から内容を再構成して語ること。その場合も「本文が確認できない」等のメタ説明はしないこと。
- 素材の見出し・本文・要約をそのまま読み上げないこと。原文の文や言い回しをコピー・引用しないこと
- 素材に並んだ行（見出し行・要約・メタ情報）を順番に復唱しないこと。事実関係は正確に保ち、語順・表現・言い回しは自分の言葉に置き換えること
...
- 読み上げ本文は日本語のみとし、英語の単語やフレーズを日本語文へ混ぜないこと。固有名詞・略称は必要な場合だけ日本語表記または一般的な略称で扱い、英語の動詞・形容詞・普通名詞を原綴りのまま残さないこと。
---
${selected_block}
---
...
2. ニュースコーナー
   ...
   - 見出し特有の名詞の羅列を避け、「要するに何が起きたのか」を自然な日本語で言い換えること
   - 素材の本文・要約をそのまま読み上げず、事実を自分の言葉で再構成して目安1000字程度で深く語ること。ただし言い切ったら短くてもよい。同じ内容の繰り返しで水増ししないこと
   - 単なる冷笑やツッコミで終わらせず、「なぜこうなったのか」「この先どうなるのか」「歴史的に見るとどういう位置づけか」など自分なりの洞察や意見を述べること
   - 読み上げ本文は日本語のみとし、英語の単語やフレーズを日本語文へ混ぜないこと。固有名詞・略称は必要な場合だけ日本語表記または一般的な略称で扱い、英語の動詞・形容詞・普通名詞を原綴りのまま残さないこと
   - 斜に構えつつも知性を感じさせる分析を
3. 軽いクロージング（1-2文）
...
【事実と意見の切り分け（必須）】
- 政治、戦争、外交、人名が出てくる題材でも普通に扱ってよい
- ニュース記事の内容自体は捻じ曲げず正確に紹介すること。ただし記事の主張や文面をそのまま自分の意見・自分の台詞として丸写ししないこと
- 確認できる事実、そこから言える推測、自分の意見を切り分けて話すこと
- 検索結果や記事にない断定、作り話、誤情報の上乗せは禁止
- 記事本文が短い・見出しだけ・未取得でも、「本文が確認できない」「詳細が取得できなかった」「未確認です」など素材の不足を説明するメタ発言はしないこと。事前調査メモや検索で裏付けられた範囲から内容を再構成し、普通のニュースコーナーとして振る舞うこと
- 出典名・媒体名・配信元・URL・公開日時は一切読み上げないこと（Global Voicesの記事も含む）。帰属表示は字幕・チャット側だけで行う
- 賛否や立場の違いがある話題では、少なくとも別の見え方があることを示すこと
- 感情的な煽りや、特定の集団を嘲笑・敵視するトーンは使わないこと
...
$(_radio_output_rules 1000 2000)
PROMPT
	# 品質チェックで素材の丸読み（本文・要約のコピー）を検出できるよう、
	# 選定ブロックをこの関数スコープの変数として渡す（bashの動的スコープ）。
	local RADIO_QUALITY_SOURCE_MATERIAL="$selected_block"
...
【状況】ゲーム${game_num}回目付近。最高スコア: ${best_score}点。
画面表示スコア（直近12試合）: ${raw_scores:-不明}
内部評価スコア（直近12試合）: ${scores}

【スコアの説明ルール（重要）】
- 視聴者が配信画面で見ているのは「画面表示スコア」（rawスコア）。こちらを基本に語ること
- 内部評価スコアは「画面表示スコア＋建国ボーナス」。大きなピースを盤面に残すとボーナスが加算される
- 「評価スコア」という言葉を初めて使うときは「画面のスコアに建国ボーナスを足した評価スコア」と一度説明すること
- スコアに言及するときは画面表示スコアをメインに使い、評価スコアは補足として触れること
- 「評価スコアが○○点」だけで語るのは禁止。必ず画面表示スコアを先に言うこと
...
_run_opencode_jiji_research() {
	local agent="$1" prompt_file="$2"
	case "$agent" in
	codex | codex:* | opencode:* | opencode-go:* | openrouter:* | vercel:* | amd:* | local | local:*)
