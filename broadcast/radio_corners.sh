...
【状況】ゲーム${game_num}回目付近。評価スコア(EVAL)履歴: ${scores}。最高スコア: ${best_score}点。
※ 評価スコア(EVAL) = 表示スコア(raw) + 建国ボーナス。視聴者が画面で見ているのは表示スコア(raw)の方。
※ トークでスコアに言及するときは「表示スコアは○○点で、建国ボーナスを含めた評価スコアは○○点」のように両方に触れること。片方だけで語ると視聴者が混乱する。
...
※ 分析メモのスコアは評価スコア(EVAL) = 表示スコア(raw) + 建国ボーナス。視聴者が画面で見ているのは表示スコア(raw)の方。
※ トークでスコアに言及するときは「表示スコアは○○点で、評価スコアは○○点」のように両方に触れること。
