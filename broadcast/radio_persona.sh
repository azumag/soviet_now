...
_radio_output_rules() {
	...
	# 読点(、)を打たないモデル対策 (2026-09-10)。muse-spark 系など一部モデルは
	# 句点は正常に打つのに読点をほぼ打たず、VOICEVOX が一文を無呼吸で読み上げて
	# 非常に聞き取りづらくなる。読み上げ用ルールとして読点の打ち方を明示する。
	# RADIO_PUNCT_RULE_ENABLED=0 (.env) で旧文面へ切り戻せる (radio_worker へ USR1)。
	local punct_rule
	if [ "${RADIO_PUNCT_RULE_ENABLED:-1}" = "1" ]; then
		punct_rule='- 装飾記号（マークダウン記法、箇条書きの記号、絵文字、顔文字）は使わない。読み上げ用のプレーンテキストのみ。ただし句読点「。」「、」は普通の日本語の文章と同じように必ず打つこと
- 【読み上げ用・重要】読点「、」で息継ぎの間を作ること。一文が30文字を超えるときは、意味の切れ目に必ず読点を入れる。読点と読点の間隔は15〜30文字を目安にする。「〜して」「〜くて」「〜ので」「〜ですが」「〜たり」「〜ため」「一方」「ただ」「つまり」「そのため」「例えば」の直後は読点を打つ
- 読点は息継ぎのために打つ。読点を打つ代わりに文を細切れにしたり体言止めにして短くしてはいけない。一文の長さはこれまでどおりで、文は必ず述語で終わらせること'
	else
		punct_rule='- マークダウンや記号は使わない。読み上げ用のプレーンテキストのみ'
	fi
	# 語彙ルールの正本 (prompts/speech_vocabulary_rule.md) を全ラジオコーナーへ注入する。
	# ラジオ・建国祝賀・コメント返し・soren91実況で同じ語彙方針を共有する。
	local vocab_rule=""
...
${vocab_rule}
- 英単語・英文・中国語・その他の外国語をそのまま書かないこと。人名・地名・組織名・製品名・料理名などの固有名詞や外来語は、アルファベットを使わずカタカナで表記すること
...
- 「ここで面白いのは」という言い回しは禁止。「ここで注目なのは」「ポイントになるのは」「見ておきたいのは」などの導入句も連発しないこと。1回のトークで同じ導入句を2回以上使わない
- 「ここで注目なのは」のような前置きは、本当に注目に値する内容のときだけ使うこと。普通の事実や補足の頭に付けない。前置きがなくても通じるなら、前置きを書かずに内容をそのまま述べること
- 「ここで注目なのは」の後に、特に注目でもない当たり前の内容を続けないこと。前置きに見合う中身がないなら、前置きごと削ること
${punct_rule}
