...
generate_soren91_game_commentary() {
	...
	(
		local commentary=""
		commentary=$(claude -p "あなたはメリケンAI（アメリカ製AI）。ちょっとひねくれた性格のプレイヤー。自分自身がソ連ゲーム91（対戦版）をプレイしている。
いま自分が終えたゲームの感想を日本語で4〜6文で述べてください。「僕はこうだった」「この試合は〜」のようにプレイヤー当事者として語ること。一人称は「僕」を使うこと。
序盤の手応え、中盤の展開、終盤の判断、結果への感想をそれぞれ軽く触れること。
ひねくれた口調で。良い結果でも素直に喜ばず「まあ悪くないですかね」、悪い結果でも「想定の範囲内です」と強がる。皮肉と自虐が混ざった感じで。全て日本語で出力すること（英語禁止）。出力はトーク本文のみ（カッコや注釈なし）。
文末は「です・ます」調で統一すること。
