...
_run_opencode_radio_unqueued() {
	...
	case "$agent" in
	openrouter:*)
		model="openrouter/${agent#openrouter:}"
...
_run_opencode_radio() {
	...
	case "$agent" in
	openrouter:*) resolved_model="openrouter/${agent#openrouter:}" ;;
	opencode-go:*) resolved_model="opencode-go/${agent#opencode-go:}" ;;
	opencode:*) resolved_model="opencode/${agent#opencode:}" ;;
	esac
	# Provider非依存の共有429バックオフ。特別扱いではなく全direct-chain候補に適用する。
	if command -v _ai_backoff_check >/dev/null 2>&1 && ! _ai_backoff_check "$agent"; then
		log "[RADIO] ${agent} shared backoff skip ($(_ai_backoff_remaining "$agent")s remaining)" >&2
		return 1
	fi
...
drop_line_patterns = [
    ...
    r'^\s*\{\s*"query"\s*:.*$',
    # 本文が取れないことを嘆くメタ説明・作業メモ節は読み上げ本文へ流さない。
    # 本文が無い場合は検索で再構成した材料だけを残す。
    r'(?:本文|全文|記事|元記事|一次情報).{0,25}(?:確認|取得|入手|参照|読込|読み込).{0,15}(?:でき(?:ない|ません|ず|ていな)|不可|失敗)',
    r'(?:本文|全文|記事|元記事|一次情報).{0,20}(?:404|not found|未確認|未取得)',
    r'^\s*[^\n]*(?:本文|全文)(?:の|を|は)?(?:直接)?(?:取得|確認|入手|読み込み)[^\n]*$',
    r'.*(?:推奨手順|推奨事項|残タスク|残件|未完了タスク).*$',
    r'^\s*未確認(?:点|事項|の点)?\s*[:：].*$',
    r'^\s*(?:#{1,6}\s*|[-*・●]\s*)*(?:未完了タスク|残タスク|残件|次の推奨手順|推奨手順|推奨事項|これまでの調査結果|達成状況|調査結果まとめ).*$',
    r'^\s*[（(]?(?:本文|全文)未確認[）)]?\s*$',
]
...
patterns = [
    ...
    # 本文が取れなかったことを告げるメタ文は読み上げから除去する（内容は事前調査で再構成させる）。
...
ルール:
- 読み上げ本文は書かない
- 放送で使える事実、日付、固有名詞、数値、背景だけを日本語の箇条書きにする
- 記事本文や一次情報を直接取得できなかった場合は、見出し・日時・分かっている断片からWeb検索を重ね、複数ソースの報道や公式発表から内容を再構成してまとめる。本文が無いことを理由に調査を止めない
- WebFetchが失敗した場合、その失敗文を出力しない
- 「本文が確認できない」「本文を取得できなかった」「本文未確認」「取得失敗」「404」など、素材が取れなかったことを示すメタ説明は書かない。放送で使える材料だけを書く
- 「未完了タスク」「残タスク」「残件」「次の推奨手順」「推奨事項」など作業メモ向けの節は書かない
- 断定できない細部は無理に書かず、確認できた事実だけで構成する
- 1200字以内
