...
_ai_dispatch() {
	...
	printf '%s|%s|%s\n' "$label" "$agent" "$timeout_override" >>"$ATTEMPT_LOG"
	case "$agent" in
	slow)
...
# dispatched at all and therefore cannot extend lane ownership.
: >"$ATTEMPT_LOG"
...
if [ "$lines" -eq 1 ] && [ "$first_timeout" = 1 ] && [ "$elapsed" -le 2 ]; then
	pass 'optional prepass releases after its total wall-clock budget'
else
	fail_case "optional prepass total budget (lines=$lines timeout=$first_timeout elapsed=$elapsed)"
fi

# 2. Required main generation is not constrained by the prepass budget shim.
: >"$ATTEMPT_LOG"
main_out=$(ai_generate_list 'RADIO:news' "$prompt" 'success' 2>/dev/null || true)
main_timeout=$(awk -F'|' 'NR==1 {print $3}' "$ATTEMPT_LOG")
if [ "$main_out" = ok ] && [ -z "$main_timeout" ]; then
	pass 'radio main timeout policy is unchanged'
else
	fail_case "radio main timeout policy unchanged (out=$main_out timeout=$main_timeout)"
fi
