...
assert "Soren91中は通常canvasを隠してCPUを間引く" grep -q 'Normal sorengame lifecycle=frozen canvas=hidden cpuThrottle=' "$ROOT_DIR/soren91/main.mjs"
assert "音声復帰を実測できれば通常枠を再起動しない" grep -q 'AudioContext running+BlackHole を実測、bridge再起動を省略' "$ROOT_DIR/soren91_control.sh"
assert "Soren91の既定内部解像度は640x360" grep -q 'SOREN91_VIEWPORT_WIDTH="${SOREN91_VIEWPORT_WIDTH:-640}"' "$ROOT_DIR/core/config.sh"
