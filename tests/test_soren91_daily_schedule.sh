...
assert "Soren91中は通常canvasと描画ループを止めてCPUを間引く" grep -q '__sorenRenderPaused = true' "$ROOT_DIR/soren91/main.mjs"
assert "音声復帰を実測できれば通常枠を再起動しない" grep -q 'AudioContext running+BlackHole を実測、bridge再起動を省略' "$ROOT_DIR/soren91_control.sh"
assert "Soren91の既定内部解像度は480x270" grep -q 'SOREN91_VIEWPORT_WIDTH="${SOREN91_VIEWPORT_WIDTH:-480}"' "$ROOT_DIR/core/config.sh"
