...
log() { printf '%s\n' "$*" >>"$LOGS"; }
ai_generate() {
  printf '%s|%s|%s|%s|%s|%s|gate_wait=%s|queue_wait=%s\n' "$1" "$2" "$3" "$4" "$5" "$6" "${AI_RADIO_IMPROVE_WAIT_MAX_SEC:-unset}" "${AI_GENERATION_QUEUE_MAX_WAIT_SEC:-unset}" >>"$CALLS"
  printf '%s\n' "${TEST_AI_OUTPUT:-SPAM}"
  return "${TEST_AI_RC:-0}"
}
...
grep -q '|local|opencode:muse-spark-1.3-contributor-free|20|_news_spam_verdict_valid|gate_wait=20|queue_wait=20$' "$CALLS"
...
grep -q 'gate_wait=3|queue_wait=20$' "$CALLS"
...
: >"$CALLS"
NEWS_SPAM_CHECK_QUEUE_WAIT_MAX_SEC=4 TEST_AI_OUTPUT= TEST_AI_RC=92
if _news_ai_spam_check "混雑中" $'■ 混雑中\n本文です'; then
  echo "queue giveup must fail open" >&2; exit 1
fi
grep -q 'queue_wait=4$' "$CALLS"
grep -q '生成キュー待ち上限(4s)' "$LOGS"
