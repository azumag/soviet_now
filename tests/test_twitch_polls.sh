...
if grep -Fq 'TWITCH_POLL_INTERVAL_SEC="${TWITCH_POLL_INTERVAL_SEC:-600}"' "$config" && grep -Fq 'TWITCH_POLL_INTERVAL_SEC:-600' "$worker"; then
  ok "既定間隔は10分"; else not_ok "既定間隔は10分"; fi
