...
// The overlay is a fullscreen X window, so whichever window is mapped last is
// on top.  It used to be started on demand for game-only mode, which put it
// over an already-mapped game window and blacked out the broadcast (2026-09-15
// soren91 corner).  These contracts pin the fix: the service stays up, keeps
// its own window at the bottom of the stack, and the lifecycle control surface
// never toggles it.
test('service wrapper pins the overlay window below the game windows', () => {
  const wrapper = fs.readFileSync(
    path.join(REPO_ROOT, 'start_shared_overlay_service.sh'),
    'utf8',
  );
  assert.match(wrapper, /xdotool search --name "\^Shared overlay"/);
  assert.match(wrapper, /wmctrl -i -r "\$wid" -b add,below/);
  assert.match(wrapper, /xdotool search --name "\^docich-present"/);
  assert.match(wrapper, /wmctrl -i -r "\$wid" -b add,above/);
});


test('lifecycle control surface never starts or stops the shared overlay', () => {
  const control = fs.readFileSync(
    path.join(REPO_ROOT, 'game_lifecycle_control.sh'),
    'utf8',
  );
  assert.doesNotMatch(control, /shared_overlay_(start|stop)\b/);
  assert.doesNotMatch(control, /systemctl start "?\$?\{?SOREN_SHARED_OVERLAY_UNIT/);
  assert.doesNotMatch(control, /systemctl stop "?\$?\{?SOREN_SHARED_OVERLAY_UNIT/);
  // Readiness is still verified instead of assumed before the irreversible stop.
  assert.match(control, /shared_overlay_wait_ready/);
  assert.match(control, /shared_overlay_wait_ready \|\| true/);
});


test('shared overlay unit is installable for boot (always on)', () => {
  const unit = fs.readFileSync(
    path.join(REPO_ROOT, 'deploy/soren-shared-overlay/soren-shared-overlay.service'),
    'utf8',
  );
  assert.match(unit, /WantedBy=multi-user\.target/);
  assert.match(unit, /ExecStart=\/home\/ubuntu\/soren\/start_shared_overlay_service\.sh/);
  assert.match(unit, /Environment=SOREN_SHARED_OVERLAY_ATTACH=1/);
  const readme = fs.readFileSync(
    path.join(REPO_ROOT, 'deploy/soren-shared-overlay/README.md'),
    'utf8',
  );
