...
#
# v262: deadline margin-based danger zone merge priority - rollback failure mode purge
# last_rollback_postmortem: v257で追加されたaxis 9 (HIGH_TYPE_PROXIMITY) が危険域での即時併合優先を妨害し、
# deadline_margin<0 の危険域で延命失敗した。
# rollback targetのv178に戻り、axis 9を削除し、deadline_marginを活用した危険域判定を導入。
# deadline_margin = deadline_y - top_edge_y < 0.5 を物理的危険域と定義し、この領域では即時併合を絶対優先する。
#
...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # Calculate deadline_margin for danger zone judgment
    # Note: deadline_y is the top of the red line, top_edge_y is the highest y of any piece.
    # If deadline_margin < 0.5, pieces are touching or very close to deadline.
    top_edge_y = max([p["y"] for p in pieces]) if pieces else -5.0
    deadline_y = 2.5
    deadline_margin = deadline_y - top_edge_y
    ...
    for result in results:
        ...
        # ----- evaluation axis 8.5: deadline margin-based danger zone direct merge priority (NEW: deadline_margin < 0) -----
        # deadline_margin<0 means pieces are physically touching deadline (critical danger).
        # If reactive_pairs >= 2, we must merge immediately to survive.
        # This axis is evaluated BEFORE axis 8 (reactive pairs bonus) to prioritize danger zone survival.
        if (
            deadline_margin < 0
        ...
        # ----- evaluation axis 8.6: deadline margin-based danger zone no merge penalty (NEW: deadline_margin < 0) -----
        # deadline_margin<0 (physically touching deadline) and reactive_pairs >= 1 and no immediate merge.
        # This is a rollback to v178 logic: when pieces touch deadline, any non-merge placement is critically dangerous.
        # This axis is evaluated BEFORE axis 8 (reactive pairs bonus) to prioritize danger zone survival.
        if deadline_margin < 0 and reactive_pair_count >= 1 and merge_grade == "NO":
            # deadline_margin<0の物理的危険域でreactive_pairs>=1あるのに即時併合がない場合、極めて強力なペナルティを与える
            # 危険なHIGH_TOWER判断を完全に抑制し、即時併合優先を強制
            score -= 3000.0
        ...
        # refs: tmp/batch_summary.txt, tmp/state/last_rollback_postmortem.md, game_history/20260313_231816_score0814.jsonl turns 54-57, game_history/20260313_231248_score4925.jsonl
        if not reasons:
