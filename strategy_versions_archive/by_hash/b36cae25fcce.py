...
Decision Logic (5 evaluation axes):
  ...
  6. NO_MERGE penalty - Penalty for positions with no merge opportunity (v84/v126: -150, v158: -200, v161: -200)

Phases (determined by board max Y):
  LOW      (max_y < 0.8) : Early game. Merge priority (merge_mult=1.2)
  MEDIUM   (0.8 <= max_y < 1.8) : Mid game. Height management strengthened (height_mult=2.4, v126/v42)
  HIGH     (1.8 <= max_y < 3.0) : Late game. Height management relaxed (height_mult=1.8, v84/v155)
...
# v161: v42/v126成功構造復帰・CHAIN_MERGE削除版 - v155のCHAIN_MERGEロジック（3つの連鎖ピース評価、chain_distance=5.0、係数450.0）は評価範囲が広すぎて評価精度が低下。
# v156の履歴分析でMEDIUM height_mult=1.8（v151緩和）がHEIGHT_CONTROL過多の原因と推定。
# v42/v126のMEDIUM height_mult=2.4に復帰し、HEIGHT_CONTROLを削減してスコア安定性を向上。
# CHAIN_MERGEロジックを完全削除し、v84のNO_MERGEペナルティ-150を-200に強化して併合を強制的に促す。
...
def decide(game_state: dict, analysis: dict) -> dict:
    """v161: v42/v126成功構造復帰・CHAIN_MERGE削除版

    v155のCHAIN_MERGEロジック（3つの連鎖ピース評価、chain_distance=5.0、係数450.0）は評価範囲が広すぎて評価精度が低下。
    v156の履歴分析でMEDIUM height_mult=1.8（v151緩和）がHEIGHT_CONTROL過多の原因と推定。
    v42/v126のMEDIUM height_mult=2.4に復帰し、HEIGHT_CONTROLを削減してスコア安定性を向上。
    CHAIN_MERGEロジックを完全削除し、v84のNO_MERGEペナルティ-150を-200に強化して併合を強制的に促す。
    v84のHIGHフェーズheight_mult=1.8を維持し、v42/v126のMEDIUM height_mult=2.4と統合。
    ...
    if max_y < 0.8:
        ...
        phase = "MEDIUM"
        height_mult = 2.4  # v161: v42/v126の2.4に復帰、HEIGHT_CONTROLを削減
        merge_mult = 1.0
    elif max_y < 3.0:
        phase = "HIGH"
        height_mult = 1.8  # v161: v84/v155の1.8を維持、併合機会確保
        merge_mult = 1.0
    else:
        phase = "CRITICAL"
    ...
    for result in results:
        ...
        if phase == "HIGH" and landing_y > 0.5:
            height_penalty *= 1.3  # v84: HIGH_TOWER penalty relaxation
        ...
        if phase == "HIGH":
            balance_strength = 40.0  # v42/v126: HIGH balance control
        elif phase == "MEDIUM":
            balance_strength = 30.0  # v42/v126: MEDIUM balance control
        ...
        # ----- evaluation axis 6: NO_MERGE penalty (v84/v126/v158/v161) -----
        # penalty for positions with no merge opportunity to force merge
        # v161: strengthen from v84's -150 to -200 to promote merge
        if merge_grade == "NO":
            score -= 200.0
