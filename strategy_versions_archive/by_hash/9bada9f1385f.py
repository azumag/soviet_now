...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    deadline_buffer_drop_exists = any(
        float(r.get("top_y_after_drop", r.get("landing_y", 999.0)) or 999.0) <= deadline_buffer_y
        for r in results
    )
    for result in results:
        ...
        top_y_after_drop = float(result.get("top_y_after_drop", landing_y) or landing_y)
