...
def decide(game_state: dict, analysis: dict) -> dict:
    """v164: v155成功パラメータ復帰・バランス補正強化版

    v163の密度カウント版（v154の密度評価ロジックから変更）を破棄し、v154/v155の密度評価ロジック（3つの最も近いピースを距離重みで評価）に戻す。
    v155の成功パラメータ（chain_distance=5.0、chain_bonus_multiplier=450.0）で達成した4026点の成功構造を復帰。
    動的パラメータ調整を削除し評価精度向上。同時にv162のバランス補正強化（MEDIUM: 40.0）を維持し、盤面の左右バランスを改善。
    ...
    for result in results:
        ...
        # ----- evaluation axis 6: chain merge bonus (v164: v155 stable parameters + v162 balance enhancement) -----
        # v164: Revert to v154/v155 density evaluation logic (3 closest pieces with distance-weighted bonus).
        # v163's density count version (v162's change from v154) is discarded.
        # v155's successful parameters (chain_distance=5.0, chain_bonus_multiplier=450.0) are maintained.
        # This combines v155's successful parameters with v162's balance correction enhancement.
        if merge_grade in ["DIRECT", "NEAR"] and result.get("merges"):
            ...
            if merges:
                ...
                # v164: v155's stable parameters (chain_distance=5.0 fixed, chain_bonus_multiplier=450.0 fixed)
                ...
                # bonus calculation from closest 3 pieces (v154/v155 density evaluation logic)
