...
# v147: MEDIUM高度管理微調整版 - v146のスコア分散（stddev=352.8）とワーストゲーム（541点）の序盤盤面上昇を受けて、
# MEDIUMフェーズのheight_multを2.2→2.3に微増。batch_summaryでNEAR_MERGEのavg_score_delta=20.3〜43.7点と高価値だが、
# MEDIUMフェーズでの併合機会が不足していると分析。v42(2.4)とv145(2.2)の中間値2.3で高度管理と併合機会のバランスを調整。
...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    MEDIUMフェーズのheight_multをv146の2.2から2.3に微増し、NEAR_MERGE機会を確保。
    v42(2.4)とv145(2.2)の中間値で高度管理と併合機会のバランスを調整。
    v146の盤面平坦度スコアの動的調整機能は維持し、スコア安定性を向上させる。
    ...
    if max_y < 0.8:
        ...
        phase = "MEDIUM"
        height_mult = 2.3  # v147: MEDIUM高度管理微調整（2.2→2.3）、v42(2.4)とv145(2.2)の中間で併合機会確保
        merge_mult = 1.0
    elif max_y < 3.0:
        phase = "HIGH"
