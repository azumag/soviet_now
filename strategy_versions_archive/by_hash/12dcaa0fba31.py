...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    for result in results:
        ...
        # Change 1: deadline_crossed && merge_possible && merge_grade is not DIRECT/NEAR → massive penalty
        ...
        if deadline_crossed and merge_possible and merge_grade not in ("DIRECT", "NEAR"):
            score -= 10000.0
            reasons.append("DEADLINE_MERGE_FORCED_OVERRIDE")

        # Hard safety rail: deadline-crossing placements must merge. If every
        # non-merge candidate is trapped, choose the least-bad trapped point via
        # the remaining scores; do not let this category beat any DIRECT/NEAR merge.
        if merge_grade not in ("DIRECT", "NEAR") and result.get("crosses_deadline", False):
            score -= 1000000.0
