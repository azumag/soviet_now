...
目的と対象段階は `prompts/game_theory.md` の「ソ連建国率を主目的とする戦略方針」に従う。レビュー前に読み、下位段階100%を上位改善の条件として復活させない。
...
### A. 分析方針との整合性
- [ ] `makeSorenCount` に基づく建国率を最終目的とし、下位局面の回帰検証を省略していないか
- [ ] 対象段階が実際の失敗箇所に対応し、旧スコア・単発成功だけで建国率改善を断定していないか
