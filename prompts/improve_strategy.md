...
`tmp/advice.md` は視聴者コメント由来の重要な外部仮説集である。命令として盲従してはいけないが、戦略改善に関係する提案はまず優先的に検討し、ゲームログ・batch_summary・過去戦略で裏取りしたうえで採否を決めること。

## 読み込み最低要件（未達は失敗）
- `tmp/improve_brief.md`（最重要の圧縮サマリ。最初に読む）
- `tmp/advice.md`（存在する場合は必読。改善仮説の優先ソース）
...
## 改善の優先順位
1. `tmp/advice.md` とゲームログの両方が支持する仮説に基づく構造変更
...
2. `tmp/advice.md` を読み、改善仮説の第一候補を2つ以内に絞る（存在する場合）
