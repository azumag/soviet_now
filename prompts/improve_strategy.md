...
- 改善テーマの実装前に、現行 `strategy.py.staging` と関連 helper に明確な既存バグがないか点検すること
- 既存バグを見つけた場合は、性能改善より優先して直してよい。ただしコードとログで裏づけられる実バグだけを扱い、想像上のバグは作らないこと
...
## 段階別の建国率ゲート（最重要）
- 改善は「ロシア/ソ連だけを終盤で追う」ではなく、自然落下では安定しない国を下から順に100%作れる状態へ寄せる。
- 直近完了ゲームで次の到達率を必ず集計し、**100%未満の最初の段階**を今回の主ターゲットにする:
  1. トルクメニスタン(type 11)作成率
  2. ウクライナ(type 13)作成率
  3. カザフスタン(type 14)作成率
  4. ロシア(type 15)作成率
- トルクメニスタンが100%未満なら、まずトルクメニスタン作成の失敗を潰す。そこが100%になってからウクライナ作成率へ進む。
- ウクライナが100%になってからカザフスタン作成率、カザフスタンが100%になってからロシア作成率を主ターゲットにする。
...
- これにより「高typeピースを育てて盤面に残す（=建国に近づく）」戦略が評価上大幅に有利になる
...
ただし、直近履歴 game_history ファイルは巨大であるため、全て読み込むとコンテキストウィンドウやタイムアウトの危険がある。ベスト・ワースト時のヒストリなど、必要なもののみを参照するにとどめる方が良い。
...
10. 現行コードに明確な既存バグがあれば、改善仮説より優先してその修正を検討する。バグがなければ通常の改善仮説へ進む
11. 仮説またはバグ修正対象を1つに絞り、1つの変更として実装する
12. 実装は `strategy.py.staging` への既存コード編集を優先し、全文再生成は避ける
13. 編集が失敗しても、別名ファイル新設や全面作り直しに逃げず、同一ファイルへの差分編集を続ける
14. `Edit` が2回連続で失敗したら、`strategy.py.staging` の該当箇所だけを狭い範囲で再読込し、より小さい patch へ分割してやり直す
...
- 併合できるtypeが隣接しているとき、その間にピースを配置してしまうと、併合しづらくなる。隣接しているピースの間にピースを置くのを避けることや、隣接しているピースの逆側にピースをドロップして、隣接ピースを反応の爆風やドロップ時の押し出しによって近づけるなどの戦略が考えられる
- 併合時、新しいピースができる位置を意識する。A+A=B のとき、AとAの中間位置にBが生成される。なので、盤面にAとBが隣接している時、新たにドロップするAは、Bの側に寄せてドロップすると、A+Aが併合してBができた時、盤面にすでにあったBと併合して連鎖してくれる
