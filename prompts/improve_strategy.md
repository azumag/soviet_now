...
- ゲームログの分析を急がず、十分な根拠を集めてから仮説を立てること
- 実装前にコードを丁寧に読み、変更の影響範囲を正確に理解すること
- 「とりあえず変えてみる」ではなく、データに裏付けられた確信のある変更だけを行うこと
...
- この課題は下振れ耐性と同等以上に重要。ロシア1つ止まりでゲームオーバーになるのが最も惜しい負けパターンである
...
- ゲームオーバー時の最終盤面に残っている各ピースのtypeに応じてボーナスが加算される:
  - type 1-5: 0, 0, 1, 1, 2 / type 6-10: 3, 4, 6, 10, 16 / type 11-15: 26, 40, 70, 120, 240
  - ソ連建国(makeSorenCount > 0)時: +800
- 例: 最終盤面に type12×1, type10×1, type9×1, type8×2 → bonus ≈ 78。典型スコア(~1200)の約6%。高type多数なら20-40%
...
6. ワースト/ベスト + 追加1件以上のゲームログを読み、失敗モードと成功モードの差分を整理する
   ...
   また、type 15(ロシア)が出現した後のターンがあれば、その局面での判断パターンと結末を確認する
