...
機械契約JSONが正しくても、本文やJSON内の仮説・変更・hold理由に矛盾した集計値を書かない。`soviet`、`soviet_counter`、`founded_games`、`ソ連建国数`、`建国件数` の `=件数/試合数` または `:件数/試合数` 表記（分母省略可）は現在のhost証拠との照合対象。欠測は `soviet=unknown` とし、0にしない。分母はhostのgame_countと一致させる。

この予約表記は引用・否定・過去バッチの説明でも現在の集計として検査する。誤記を説明するときは矛盾する数値表記を再掲せず「過去の分析が欠測をゼロと誤認した」のように書く。known_founded_games、unknown_counter_games、russiaは別指標。Markdown装飾・全角・大小文字は免除理由にならない。この検査は指定表記の照合であり、任意の自然言語、割合表現、因果性の完全検証ではない。
