...
- ダッシュボードの `Founding Rates` / soren91 ランキングコメントは、現行 hash の直近 `current_strategy_run.json` から type15(ロシア) / type14(カザフスタン) / type13(ウクライナ) の到達率と、次に粛清基準として注目される stage を表示する。sample 数が少ない場合も inactive ではなく `sample不足` として出し、ロシア建国率の低下・停滞を配信上で見逃しにくくする。
