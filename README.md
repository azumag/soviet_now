...
- 既定では early culling を行わない (`WILDCARD_PARALLEL_CULL_AFTER_GAMES=0`)。全候補が `--games` を**完走**してから `choose_winner` で最良 (`min_successful_games` 達成済みの中で comp/objective 最大) を選ぶ。1試合だけの分散で有望候補を早期脱落させ「finished が 1 つだけ」になる探索浅化を防ぐため。高速優先に戻したい場合のみ `WILDCARD_PARALLEL_CULL_AFTER_GAMES` を `>0` にすると、N 試合ごとに leader 比 `WILDCARD_PARALLEL_CULL_COMP_RATIO` 未満を cull し、空いた slot を再摂動で埋める旧モードになる。
