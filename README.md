...
- `stream_bug_report` は「無音になってる？」「BGM聞こえない？」「ゲーム音なし」「すぐゲーム音でなくなるね」のような疑問形・短文・再発報告でも配信不具合として扱い、`tmp/codex_bug_queue` 経由で Codex 側へ渡す。通常の一般質問や短文リアクションへ落とすと音声/表示トラブルが再現中に埋もれるため。
