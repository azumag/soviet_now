...
- `post_improve_param_parallel` は WILDCARD 並列評価と同じ隔離ブラウザ基盤を使い、既定で有効にする。0 game `infra_failed` は parallel を止める理由にせず、candidate Chrome の HOME/XDG/TMPDIR 隔離や OBS 音声/候補窓配線のように原因側を修正して継続する。
