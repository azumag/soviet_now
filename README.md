...
- `post_improve_param_parallel` は macOS/OBS/Chrome の隔離起動が崩れると 0 game 失敗や OBS の候補窓 churn を起こすため、既定では `POST_IMPROVE_PARAM_PARALLEL_ENABLED=0` で安全停止する。再開する場合は明示的に `POST_IMPROVE_PARAM_PARALLEL_ENABLED=1` を設定し、候補窓OBSソースも必要な時だけ `WILDCARD_PARALLEL_OBS_WINDOW_SOURCES=1` で開く。
