...
_generate_poll() {
	...
	cat >"$prompt" <<'EOF'
あなたは日本語のライブ配信「ソ連ゲーム」の短いアンケートコーナーを作ります。
視聴者が気軽に答えられ、結果を話題にできる質問を1つ作ってください。

制約:
- 政治的主張、個人情報、攻撃的内容、正解を要求する難問、配信運用の内部事情は避ける
- ゲーム、食べ物、日常、音楽、配信の楽しみ方など軽い話題にする
- 質問は日本語60文字以内
...
print("ライブ配信のアンケート結果について、日本語で自然な感想を1〜2文、120文字以内で述べてください。")
print("最多票や同率を正確に扱い、得票数を捏造・変更しないでください。視聴者を責めず、説明やMarkdownは付けません。")
...
PY
	...
	rm -f "$prompt"
	raw=$(printf '%s' "$raw" | _ai_guard_model_output | tr '\n' ' ' | sed 's/[[:space:]][[:space:]]*/ /g; s/^ //; s/ $//' | cut -c1-120)
	[ -n "$raw" ] || raw="投票ありがとうございました。みなさんの選び方が見えて、面白い結果になりました。"
	printf '%s\n' "$raw"
}
...
PY
	rm -f "$STATE_FILE"
	_schedule_after "${TWITCH_POLL_INTERVAL_SEC:-3600}" completed
	_log "結果コメントをキューへ登録"
	return 0
}
