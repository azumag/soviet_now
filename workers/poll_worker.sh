...
_generate_poll() {
	...
	cat >"$prompt" <<'EOF'
あなたは日本語のライブ配信「ソ連ゲーム」のアンケートコーナーを作ります。ボケ強めで面白い質問を1つ作ってください。
...
方針:
- ボケ強め。常識的で当たり障りのない質問（例: 好きな季節は？）は禁止。選択肢のうち1〜2個は極端なボケ、妄想、ソ連・計画経済ネタ、資本主義あるある、時事パロディを入れる
- ソ連・共産主義vs資本主義・政治・時事ネタはむしろ積極的に使う。説教や押し付け、特定の政治的主張の断定、攻撃的な内容にはせず、ユーモアとして軽く扱う
- ゲーム、食べ物、日常、音楽、配信の楽しみ方などもOKだが、単なる一般常識クイズにはしない。時事や社会ネタを絡めるとより良い
- 個人情報、正解を要求する難問、配信運用の内部事情は避ける
