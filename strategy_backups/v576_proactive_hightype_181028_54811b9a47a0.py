...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    for result in results:
        ...
        if merge_grade == "NEAR" and landing_y > 0 and reactor_margin < 1.0:
            risk_factor = min(1.680, max(0.1332, 1.0 - reactor_margin))
        ...
        if merge_grade == "NEAR" and piece_count >= 33 and reactor_margin < 0.493 and landing_y >= 1.0:
            score -= 600.0 * merge_mult
        ...
        if reactive_pair_count >= 1 and reactive_pair_count < 3 and merge_grade == "NO" and same_type_stack_top is not None:
            ...
            if current_type_has_reactive or current_type_has_near:
                ...
                if horizontal_distance < 2.0:
                    ...
                    if piece_count >= 28:
                        congestion_scale = 1.0 + (piece_count - 28) * 0.0452
        ...
        for r in reactor.get("recent_results", []):
            if r.get("merge_grade") == "NO":
                ...
                no_merge_streak = 0
        # v576 (goal-session manual): fire the same-type proximity guidance proactively
        # for the Russia-path types (T13→T14, T14→T15) instead of only after a deep
        # 3-turn drought. The first Russia stalls because T13x2/T14x1 sit on the board
        # un-consolidated; waiting for a 3-turn drought lets other axes scatter them
        # first. For next_type>=13 a single no-merge turn is enough to start steering the
        # drop toward the same-type partner; lower types keep the conservative >=3 gate.
        ...
        if (merge_grade == "NO" and len(same_type_pieces) >= 1
            ...
            if min_horiz_dist < 5.0:
                # v575 (goal-session manual): scale by piece type. Russia-path merges
                # (T13→T14, T14→T15) are worth far more in the EVAL building bonus
                # (T15=12096 ≫ T14=5760 ≫ lower) than low-type merges, yet the flat
                # bonus valued them identically. Live bottleneck: best_max_type stuck at
                # 14 — T13x2/T14x1 accumulate but never consolidate into a 2nd T14 → the
                # first Russia. Boost high-type same-type proximity so the drop is pulled
                # toward forming the high merge path. Additive + drought-only + max_y<3.0,
                # so it cannot drive the height-runaway the suppression axes guard against.
                high_type_scale = 1.0 + 0.12 * max(0, next_type - 10)
                drought_bonus = max(0, 400.0 - min_horiz_dist * 80.0) * high_type_scale
                if drought_bonus > 0:
                    score += drought_bonus
              ...
              if merge_grade in ["DIRECT", "NEAR"]:
                  ...
                  if russia_phase_second:
                      ...
                      if reactive_pair_count >= 1:
                          if merge_grade == "DIRECT":
                              score += 1600.0 if reactive_pair_count >= 3 else 1400.0
                          else:
                              score += 1501.5 if reactive_pair_count >= 3 else 1200.0
                      else:
                          if merge_grade == "DIRECT":
                              score += 1600.0
                          else:
        ...
        if result.get("decision_crosses_deadline", True) and merge_grade == "NO" and reactor_margin < 2.0:
            score -= 2500.0
        ...
        if merge_grade == "NO" and reactor_margin < 2.0 and not russia_phase:
            score -= 2500.0
