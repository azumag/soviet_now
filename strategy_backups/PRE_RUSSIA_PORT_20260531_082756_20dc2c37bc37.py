             ...
             8.5. Danger zone immediate merge bonus - v331: deadline_crossed時即時併合強化
              8.6. Reactive pairs immediate merge bonus - v321: 即時併合ボーナス維持
               8.7. Russia phase immediate merge priority - v336: ロシア建国後フェーズ即時併合強化版 - axis 8.7ボーナス強化
                # v532m: rp>=3+NO_MERGE局面での候補差別化不足の改善
                #   - axis9.6 stacking bonus: 100-400 → 2500(base)+decay
                #   - axis 9.6b proximity bonus: 120-900 → 2000(base)+decay
                #   - REACTIVE_PAIRS_NO_MERGE_PENALTY: -4500 → -2500
...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # v577: HOIST max_type_on_board to before the candidate loop (CRASH-SAFETY).
    # The growth-center axis (further below) assigns max_type_on_board INSIDE the loop;
    # the axis 9.75 attempt crashed by referencing it before that in-loop assignment.
    # Defining it here (alongside pieces/max_y) makes it safe for any earlier-in-loop block.
    # The in-loop assignment below is idempotent (same value), so existing logic is unchanged.
    ...
    # =======================================================================
    # v577: PLANNED HIGH-MERGE LOOKAHEAD (reliable, non-speculative) — precompute
    # -----------------------------------------------------------------------
    # GOAL: build the 2nd T13 (and beyond) by co-locating the high-type feeder
    # pair instead of scattering it. The 2nd high partner can NEVER spawn (spawn
    # ceiling=11) — it must already exist on the board as a merge product. So the
    # plan is driven ENTIRELY off on-board state (no spawn betting):
    #   - product_type = next_type + 1 (the type this turn's same-type merge yields)
    #   - HIGH build regime requires product_type >= 12 (so the product is a T12+
    #     that can chain to a 2nd T13/T14/T15(Russia)/T16(Soviet)).
    #   - same_type_pieces (already computed) must have >= 2 members, so an
    #     immediate same-type merge to product_type is geometrically possible.
    #   - There must ALREADY be >= 1 on-board piece of product_type (the partner
    #     that the freshly-built product will pair with next) — proving the chain
    #     can complete soon from real board state, not a future spawn.
    # CRISIS GUARD: only plan when the board is NOT near death, so the genuine
    # death-safety penalties (DEADLINE_CROSSED axis 9.6, CROSSES_DEADLINE,
    # near-risk) always win in a real crisis. Gate on:
    #   max_y < 2.5  (well below deadline ~3.38),  reactor_margin >= 1.647,
    #   not deadline_crossed.
    # All variables here are defined BEFORE the candidate loop (crash-safe):
    #   next_type/next_next_type (L752-753), same_type_pieces (L763),
    #   max_type_on_board (hoisted near L700), pieces/max_y (L697-698),
    #   reactive_pair_count (L708), reactor_margin (L710), deadline_crossed (L702).
    plan_product_type = next_type + 1
    plan_high_partners = [p for p in pieces if p.get("type") == plan_product_type]
    plan_regime_safe = (
        max_y < 2.5
        and reactor_margin >= 1.647
        and not deadline_crossed
    )
    # RELIABLE detection precondition (board-grounded, no spawn speculation):
    plan_active = (
        plan_product_type >= 12
        and len(same_type_pieces) >= 2
        and len(plan_high_partners) >= 1
        and plan_regime_safe
    )
    ...
    for result in results:
        ...
        if reactive_pair_count >= 1 and reactive_pair_count < 3 and merge_grade == "NO" and same_type_stack_top is not None:
            ...
            if current_type_has_reactive or current_type_has_near:
                ...
                if horizontal_distance < 2.0:
                    # v532m: boost from ~100-400 to 2500 (base) + distance decay
                    stacking_bonus = 2500.0 + max(0, 2500.0 - horizontal_distance * 500.0)
        ...
        if merge_grade == "NO" and same_type_stack_top is not None:
            if not (current_type_has_reactive or current_type_has_near):
                ...
                if horiz_dist < 2.0:
                    ...
                    # v532m: boost from ~120-900 to 2000 (base) + distance decay
                    proximity_bonus = max(0, 2000.0 - horiz_dist * 500.0)
                    if piece_count >= 28:
        ...
        if deadline_crossed and reactive_pair_count >= 1 and merge_grade == "NO":
            # v532m fix: rollback target uses flat -2500 (v532), not gradient.
            # v432 gradient (-3000..-7000) violates postmortem constraint:
            # "deadline_margin NO_MERGE penalty must not exceed -2500".
            # At rp>=3+NO, gradient summed with axis 8.8 overwhelms proximity/stacking
            # bonuses (~2000-2500), killing all candidate differentiation and causing
            # HIGH_TOWER/HIGH_LAYER death spiral. Flat -2500 preserves differentiation.
            score -= 2500.0
              ...
              if merge_grade in ["DIRECT", "NEAR"]:
                  ...
                  if russia_phase_second:
                      ...
                      if reactive_pair_count >= 1:
                          if merge_grade == "DIRECT":
                              score += 1600.0 if reactive_pair_count >= 3 else 1111.0
                          else:
        ...
        if reactive_pair_count >= 3 and merge_grade == "NO":
            # v532m: reduce from -4500 to -2500 (analysis: rp>=3 + NO_MERGE 局面での
            # 候補差別化不足を改善するためペナルティを抑制し、axis 9.6/9.6b 高額ボーナスで
            # 最も良い併合配置を選択させる)
            score -= 2500.0
        ...
        # ----- v577: PLANNED HIGH-MERGE OVERRIDE (build the 2nd T13+ pair) -----
        # Fires ONLY when plan_active (precomputed before the loop): product_type>=12,
        # >=2 same-type on board, >=1 existing partner of product_type, and the board
        # is in the SAFE regime (max_y<2.5, margin>=1.647, not deadline_crossed). When
        # plan_active is False, this block changes NOTHING — general play and the genuine
        # death-safety are fully preserved. The crisis guard is baked into plan_active,
        # so even with a plan we never fight the real DEADLINE penalties (they fire only
        # when the guard conditions are violated).
        #
        # The freshly-built product (type product_type) will appear at ~(settled_x,
        # landing_y) where settled_x = x + drift_x. We reward placements that put that
        # product close to an EXISTING product_type partner so the next step completes
        # the high pair (-> 2nd T13/T14/T15/T16). Two legs:
        #   (A) CHAIN: this candidate actually merges the same-type pair now
        #       (merge_grade DIRECT/NEAR) -> bias toward the partner-adjacent x.
        #   (B) SETUP-HOLD: this candidate is a NO-merge feeder placement that lands
        #       the current piece near an existing same-type piece AND near a partner,
        #       deliberately co-locating the feeder. Neutralize axis 8.8 (-4500) so the
        #       co-locating hold can beat a scatter hold instead of being flattened.
        # Strength is capped so it cannot beat the genuine death-safety (which is
        # excluded by plan_active anyway) and cannot create height-runaway: the bonus
        # decays to 0 as the product point gets far from any partner.
        if plan_active and merge_grade != "FAR":
            settled_x = x + drift_x
            # nearest existing high partner to the predicted product point
            best_partner_d = None
            for hp in plan_high_partners:
                d = ((hp.get("x", 0.6922) - settled_x) ** 2 + (hp.get("y", 0.0) - landing_y) ** 2) ** 0.5
                if best_partner_d is None or d < best_partner_d:
                    best_partner_d = d
            # proximity reward window: full reward when product lands within ~contact
            # range of a partner, decaying to 0 by ~6 units away (no reward when far).
            if best_partner_d is not None and best_partner_d < 6.0:
                proximity_frac = max(0.0, 1.0 - best_partner_d / 6.0)
                if merge_grade in ("DIRECT", "NEAR"):
                    # (A) CHAIN: productive same-type merge that co-locates the product
                    # next to a partner. Additive bonus (~0..1300) — enough to prefer the
                    # partner-side merge over a far merge, never enough to beat a real
                    # crisis (excluded) or to justify a high landing on its own.
                    chain_bonus = 1300.0 * proximity_frac
                    if chain_bonus > 1.0:
                        score += chain_bonus
                        reasons.append("PLANNED_HIGH_CHAIN_2ND_PAIR")
                elif merge_grade == "NO":
                    # (B) SETUP-HOLD: is the current piece landing adjacent to an existing
                    # same-type piece (so next turn yields the product right here)?
                    near_same = False
                    for sp in same_type_pieces:
                        dsx = abs(sp.get("x", 99.0) - settled_x)
                        dsy = abs(sp.get("y", 99.0) - landing_y)
                        if dsx < 2.0 and dsy < 2.5:
                            near_same = True
                            break
                    if near_same and reactive_pair_count >= 3:
                        # Cancel axis 8.8 (-4500) for THIS co-locating hold only, plus a
                        # small partner-proximity nudge so the partner-side hold wins over
                        # an equally-flat scatter hold. Bounded: <= 4500 + 700.
                        setup_bonus = 4500.0 + 700.0 * proximity_frac
                        score += setup_bonus
                        reasons.append("PLANNED_HIGH_SETUP_HOLD")
