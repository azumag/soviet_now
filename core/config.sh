...
MODEL_LAST_RESORT="qwen35e"

GAME_COUNT_FILE="game_count.txt"

RADIO_AGENT="qwen35e"
RADIO_MAIN_AGENT="${RADIO_MAIN_AGENT:-minimax}"
RADIO_MAIN_FALLBACK="${RADIO_MAIN_FALLBACK:-opencode:glmflash}"
RADIO_MAIN_OLLAMA_FALLBACK="${RADIO_MAIN_OLLAMA_FALLBACK:-qwen35e}"
RADIO_FALLBACK="opencode:glmflash"
...
RADIO_OLLAMA_TIMEOUT="${RADIO_OLLAMA_TIMEOUT:-180}"
...
COMMENT_MAIN_AGENT="${COMMENT_MAIN_AGENT:-minimax}"
COMMENT_MAIN_FALLBACK="${COMMENT_MAIN_FALLBACK:-opencode:glmflash}"
COMMENT_MAIN_OLLAMA_FALLBACK="${COMMENT_MAIN_OLLAMA_FALLBACK:-qwen35e}"
...
ROLLBACK_POSTMORTEM_MODEL="qwen35e"
...
PENDING_CYCLE_RADIO_DIR="$TMP_STATE_DIR/pending_cycle_radios"
...
BRANCH_MAX_GAMES=$((MIN_GAMES_BEFORE_IMPROVE * 4)) # 4サイクル分 (base: 12*4=48)
...
mkdir -p "$STRATEGY_VERSIONS_DIR" "$STRATEGY_HASH_ARCHIVE_DIR" "$HISTORY_DIR" \
	...
	"$MANUAL_AUDIO_TRIGGER_DIR" "$RADIO_WEB_GROUNDING_CACHE_DIR" "$PENDING_CYCLE_RADIO_DIR" \
