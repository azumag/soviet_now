...
# --- v768: 解析器が NO と言った候補の「実際に併合する確率」を学習モデルで見積もる ---
# 実測 (2026-09-03、実戦 14386 手で学習・3 割を検証に確保):
#   * 解析器の merge_grade だけで併合を予測すると AUC 0.852。
#     幾何特徴を足したロジスティック回帰は **AUC 0.934 / 正答率 0.900**。
#   * 効いている特徴は「落下先の同型駒の上方 1.5 内にある、より高い駒の数」(係数 -2.29) と
#     「|落下x - 同型x|」(-0.91)。実測でも上方 0 個なら併合率 90.4%、6 個以上なら 0.9%。
#   * 解析器はこの集団を一律 NO と呼んでいるので、ここに勾配を入れる。
# 注意: 学習データは「戦略が実際に選んだ手」のみで、全候補への外挿には選択バイアスがある。
# そのため確率そのものを信用せず、NO 候補どうしの順位付けにだけ使う。
# V768_LEARNED_W=0 (既定) で従来と完全一致する。
_V768_BASE_BONUS = 900.0
_V768_W = {"bias": 0.624, "grade": 1.493, "dx": -0.905, "above": -2.287,
           "clear": 0.253, "pc": 0.036, "maxy": -0.021, "ntype": -0.046, "vis": -0.635}


def _v768_weight():
    """V768_LEARNED_W: 0=無効。1 で NO 候補に「予測併合確率 x 900」を加点。"""
    raw = str(os.environ.get("V768_LEARNED_W", "0") or "").strip()
...
def _v768_same_type_geometry(pieces, ntype, drop_x):
    """落下先に最も近い同型駒について (|dx|, 上方の駒数, 頭上クリアランス) を返す。"""
    same = [p for p in pieces if int(p.get("type", 0) or 0) == int(ntype)]
    if not same:
        return 5.0, 12, -2.0
    try:
        target = min(same, key=lambda p: abs(float(drop_x) - float(p.get("x"))))
        tx = float(target.get("x")); ty = float(target.get("y"))
    except (TypeError, ValueError):
        return 5.0, 12, -2.0
    above = []
    for other in pieces:
        if other is target or other.get("id") == target.get("id"):
            continue
        try:
            if float(other.get("y")) > ty and abs(float(other.get("x")) - tx) <= 1.5:
                above.append(other)
        except (TypeError, ValueError):
            continue
    try:
        top = ty + float(board_stats.seed_top_radius(int(ntype)))
        clear = (min(float(o.get("y")) - float(board_stats.seed_bottom_radius(int(o.get("type"))))
                     for o in above) - top) if above else 3.0
    except (TypeError, ValueError):
        clear = 3.0
    return min(abs(float(drop_x) - tx), 2.0), min(len(above), 12), max(-2.0, min(clear, 3.0))


def _v768_merge_probability(pieces, ntype, drop_x, piece_count, max_y, visual_same):
    """学習した係数で、その落下が併合につながる確率を見積もる (grade=NO 前提)。"""
    dx, above, clear = _v768_same_type_geometry(pieces, ntype, drop_x)
    w = _V768_W
    z = (w["bias"] + w["grade"] * 0.0 + w["dx"] * dx + w["above"] * (above / 12.0)
         + w["clear"] * clear + w["pc"] * (float(piece_count) / 50.0)
         + w["maxy"] * (max(-5.0, min(float(max_y), 5.0)) / 5.0)
         + w["ntype"] * (float(ntype) / 11.0) + w["vis"] * (1.0 if visual_same else 0.0))
    if z < -30.0:
        return 0.0
    if z > 30.0:
        return 1.0
...
def _decide_base(game_state: dict, analysis: dict) -> dict:
    ...
    for result in results:
        ...
        if _v768_w > 0.0 and merge_grade == "NO":
            try:
                _v768_p = _v768_merge_probability(pieces, int(next_type), float(x),
                                                  piece_count, max_y,
                                                  bool(reactor.get("visual_same_type_count")))
            except (TypeError, ValueError, AttributeError):
                _v768_p = 0.0
            if _v768_p > 0.0:
                score += _v768_w * _V768_BASE_BONUS * _v768_p
                reasons.append("LEARNED_MERGE_P")
