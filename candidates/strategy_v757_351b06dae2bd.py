...
def _decide_base(game_state: dict, analysis: dict) -> dict:
    ...
    # --- v757 SURFACE_DIVERSITY pre-computation ---
    # 実測（2026-08-29、実戦 14 試合）:
    #   * settled した同型ペアは「寄せ」で併合できない（近接ペア 5573 件の 3 手以内併合 3%、ドロップ位置に無関係）
    #   * 終了時の駒の 96% は相方が盤上にいるのに併合していない = 回収不能
    #   * 併合の唯一の実効経路は「落とす駒が露出した同型に届くこと」
    #     （next_type の同型が露出している手は 38%、その手の併合率は 76%）
    # よって盤面の表面（露出している駒）に**異なる型を何種類並べられるか**が併合機会を決める。
    # 各候補について「落下後に露出している型の種類数」の増減を評価する。
    _v757_exposed_by_type = {}
    for _sd_p in pieces:
        if not isinstance(_sd_p, dict):
            continue
        _sd_t = _sd_p.get("type")
        if not isinstance(_sd_t, int) or _sd_t < 1:
            continue
        try:
            _sd_x = float(_sd_p.get("x")); _sd_y = float(_sd_p.get("y"))
            _sd_r = float(board_stats.seed_horiz_radius(_sd_t))
            _sd_top = _sd_y + float(board_stats.seed_top_radius(_sd_t))
        except Exception:
            continue
        _sd_open = True
        for _sd_o in pieces:
            if _sd_o is _sd_p or _sd_o.get("id") == _sd_p.get("id"):
                continue
            try:
                _sd_ox = float(_sd_o.get("x")); _sd_oy = float(_sd_o.get("y")); _sd_ot = int(_sd_o.get("type"))
                _sd_or = float(board_stats.seed_horiz_radius(_sd_ot))
                _sd_ob = _sd_oy - float(board_stats.seed_bottom_radius(_sd_ot))
            except Exception:
                continue
            if abs(_sd_ox - _sd_x) <= _sd_r + 0.6 * _sd_or and _sd_ob >= _sd_top - 0.25 and _sd_oy > _sd_y:
                _sd_open = False
                break
        if _sd_open:
            _v757_exposed_by_type.setdefault(_sd_t, []).append((_sd_x, _sd_r, _sd_top, _sd_y))
    _v757_base_types = len(_v757_exposed_by_type)
    _v757_next_r = float(board_stats.seed_horiz_radius(next_type))
    _v757_next_bot = float(board_stats.seed_bottom_radius(next_type))
    ...
    for result in results:
        ...
        # ----- v757: SURFACE_DIVERSITY (表面に並ぶ型の種類数を増やす) -----
        if (
            merge_grade == "NO"
            and _v757_exposed_by_type
            and not deadline_crossed
            and reactor_margin >= 1.0
            and not result.get("crosses_deadline")
            and not death_spiral
            ...
            _v757_x = _as_float(x, 0.0)
            _v757_cy = _as_float(result.get("landing_y"), 0.0)
            _v757_bot = _v757_cy - _v757_next_bot
            _v757_after = 0
            for _sd_t, _sd_items in _v757_exposed_by_type.items():
                _sd_alive = 0
                for _sd_x, _sd_r, _sd_top, _sd_y in _sd_items:
                    if abs(_v757_x - _sd_x) <= _sd_r + 0.6 * _v757_next_r and _v757_bot >= _sd_top - 0.25 and _v757_cy > _sd_y:
                        continue  # この駒は落下で覆われる
                    _sd_alive += 1
                if _sd_alive > 0:
                    _v757_after += 1
            if next_type not in _v757_exposed_by_type:
                _v757_after += 1  # 落とした駒自身が新しい型を表面に出す
            _v757_d = _v757_after - _v757_base_types
            if _v757_d != 0:
                score += 200.0 * _v757_d
                if _v757_d > 0:
                    reasons.append("SURFACE_DIVERSITY")
