...
def _decide_base(game_state: dict, analysis: dict) -> dict:
    ...
    # --- v758 ARRIVAL_WEIGHTED_MATCHABILITY pre-computation ---
    # v757 (露出している型の種類数) の 2 つの欠陥を直した後継 (issue #132 P0-4):
    #   1. v757 は type 12 以上も 1 種類として数えるが、それらは直接降ってこないので
    #      併合機会にならない。実測では数えている 4.97 種類のうち 0.50 (10%) が無効だった。
    #   2. v757 は「露出しているか」の 0/1 しか見ない。実測 (プローブ 379 発火) では
    #      同じ露出でも隣接の相対高さ relief で実併合率が 79% / 59% / 20% と大きく違う。
    # ここでは「直接降る型 1..11 それぞれについて、到来したときに併合できる期待値」を積む:
    #   utility = Σ_t  arrival_weight(t) × match_prob(t の盤上での最良の状態)
    # arrival_weight: 次々ピース (既知) は 1.0、それ以外は露出寿命 ~4 手のあいだに来る確率
    #   1 - (10/11)^4 ≒ 0.316 を 11 型で等分に見た値。
    _v758_P_EXP_LOW = 0.79    # 露出 & relief < 0.5
    _v758_P_EXP_MID = 0.59    # 露出 & 0.5 <= relief <= 1.0
    _v758_P_EXP_HIGH = 0.20   # 露出 & relief > 1.0
    _v758_P_COVERED = 0.10    # 覆われている
    _v758_best = {}
    for _mq_p in pieces:
        if not isinstance(_mq_p, dict):
            continue
        _mq_t = _mq_p.get("type")
        if not isinstance(_mq_t, int) or _mq_t < 1 or _mq_t > 11:
            continue
        try:
            _mq_x = float(_mq_p.get("x")); _mq_y = float(_mq_p.get("y"))
            _mq_r = float(board_stats.seed_horiz_radius(_mq_t))
            _mq_top = _mq_y + float(board_stats.seed_top_radius(_mq_t))
        except Exception:
            continue
        _mq_open = True
        _mq_relief = -9.0
        for _mq_o in pieces:
            if _mq_o is _mq_p or _mq_o.get("id") == _mq_p.get("id"):
                continue
            try:
                _mq_ox = float(_mq_o.get("x")); _mq_oy = float(_mq_o.get("y")); _mq_ot = int(_mq_o.get("type"))
                _mq_or = float(board_stats.seed_horiz_radius(_mq_ot))
                _mq_otop = _mq_oy + float(board_stats.seed_top_radius(_mq_ot))
                _mq_ob = _mq_oy - float(board_stats.seed_bottom_radius(_mq_ot))
            except Exception:
                continue
            if abs(_mq_ox - _mq_x) <= _mq_r + 0.6 * _mq_or and _mq_ob >= _mq_top - 0.25 and _mq_oy > _mq_y:
                _mq_open = False
                break
            if abs(_mq_ox - _mq_x) <= 1.2:
                _mq_relief = max(_mq_relief, _mq_otop - _mq_top)
        if _mq_open:
            _mq_q = _v758_P_EXP_LOW if _mq_relief < 0.5 else (_v758_P_EXP_MID if _mq_relief <= 1.0 else _v758_P_EXP_HIGH)
        else:
            _mq_q = _v758_P_COVERED
        _mq_prev = _v758_best.get(_mq_t)
        if _mq_prev is None or _mq_q > _mq_prev[0]:
            _v758_best[_mq_t] = (_mq_q, _mq_x, _mq_r, _mq_top, _mq_y, _mq_open)
    _v758_w_unknown = 0.316 / 11.0
    _v758_weight = {}
    for _mq_t in range(1, 12):
        _v758_weight[_mq_t] = _v758_w_unknown + (1.0 if _mq_t == next_next_type else 0.0)
    _v758_base_u = sum(_v758_weight[t] * v[0] for t, v in _v758_best.items())
    _v758_next_r = float(board_stats.seed_horiz_radius(next_type))
    _v758_next_bot = float(board_stats.seed_bottom_radius(next_type))
    _v758_next_top_r = float(board_stats.seed_top_radius(next_type))
    ...
    for result in results:
        ...
        # ----- v758: ARRIVAL_WEIGHTED_MATCHABILITY (到来確率つきの併合可能性) -----
        if (
            merge_grade == "NO"
            and _v758_best
            and not deadline_crossed
            ...
            _mq_x0 = _as_float(x, 0.0)
            _mq_cy = _as_float(result.get("landing_y"), 0.0)
            _mq_bot = _mq_cy - _v758_next_bot
            _mq_after = 0.0
            for _mq_t, (_mq_q, _mq_px, _mq_pr, _mq_ptop, _mq_py, _mq_wasopen) in _v758_best.items():
                _mq_q2 = _mq_q
                if _mq_wasopen and abs(_mq_x0 - _mq_px) <= _mq_pr + 0.6 * _v758_next_r \
                        and _mq_bot >= _mq_ptop - 0.25 and _mq_cy > _mq_py:
                    _mq_q2 = _v758_P_COVERED  # この落下で覆う
                _mq_after += _v758_weight[_mq_t] * _mq_q2
            # 落とした駒自身は着地後に露出している (上に何も無い)。その型の状態を底上げする。
            if 1 <= next_type <= 11:
                _mq_self = _v758_P_EXP_LOW if not result.get("wall_rotation_risk") else _v758_P_EXP_MID
                _mq_cur = _v758_best.get(next_type)
                _mq_curq = _v758_P_COVERED if _mq_cur is None else _mq_cur[0]
                if _mq_self > _mq_curq:
                    _mq_after += _v758_weight[next_type] * (_mq_self - _mq_curq)
            _mq_d = _mq_after - _v758_base_u
            if _mq_d != 0.0:
                score += 900.0 * _mq_d
                if _mq_d > 0.0:
                    reasons.append("ARRIVAL_MATCHABILITY")
