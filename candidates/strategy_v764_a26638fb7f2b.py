...
# V764_NEXTNEXT_W=0 (既定) で従来と完全一致する。
_V764_BASE_BONUS = 300.0


def _v764_weight():
    """V764_NEXTNEXT_W: 0=無効。1 で「nextNext の露出同型が残る」置き方に +300。"""
    raw = str(os.environ.get("V764_NEXTNEXT_W", "0") or "").strip()
...
def _decide_base(game_state: dict, analysis: dict) -> dict:
    ...
    _v764_w = _v764_weight()
    _v764_nn_opens = None
    if _v764_w > 0.0 and isinstance(next_next_type, int) and 1 <= next_next_type <= 11:
