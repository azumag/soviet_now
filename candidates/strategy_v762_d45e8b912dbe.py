...
# --- v762: 落下後の表面の凸凹（列天端の標準偏差）を罰する ---
# 実測 (2026-08-31):
#   * 盤面に「本当に露出した同型」がある型は 1..11 のうち平均 4.37 個しかない。
#     落下型は一様なので、併合機会が生じるのは 39.6% の手だけ (4.37/11 = 39.8% と一致)。
#   * 表面が平坦なほど露出型は多い: 列天端 std の平坦上位 1/4 で 4.84 型 / 凸凹下位 1/4 で 4.12 型。
#     この差 (+0.72 型) は機会率 +6.5pt、露出同型への落下は 86.5% で併合するので
#     +0.056 併合/手 に相当し、容量モデルでは 93 -> 107 手に効く。
#   * 既存の height_penalty との違い: 候補内の順位相関 (Spearman) は
#     landing_y vs 落下後 std が median +0.733（>0.95 は 6.2% の手だけ）で、順位付けが実際に異なる。
#     一方 landing_y vs 天井押し上げ量は median +1.000（92.2% の手で >0.95）で、
#     v760 の天井項は 9 割の手で高さ罰の再重み付けにすぎなかった。
# V762_ROUGHNESS_W=0 (既定) で従来と完全一致する。
_V762_FLOOR_Y = -4.48
_V762_BASE_PENALTY = 400.0   # 落下後 std が 1 増えるごとの罰 (W=1 のとき)


def _v762_weight():
    """V762_ROUGHNESS_W: 0=無効。1 で「落下後の列天端 std +1」に -400。"""
    raw = str(os.environ.get("V762_ROUGHNESS_W", "0") or "").strip()
...
def _v762_column_tops(pieces):
    """列 (x 四捨五入 -3..3) ごとの天端。空の列は床。"""
    cols = {b: _V762_FLOOR_Y for b in range(-3, 4)}
    for piece in pieces or []:
        try:
            bucket = int(round(float(piece.get("x", 0.0))))
            top = float(piece.get("y", 0.0)) + float(board_stats.seed_top_radius(int(piece.get("type", 1))))
        except (TypeError, ValueError):
            continue
        if bucket in cols and top > cols[bucket]:
            cols[bucket] = top
    return cols


def _v762_roughness(tops):
    """列天端の母標準偏差。"""
    vals = list(tops.values())
    if not vals:
        return 0.0
...
def _decide_base(game_state: dict, analysis: dict) -> dict:
    ...
    _v762_w = _v762_weight()
    _v762_tops = _v762_column_tops(pieces) if _v762_w > 0.0 else None
    _v762_base_rough = _v762_roughness(_v762_tops) if _v762_tops else 0.0
    ...
    for result in results:
        ...
        if _v762_w > 0.0 and _v762_tops is not None:
            _v762_top = result.get("top_y_after_drop")
            if _v762_top is not None:
                try:
                    _v762_b = int(round(float(x)))
                    if _v762_b in _v762_tops:
                        _v762_after = dict(_v762_tops)
                        if float(_v762_top) > _v762_after[_v762_b]:
                            _v762_after[_v762_b] = float(_v762_top)
                        _v762_d = _v762_roughness(_v762_after) - _v762_base_rough
                        if _v762_d > 0.0:
                            score -= _v762_w * _V762_BASE_PENALTY * _v762_d
                            reasons.append("SURFACE_ROUGHNESS")
                except (TypeError, ValueError):
                    pass
