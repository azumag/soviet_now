...
# --- v764: 落下後に nextNext の露出同型が残る（作れる）置き方を優先する ---
# 実測 (2026-08-31、実戦 304 手、候補ごとに post-state を作って評価):
#   * 実際に選んだ手の後、nextNext の露出同型がある割合は 36.5%。
#   * 同じ手の中で最良の候補を選べば 45.1% まで上がる（+8.6pt）。
#   * うち「作れたのに作らなかった」が 8.6%、「どの候補でも作れない」が 54.9%。
#   * 機会がある手と無い手の併合差は 1.291 対 0.169 なので、+8.6pt は上界で
#     +0.097 併合/手 に相当する。これは目標の +0.09 と同じ桁で、
#     これまで測ったどのレバー（v763 の +0.025 が最大）より大きい。
# 既存の AVOID_BLOCK_NEXTNEXT / NEXTNEXT_TWIN_COVER_AVOID は「既にあるレーンを潰さない」
# 側の軸で、ここは「レーンが残る／生まれる置き方を選ぶ」側。nextNext == next のときは
# 落とした駒自身が露出同型になるので、その場合も加点対象になる。
# V767_BUNDLE=0 (既定) で従来と完全一致する。
_V764_BASE_BONUS = 300.0


def _v764_weight():
    """V767_BUNDLE: 0=無効。1 で束ね候補 (nextNext レーン + 壁際回避) を有効化。"""
    raw = str(os.environ.get("V767_BUNDLE", "0") or "").strip()
...
def _v764_nn_open_after(nn_opens, nn_type, drop_type, drop_x, drop_y):
    """落とした後に nextNext 型の露出同型が残るか。

    落とす駒自身が nextNext と同型なら、上に何も無いので必ず残る。
    それ以外は、既存の露出駒のうち 1 つでも真上帯に入らずに済めば残る。
    """
    if int(nn_type) == int(drop_type):
        return True
    if not nn_opens:
        return False
    try:
        ...
        band = float(board_stats.seed_horiz_radius(nn_type)) + drop_band
    except (TypeError, ValueError):
        return True
    for piece in nn_opens:
        try:
            px = float(piece.get("x"))
            py = float(piece.get("y"))
            ptop = py + float(board_stats.seed_top_radius(nn_type))
        except (TypeError, ValueError):
            return True
...
# --- v767: 型 5 以上を壁際へ落とさない ---
# 実測 (非併合落下 3529 件、型 5-8): その駒が 30 手以内に併合できた割合は
#   壁からの距離 <0.8 で 27.3〜59.7%、>=0.8 で 42.7〜68.3% と、max_y のどの帯でも
#   -6.6 〜 -15.4 ポイント一貫して低い。型 1-4 では差が +2.6 で不利が無いので、
#   中型以上だけが対象。大きい駒は壁際だと横から届かず、レーンも塞ぐ。
_V767_WALL_X = 3.44          # 実測の壁位置 (型別に max|x| + 物理半径 を取った中央値)
_V767_WALL_MARGIN = 0.8
_V767_WALL_PENALTY = 250.0
_V767_WALL_MIN_TYPE = 5
...
def _decide_base(game_state: dict, analysis: dict) -> dict:
    ...
    _v767_on = _v764_weight()
    _v764_nn_opens = None
    if _v767_on > 0.0 and isinstance(next_next_type, int) and 1 <= next_next_type <= 11:
        _v764_nn_opens = _v742_open_twin(pieces, next_next_type)
    ...
    for result in results:
        ...
        if _v767_on > 0.0:
            if _v764_nn_opens is not None:
                try:
                    if _v764_nn_open_after(_v764_nn_opens, int(next_next_type), int(next_type),
                                           float(x), float(landing_y)):
                        score += _v767_on * _V764_BASE_BONUS
                        reasons.append("NEXTNEXT_LANE_KEPT")
                except (TypeError, ValueError):
                    pass
            if merge_grade == "NO" and int(next_type) >= _V767_WALL_MIN_TYPE:
                if abs(float(x)) >= _V767_WALL_X - _V767_WALL_MARGIN:
                    score -= _v767_on * _V767_WALL_PENALTY
                    reasons.append("WALL_AVOID_MID_TYPE")
