...
def decide(game_state: dict, analysis: dict) -> dict:
    """振り子パターンを回避し、中程度のバランス補正と控えめなchain reaction緩和を統合"""
    ...
    # reactor情報（控えめなchain reaction緩和）
    ...
    for result in results:
        ...
        # 2. 高度によるペナルティ（v78: 控えめなchain reaction緩和、v77の高度ペナルティ緩和は削除）
        if phase == "CRITICAL":
            ...
            # v78: chain reaction中は高度管理緩和（控えめ）
            # v31のreactive_pairs>=3を>=2に緩和
            if reactive_pairs >= 2:
                height_multiplier = 35.0  # chain reaction中は緩和
            else:
                height_multiplier = 50.0  # v42の50.0を維持
        ...
        # 3. ドリフトによるペナルティ（v78: v77の35.0から30.0に戻しシンプル化）
        drift_penalty = (abs(drift_x) + drift_unc) * 30.0
        score -= drift_penalty

        # 4. 左右バランス補正（v78: v42より緩やかだがv77より強い中程度の補正）
        # v42: 20.0/30.0/40.0 → v78: 15.0/20.0/25.0
