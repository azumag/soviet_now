...
#       - 履歴分析(致命的): ターン44-46でmax_y=1.11→2.14→3.07に急増しゲームオーバー
#       - 履歴分析(致命的): ターン44-46で左側に連続3ターンドロップし続け、旗側max_yが上昇
#       - 履歴分析(致命的): ターン42-44でtype11とtype8が左側に配置され、max_y急増の原因となった
#       - 履歴分析: 最終盤面左側type9+=5,max_y=1.04,右側type9+=2,max_y=-1.10(旗側集約失敗)
#       - 履歴分析: 旗側max_yが高くならないよう早期に旗側変更が必要
#       - 高度危機回避の早期化: max_y>1.2で発動(v036: 1.0→1.2,条件厳格化)
#       - 高度危機回避の旗側ドロップ: 高度危機時は常に旗側の反対側にドロップして旗側max_yを低下させる(v042新規)
#       - 中程度危機回避の旗側ドロップ: 中程度危機時も旗側の反対側にドロップ(v042新規)
#       - 旗側max_y管理の超厳格化: 旗側max_y>1.2なら即時旗側変更(v036: 1.3→1.2,早期化)
#       - 旗側変更ロジックの簡素化: 反対側max_y<1.5かつ旗側max_y>反対側+0.5なら旗側変更(v036: 0.3→0.5,より厳格化)
#       - 旗側max_y管理の強化: max_y>0.8かつ旗側max_y>1.5かつ旗側max_y>反対側なら旗側変更(v036: 0.7→0.8)
#       - 旗側変更クールダウンの短縮: 2ターン(v036: 2→2,維持)
#       - マージ戦略の超緊急時強化: max_y>1.5ならscore>-15のマージを許容(v042改善: -8→-15)
#       - マージ戦略の緊急時改善: max_y>1.0ならscore>-10のマージを許容(v042改善: -5→-10)
#       - 大型ピース旗側配置の改善: 旗側max_y>1.5なら旗側変更してから配置(v036: 1.0→1.5)
#       - クールダウン中旗側回避の条件緩和: max_y>1.0で無効化(v036: 0.8→1.0)
#       - シェイク戦略の発動条件緩和: 無マージ3ターンで発動(v036: 3→3,維持)
#       - 次のピース保護の改善: 旗側max_y>1.5なら旗側にドロップしない(v036新規)
#
# v043: マージ超最優先化と旗側変更の条件厳格化(2026-02-25 最新)
#       - 履歴分析(致命的): ターン69-71でmerge_available=trueだが「旗側変更(危機)」が優先されマージ見逃し
#       - 履歴分析(致命的): ターン69-71でmax_y=3.34→2.07→2.54に推移し、マージの機会を逃した
#       - 履歴分析(致命的): ターン56-73で旗側が2ターンおきに頻繁に変更され、type9+が左右散在した
#       - 履歴分析(致命的): ターン69で旗側max_y=2.20だが旗側変更し、反対側max_y=2.82も高く無意味
#       - 履歴分析(致命的): ターン73で左側max_y=2.996,右側max_y=2.541でtype9+散在
#       - 履歴分析: マージ成功率84.2%だが、危機的状況でのマージ見逃しが致命的
#       - マージ判定の超最優先化: 危機回避より先にマージ判定を実行(v043新規)
#       - マージ閾値の超緩和: max_y>2.0ならscore>-20のマージを許容(v043: -15→-20)
#       - マージ閾値の大幅緩和: max_y>1.5ならscore>-20のマージを許容(v043: -15→-20)
#       - マージ閾値の緩和: max_y>1.0ならscore>-15のマージを許容(v043: -10→-15)
#       - 旗側変更条件の超厳格化: 反対側max_y<1.0かつ旗側max_y>反対側+0.5なら旗側変更(v043: 1.5→1.0,0.5→0.5)
#       - 旗側変更条件の厳格化: 反対側max_y<1.5かつ旗側max_y>反対側+0.7なら旗側変更(v043新規)
#       - 旗側max_y管理の超厳格化: 旗側max_y>1.5かつ反対側max_y<1.0なら即時旗側変更(v043新規)
#       - 旗側max_y管理の厳格化: 旗側max_y>1.3かつ反対側max_y<1.2なら旗側変更(v043: 1.2→1.3)
#       - 旗側変更クールダウンの延長: 4ターン(v042: 2→4,安定性向上)
#       - 高度危機回避の旗側ドロップ強化: 旗側max_y>反対側max_yなら旗側の反対側にドロップ(v043改善)
#       - 大型ピース旗側配置の厳格化: 旗側max_y>1.2なら反対側に配置(v043: 1.5→1.2)
#       - type9+旗側集約の強化: type9+ドロップ時に旗側max_yを監視し、散在を防止(v043新規)
#       - 反応器状態の活用: reactive_pairs>0ならマージ候補を優先(v043新規)
...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- v043改善: マージ判定の超最優先化(危機回避より先にマージ判定を実行) ---
    mergeable_results = []
    for r in results:
    ...
    if mergeable_results:
        # v043改善: 超致命時マージ戦略(max_y>2.0ならscore>-20のマージを許容)
        if max_y > 2.0:
            ...
            if valid_merges:
                ...
                return {
                    "x": x,
                    "reason": f"マージ(超致命) x={x:.2f} (score={score:.1f})",
                }

        # v043改善: 超緊急時マージ戦略(max_y>1.5ならscore>-20のマージを許容)
        if max_y > 1.5:
            valid_merges = [r for r in mergeable_results if r.get("score", 0) > -20]
            if valid_merges:
                best = max(valid_merges, key=lambda r: r.get("score", 0))
        ...
        # v043改善: 緊急時マージ戦略(max_y>1.0ならscore>-15のマージを許容)
        if max_y > 1.0:
    ...
    # --- v043改善: 旗側変更ロジックの超厳格化(反対側max_y<1.0かつ旗側max_y>反対側+0.5なら旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        ...
        # v043改善: 反対側max_y<1.0かつ旗側max_y>反対側+0.5なら旗側変更
        if opposite_side_max_y < 1.0 and flag_side_max_y > opposite_side_max_y + 0.5:
            _flag_side = "right" if _flag_side == "left" else "left"
            _flag_change_cooldown = 4  # v043改善: 4ターンに延長
    ...
    # --- v043改善: 旗側変更ロジックの厳格化(反対側max_y<1.5かつ旗側max_y>反対側+0.7なら旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
        ...
        # v043改善: 反対側max_y<1.5かつ旗側max_y>反対側+0.7なら旗側変更
        if opposite_side_max_y < 1.5 and flag_side_max_y > opposite_side_max_y + 0.7:
            _flag_side = "right" if _flag_side == "left" else "left"
            _flag_change_cooldown = 4
            target_x = 2.8 if _flag_side == "right" else -2.8
            _consecutive_no_merge += 1
            _last_drop_x = target_x
            return {
    ...
    # --- v043改善: 旗側max_y管理の超厳格化(旗側max_y>1.5かつ反対側max_y<1.0なら即時旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
    ...
    # --- v043改善: 旗側max_y管理の厳格化(旗側max_y>1.3かつ反対側max_y<1.2なら旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
    ...
    # --- v043改善: クールダウン中旗側回避の無効化(マージ可能なら旗側回避をスキップ) ---
    if _flag_side is not None:
        ...
        if max_y <= 1.0:
            if flag_side_max_y > 1.2 and flag_side_max_y > opposite_side_max_y:
    ...
    # --- v043改善: 高度危機回避の旗側ドロップ強化(max_y>1.2で発動、旗側max_y>反対側なら旗側の反対側にドロップ) ---
    if max_y > 1.2:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # v043改善: 旗側max_y>反対側max_yなら旗側の反対側にドロップ
        if _flag_side == "left":
            flag_side_max_y = left_max_y
            opposite_side_max_y = right_max_y
        else:
        ...
        if flag_side_max_y > opposite_side_max_y:
            target_x = 2.8 if _flag_side == "left" else -2.8  # 旗側の反対側
        else:
    ...
    # --- v043改善: 中程度危機回避の旗側ドロップ(max_y>0.5で発動、旗側max_y>反対側なら旗側の反対側にドロップ) ---
    if max_y > 0.5:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
    ...
    # --- v043改善: 大型ピース旗側配置(type7-12を旗側に配置) ---
    if _flag_side is not None and 7 <= next_type <= 12:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # v043改善: 旗側max_y>1.2なら反対側に配置
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
        opposite_side_max_y = right_max_y if _flag_side == "left" else left_max_y
    ...
    # --- v043改善: シェイク戦略の発動条件緩和(無マージ3ターンで発動) ---
    _consecutive_no_merge += 1
