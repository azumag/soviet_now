...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    if max_y < 0.8:
        ...
        phase = "MEDIUM"
        height_mult = 2.4  # v133: v42の2.4を維持
        merge_mult = 1.0
    elif max_y < 3.0:
        phase = "HIGH"
        height_mult = 1.8  # v133: v128の1.8を復帰（v132の1.6緩和は過剰）
        merge_mult = 1.0
    else:
        phase = "CRITICAL"
        height_mult = 1.0  # CRITICAL: height_multなし
        merge_mult = 0.6  # v133: v42の0.6を維持（v132の0.8強化は過剰）
    ...
    for result in results:
        ...
        # 1. マージグレードによるスコア（v133: v42の強力な値を維持）
        ...
        # 2. 高度によるペナルティ（v133: v42/v128の設定を採用）
        height_penalty = landing_y * 50.0 * height_mult

        # HIGH_TOWERペナルティ（v133: MEDIUMはv42、HIGHはv128）
        if phase == "HIGH" and landing_y > 0.5:
            height_penalty *= 1.3  # v133: v128の1.3倍を維持
            reasons.append("HIGH_TOWER")
        elif phase == "MEDIUM" and landing_y > 0.5:
            height_penalty *= (
                1.5  # v133: v42の1.5倍を採用（v131の過剰性ではなくv42のバランス）
            )
        ...
        # 3. ドリフトによるペナルティ（v133: v42の一律30.0を維持）
        drift_penalty = (abs(drift_x) + drift_unc) * 30.0
        score -= drift_penalty

        # 4. 左右バランス補正（v133: v42の設定を維持）
        ...
        # 5. nextNextが同じタイプなら中央寄せボーナス（v133: v42の一律50.0を維持）
