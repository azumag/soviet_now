...
def decide(game_state: dict, analysis: dict) -> dict:
    """v19のシンプル構造をベースに、マージなしペナルティ削除とreactor活用で振り子を解消"""
    ...
    # reactor情報（v40: v31のアイデを採用しつつ条件を緩和）
    ...
    # フェーズ判定（v40: v19の閾値0.8/1.8/3.0を維持）
    if max_y < 0.8:
        phase = "LOW"
        height_mult = 1.0
        merge_mult = 1.2
        ...
        phase = "MEDIUM"
        height_mult = 2.4  # v40: v19の2.4を維持（HIGH到達遅延）
        merge_mult = 1.0
    elif max_y < 3.0:
        phase = "HIGH"
        height_mult = 2.6  # v40: v19の2.6を維持（マージ機会確保）
        merge_mult = 1.0
    else:
        phase = "CRITICAL"
        height_mult = 1.0  # CRITICAL: height_multなし
        merge_mult = 0.6  # v40: v19の0.6を維持（マージ優先）
    ...
    for result in results:
        ...
        # === v40: マージなしペナルティ削除・reactor活用戦略 ===

        # 1. マージグレードによるスコア（v40: v19の成功値に戻す）
        if merge_grade == "DIRECT":
            score += 1200.0 * merge_mult  # v40: v39の1800からv19の1200に戻す
            reasons.append("DIRECT_MERGE")
        elif merge_grade == "NEAR":
            score += 600.0 * merge_mult  # v40: v39の900からv19の600に戻す
            reasons.append("NEAR_MERGE")
        elif merge_grade == "FAR":
            score += 200.0 * merge_mult  # v40: v39の400からv19の200に戻す
            reasons.append("FAR_MERGE")

        # 2. 高度によるスコア（v40: chain reaction時に緩和）
        if phase == "CRITICAL":
            ...
            # v40: chain reaction時（reactive_pairs >= 2）にheight_penalty緩和
            height_penalty_factor = 1.0
            height_multiplier = 50.0

            if reactive_pairs >= 2:
                # chain reaction中は高度管理を緩和（v31のアイデ、条件を>=2に緩和）
                height_multiplier = 35.0
                height_penalty_factor = 0.8
            ...
            if phase == "HIGH" and landing_y > 0.5:
                height_penalty *= 2.0  # v40: v19の2.0を維持
                reasons.append("HIGH_TOWER")
            elif phase == "MEDIUM" and landing_y > 0.5:
                height_penalty *= 1.5  # v40: v19の1.5を維持
        ...
        # 3. ドリフトによるペナルティ（v40: v19の値を維持）
        if phase == "HIGH":
            drift_penalty = (abs(drift_x) + drift_unc) * 35.0  # v40: v19の35.0を維持
        elif phase == "MEDIUM":
            drift_penalty = (abs(drift_x) + drift_unc) * 35.0  # v40: v19の35.0を維持
        else:  # LOW, CRITICAL
        ...
        # 4. 左右バランス補正（v40: v19の値を維持）
        balance_strength = 20.0
        if phase == "HIGH":
            balance_strength = 40.0  # v40: v19の40.0を維持
        elif phase == "MEDIUM":
            balance_strength = 30.0  # v40: v19の30.0を維持
        ...
        # 5. nextNextが同じタイプなら中央寄せボーナス（v40: v19の設定を維持）
        if next_next_type == next_type:
            if phase == "CRITICAL":
                center_bonus = max(0, 1.0 - abs(x) / 2.0) * 60.0  # v40: v19の成功値
            else:
                center_bonus = max(0, 1.0 - abs(x) / 2.0) * 50.0  # v40: v19の成功値
            score += center_bonus
