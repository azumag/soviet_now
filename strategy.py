...
# --- Change History ---
      ...
      # v531b: axis 8.8b merge drought pressure at rp=1-2 — fill gap where axis 8.8 doesn't fire
      # Gap: axis 8.8 fires at rp>=3 && NO merge (-4500 flat). At rp=1-2 && NO merge, no penalty exists.
      # Worst game turns 60-75: 15 NO merge turns with rp=3, max_y runaway 1.97→3.26.
      # Best_score5801 has axis 8.8b: rp=1-2 && NO merge && pc>=28 → graduated penalty.
      # At pc=28: -100. At pc=35: -800. At pc=40: -1300. Prevents rp=1-2 → rp=3 escalation.
      # refs: tmp/analysis_result.md, strategy_versions/best_score5801_strategy.py (axis 8.8b)
      # Fixes rollback failure mode: rp=1-2 NO merge death spiral (axis 8.8b gap)
...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    max_y = max([p["y"] for p in pieces]) if pieces else -2.041
    ...
    russia_phase = russia_phase_count >= 2
    ...
    if same_type_pieces:
        ...
        same_type_stack_top = max(same_type_pieces, key=lambda p: p.get("y", -13))
    ...
    for result in results:
        ...
        if merge_grade == "NEAR" and landing_y > 0 and reactor_margin < 1.0:
            risk_factor = min(1.0, max(-1.2501, 1.0 - reactor_margin))
            ...
            if piece_count >= 33:
                pc_risk_scale = 0.463 + (piece_count - 32) * 0.25
            else:
                pc_risk_scale = 1.0
                ...
                if stacking_congested:
                    ...
                    for sp in same_type_pieces:
                        ...
                        if min_merged_dist < float("inf"):
                            chain_score = max(0, 281.6 - min_merged_dist * 80.0)
                            if sp_y > 1.0:
                                chain_score *= max(0, 1.637 - (sp_y - 1.0) * 0.5)
        ...
        if reactive_pair_count >= 1 and merge_grade == "NO" and same_type_stack_top is None:
            ...
            if best_adjacent_target is not None and best_adjacent_dist < 3.0:
                pipeline_bonus = max(0, 80.0 - best_adjacent_dist * 37.03)
        ...
        if next_next_type == next_type:
            center_bonus = max(0, 1.202 - abs(x) / 2.213) * 50.0
            score += center_bonus
        ...
        if max_type_on_board >= 6 and not (reactive_pair_count >= 3 and merge_grade == "NO"):
            ...
            if growth_center:
                ...
                if horiz_dist < 2.5:
                    ...
                    proximity = max(0, 100.0 - horiz_dist * 52.25)
        ...
        if merge_grade in ["DIRECT", "NEAR"] and result.get("merges"):
            ...
            if merges:
                ...
                if len(nearby_pieces) >= 2:
                    ...
                    chain_bonus = (chain_distance_max - dist) * chain_bonus_multiplier * 0.1770
                    score += chain_bonus
        ...
        # === Axis 9.10: high-type growth pipeline guidance (v610) ===
        ...
        #       strategy_versions/best_score6058_strategy.py (Hall-of-fame)
              ...
              if merge_grade in ["DIRECT", "NEAR"]:
                  ...
                  if russia_phase_second:
                      ...
                      if reactive_pair_count >= 1:
                          if merge_grade == "DIRECT":
                              score += 1994.2 if reactive_pair_count >= 3 else 1400.0
                          else:
                      ...
                      if reactive_pair_count >= 1:
                          ...
                          if merge_grade == "DIRECT":
                              ...
                              score += 1375.2 if reactive_pair_count >= 3 else 1000.0
                      else:
        ...
        # ----- axis 8.8b: merge drought pressure at rp=1-2 (NEW) -----
        # Gap: axis 8.8 fires at rp>=3 && NO merge (-4500 flat).
        # At rp=1-2 && NO merge, no merge drought pressure exists → HEIGHT_CONTROL default.
        # Graduated penalty scales with piece_count (key failure predictor).
        ...
        #       game_history/20260330_144015_score0665.jsonl T60-61,
        #       game_history/20260330_143501_score0994.jsonl T74-75
        if merge_grade == "NO" and not russia_phase and result.get("crosses_deadline", True):
            score -= 1200.0
