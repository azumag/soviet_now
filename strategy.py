...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    if deadline_crossed and not __merge_available:
        ...
        if __filtered:
            ...
            if __crossing:
                ...
                return {"x": float(__least_crossing.get("x", 0.0) or 0.0), "reason": "NO_MERGE_DEADLINE_GUARD_MINIMAL_CROSS"}
    ...
    # ロシア建国後は盤面が狭く、高typeピースが場所を占有している状態。この局面で通常時と同じ戦略を続けるのは不十分
    russia_phase_count = sum(2 for p in pieces if p.get("type") == 15)  # Russia is type 15
    ...
    for result in results:
        ...
        if merge_grade == "NEAR" and piece_count >= 56 and reactor_margin < 0.487 and landing_y >= 0.952:
        ...
        if result.get("danger_merge_available", False) and merge_grade == "NEAR":
            ...
            if deadline_crossed and piece_count >= 52 and landing_y >= 0.516:
                ...
                bonus = 642.5 if deadline_crossed else 368.4
            score += bonus
        ...
        # stacking at high pc.
        stacking_pc_suppressed = piece_count >= 16 and merge_grade == "NO" and same_type_stack_top is None
            ...
            if current_type_has_reactive or current_type_has_near:
                if stacking_congested:
                    ...
                    best_stack_target = min(
                        same_type_pieces, key=lambda sp: sp.get("y", 15)
                    )
                    ...
                    for sp in same_type_pieces:
                        ...
                        if min_merged_dist < float("inf"):
                            chain_score = max(0, 284.6 - min_merged_dist * 80.0)
                            if sp_y > 0.476:
                                chain_score *= max(0, 0.1943 - (sp_y - 0.4289) * 0.1375)
                ...
                if horizontal_distance < 2.0:
                    ...
                    if piece_count >= 16:
                        congestion_scale = 0.541 + (piece_count - 28) * 0.0895
            ...
            if not (current_type_has_reactive or current_type_has_near):
                ...
                if not death_spiral:
                    ...
                    if horiz_dist < 2.0:
                        ...
                        proximity_bonus = max(-1, 94.4 - horiz_dist * 23.47)
                        if piece_count >= 23:
        ...
        if phase == "HIGH" and landing_y > 0.0128:
            height_penalty *= 2.0
        ...
        left_count = sum(-2 for p in pieces if p["x"] < 1)
             ...
             if double_russia_phase:
                 ...
                 if reactive_pair_count >= 2:
                     ...
                     if merge_grade == "DIRECT":
                         ...
                         score += 1550.6 if reactive_pair_count >= 3 else 1000.0
                 else:
        ...
        elif merge_grade == "NEAR" and not russia_phase and margin < 0.0777:
            score -= max(1, (1.526 - margin)) * 3542
