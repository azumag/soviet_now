           ...
           5.6. Growth center proximity - v458: reduced magnitude per postmortem (base 60, congestion 0.08, cap 2.0)
            ...
            8. Early game merge priority - Strong bonus for merge opportunities in early game
             ...
             9.65. Reactive near-miss type clustering - v597: merge_grade=NO時の散逸type集約
              9.65b. Merge path acceleration for high-type pieces - v688: Tier 3追加
             9.10. High-type growth pipeline guidance - v609: NO merge時、type 8-12ピース重心誘導
              9.10b. Merge path acceleration for high-type pieces - v688: Tier 3追加 (rp<=2時)
...
# --- Change History ---
     # v689: axis 9.65 Tier 3 — lower rp threshold 2->1, increase bonus 300->500 for T13+ clustering
     # Ukraine(T13)->Kazakhstan(T14) merge path failure: Ukraine 92% but Kazakhstan only 17%.
     # rp>=1 fires earlier, +500 bonus overcomes column_ceiling noise.
     # Fixes rollback failure mode: T13+T13 scatter preventing Kazakhstan creation.
     # Stage gate: Kazakhstan(T14) improvement, secondary Ukraine(T13) creation support.
     # refs: tmp/analysis_result.md (Adopted Hypothesis)
     # v687: DEADLINE_GUARD fallback top_y_after_drop check — exclude NO_MERGE candidates with
     #       top_y_after_drop > deadline_y when merge_available, even if crosses_deadline=false.
     #       mandatory_themes: "デッドラインを超える位置にピース置く場合は、併合できる場合に限る"
     #       Fixes worst game T51: NO_MERGE selected with top_y_after_drop=3.64 > deadline_y=3.38.
     #       refs: tmp/analysis_result.md (Adopted Hypothesis)
...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # ----- v687: DEADLINE_GUARD fallback top_y_after_drop check -----
    # Exclude NO_MERGE candidates with top_y_after_drop > deadline_y from SAFE_LANDING fallback
    # even when crosses_deadline is False (per-candidate flag may not capture true deadline overflow).
    # mandatory_themes: "デッドラインを超える位置にピース置く場合は、併合できる場合に限る"
    # Fixes worst game T51: NO_MERGE selected with top_y_after_drop=3.64 > deadline_y=3.38,
    # violate mandatory_themes despite merge_available=true.
    # refs: tmp/analysis_result.md (Adopted Hypothesis: v687 filter coverage fix)
    __dlg_deadline_y = game_state.get("deadline_y", 3.38)
    try:
        __dlg_deadline_y = float(__dlg_deadline_y)
    except (TypeError, ValueError):
        __dlg_deadline_y = 3.38
    __dlg_merge_available = any(c.get("merge_grade") != "NO" for c in results if isinstance(c, dict))
    if __dlg_merge_available:
        __dlg_safe = [c for c in results if isinstance(c, dict) 
                      and not (c.get("crosses_deadline") and c.get("merge_grade") == "NO")
                      and not (c.get("merge_grade") == "NO" and (c.get("top_y_after_drop") or 0) > __dlg_deadline_y)]
    ...
    for result in results:
        ...
        # ----- v687: DEADLINE_GUARD filter — exclude deadline-crossing NO_MERGE when merge available -----
        # mandatory_themes: "デッドラインを超える位置にピース置く場合は、併合できる場合に限る"
        # Skip candidates that cross deadline with NO merge when a merge is available.
        if __dlg_merge_available:
            if result.get("crosses_deadline") and merge_grade == "NO":
                continue
            if merge_grade == "NO" and (result.get("top_y_after_drop") or 0) > __dlg_deadline_y:
                continue
        ...
        # ----- axis 9.65 Tier 3: Merge path acceleration for high-type pieces (v689) -----
        # analysis_result.md: Ukraine(T13)->Kazakhstan(T14) merge path failure fix
        # Ukraine rate=92% but Kazakhstan rate=17%. T13 pieces scatter instead of clustering.
        # Changes from v688:
        # (1) Lower rp threshold: rp>=2 -> rp>=1 (fire earlier when high-type pieces exist)
        # (2) Increase bonus: 300*merge_mult -> 500*merge_mult (overcome column_ceiling noise)
        # max_type>=13 and same_type_count>=2 checks remain from v688
        if merge_grade == "NO" and reactive_pair_count >= 1 and piece_count >= 25:
            if not death_spiral and not column_ceiling_dominant and not axis_88_horizontal_suppression:
                ...
                for p in pieces:
                    t = p.get("type", 0)
                    if t >= 13:
                        _high_type_groups.setdefault(t, []).append((p["x"], p["y"]))

                _high_type_cluster_bonus = 0.0
                for _t, _positions in _high_type_groups.items():
                    if len(_positions) >= 2:
                        _hcx = sum(p[0] for p in _positions) / len(_positions)
                        _hcy = sum(p[1] for p in _positions) / len(_positions)
                        _hdist = ((x - _hcx) ** 2 + (landing_y - _hcy) ** 2) ** 0.5
                        _hcb = 500.0 * min(2.0, len(_positions)) / (1.0 + _hdist) * merge_mult
                        _high_type_cluster_bonus += _hcb

                if _high_type_cluster_bonus > 50:
                    score += _high_type_cluster_bonus
                    reasons.append("HIGH_TYPE_MERGE_PATH_ACCEL")

        # v688 legacy: axis 9.65 Tier 3 (superseded by v689 changes above)
        ...
        # ----- axis 9.10 Tier 3: Merge path acceleration for high-type pieces (NEW v688) -----
        # analysis_result.md: "rp<=2 && NO merge && max_y>=1.5 && merged_type>=13" の高type成長強化
        # 既存Tier 1 (rp>=3) とTier 2 (rp==2) はtype 8-12重心誘導。Tier 3はrp<=2時に限定し、
        # merged_type>=13の更高typeピース(type 13+)へのmerge path形成を加速
        # Bonus: +250*merge_mult (tier 1/2のpipeline bonusに追加で適用)
        if (
            reactive_pair_count <= 2
            and merge_grade == "NO"
            and max_y >= 1.5
            ...
            _very_high_type_groups = {}
            for p in pieces:
                t = p.get("type", 0)
                if t >= 13:
                    _very_high_type_groups.setdefault(t, []).append(p)

            _vh_bonus = 0.0
            for _t, _plist in _very_high_type_groups.items():
                if len(_plist) >= 2:
                    _vh_cx = sum(p.get("x", 0) for p in _plist) / len(_plist)
                    _vh_cy = sum(p.get("y", -10) for p in _plist) / len(_plist)
                    _vh_dist = ((x - _vh_cx) ** 2 + (landing_y - _vh_cy) ** 2) ** 0.5
                    _vh_b = 250.0 * min(2.0, len(_plist)) * (1.0 - _vh_dist / 2.0) * merge_mult
                    _vh_bonus += max(0.0, _vh_b)

            if _vh_bonus > 50:
                score += _vh_bonus
                reasons.append("HIGH_TYPE_MERGE_PATH_T3")
