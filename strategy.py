...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    for result in results:
        ...
        # ----- axis 9.65 Tier 3: Merge path acceleration for high-type pieces (NEW v688) -----
        # analysis_result.md: "visual_same_type_count>=2 && merged_type>=13" の高typeクラスタリング強化
        # Ukraine(T13)段階ではtype 13ピースが11/12ゲームで作成されるが、T13→T14への成長が滞る
        # 原因: 同type(T13+T13)のピースが盤面に散在し、併合パスが形成されない
        # Tier 3: merge_grade==NO && rp>=2 && pc>=25 の既存条件基础上に、
        # merged_type>=13 && 同typeが2つ以上存在するケースで重心誘導を強化
        # Bonus: +300*merge_mult (既存Tier 1/2のclustering bonusに追加で適用)
        if merge_grade == "NO" and reactive_pair_count >= 2 and piece_count >= 25:
            if not death_spiral and not column_ceiling_dominant and not axis_88_horizontal_suppression:
                _high_type_groups = {}
                for p in pieces:
                ...
                for _t, _positions in _high_type_groups.items():
                    if len(_positions) >= 2:
                        ...
                        _hcb = 300.0 * min(2.0, len(_positions)) / (1.0 + _hdist) * merge_mult
