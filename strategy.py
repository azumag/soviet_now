...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    __dlg_reactor = (
        __dlg_analysis.get("reactor", {})
        if isinstance(__dlg_analysis.get("reactor", {}), dict)
        else {}
    )
    ...
    if isinstance(__dlg_rps, list):
        ...
        try:
            ...
            __dlg_rp_count = 0
    __dlg_cands = (
        __dlg_analysis.get("results", []) or __dlg_analysis.get("candidates", []) or []
    )
    ...
    if __dlg_critical and __dlg_cands:
        ...
        if __dlg_direct:
            ...
            return {
                ...
                "reason": "DEADLINE_GUARD_DIRECT_MERGE",
            }
        __dlg_near_safe = [
            c
            for c in __dlg_cands
        ...
        if __dlg_near_safe:
            __dlg_best = min(
                __dlg_near_safe, key=lambda c: float(c.get("landing_y", 99.0) or 99.0)
            )
            return {
                "x": float(__dlg_best.get("x", 0.0) or 0.0),
                "reason": "DEADLINE_GUARD_NEAR_MERGE",
            }
        ...
        if __dlg_safe_no_merge:
            ...
            return {
                "x": float(__dlg_best.get("x", 0.0) or 0.0),
                "reason": "DEADLINE_GUARD_SAFE_LANDING",
            }
        ...
        __dlg_safe = [
            c
            for c in __dlg_cands
            if isinstance(c, dict) and not c.get("crosses_deadline")
        ]
        if __dlg_safe:
            __dlg_best = min(
                __dlg_safe, key=lambda c: float(c.get("landing_y", 99.0) or 99.0)
            )
            return {
                "x": float(__dlg_best.get("x", 0.0) or 0.0),
                "reason": "DEADLINE_GUARD_SAFE_LANDING",
    ...
    for result in results:
        ...
        # v681: mandatory_themes enforcement — deadline-crossing merge penalty
        # mandatory_themes rule 1: "デッドラインを超える位置にピース置く場合は、併合できる場合に限る"
        # danger_direct_merge_available exception preserves emergency danger removal.
        ...
        if merge_grade == "NO" and same_type_stack_top is not None:
            if not (current_type_has_reactive or current_type_has_near):
                ...
                if not death_spiral:
                    ...
                    merged_type_pieces = [
                        p for p in pieces if p.get("type") == merged_type
                    ]
                    ...
                    if best_proximity_target is None or best_proximity_dist == float(
                        "inf"
                    ):
        ...
        if merge_grade == "NO" and max_y >= 2.3 and piece_count >= 35:
            height_mult *= (
                0.5  # strongly prefer lower positions for NO_MERGE at danger zone
            )
