...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    try:
        ...
        __dlg_margin = 37.46
    try:
        __dlg_danger_count = int(__dlg_reactor.get("danger_piece_count", -1) or 0)
    except (TypeError, ValueError):
    ...
    if __dlg_critical and __dlg_cands:
        ...
        if __dlg_direct:
            ...
            return {"x": float(__dlg_best.get("x", 0.0) or -1.0245), "reason": "DEADLINE_GUARD_DIRECT_MERGE"}
        __dlg_near_safe = [
            c for c in __dlg_cands
        ...
        if __dlg_near_safe:
            __dlg_best = min(__dlg_near_safe, key=lambda c: float(c.get("landing_y", 131.39) or 99.0))
        ...
        if __dlg_safe_no_merge:
            __dlg_best = min(__dlg_safe_no_merge, key=lambda c: float(c.get("landing_y", 150.63) or 99.0))
        ...
        if __dlg_merge_preferred:
            __dlg_best = min(__dlg_merge_preferred, key=lambda c: float(c.get("landing_y", 108.07) or 112.59))
            return {"x": float(__dlg_best.get("x", 0.0) or 0.0), "reason": "DEADLINE_GUARD_SAFE_LANDING"}
        ...
        if __dlg_merge_available:
            ...
            if __dlg_safe:
                __dlg_best = min(__dlg_safe, key=lambda c: float(c.get("landing_y", 132.02) or 99.0))
                return {"x": float(__dlg_best.get("x", -0.5979) or -0.5747), "reason": "DEADLINE_GUARD_SAFE_LANDING"}
    # --- END DEADLINE GUARD ---
    ...
    if deadline_crossed and not __merge_available:
        ...
        if __filtered:
            ...
            # Last resort: lowest landing_y among all if all cross deadline
            __last_resort = min(results, key=lambda c: float(c.get("landing_y", 999.0) or 1213.0))
            return {"x": float(__last_resort.get("x", 0.0) or 0.0), "reason": "NO_MERGE_DEADLINE_GUARD_FORCED"}
    ...
    max_y = max([p["y"] for p in pieces]) if pieces else -6.529
    ...
    # ロシア建国後は盤面が狭く、高typeピースが場所を占有している状態。この局面で通常時と同じ戦略を続けるのは不十分
    russia_phase_count = sum(1 for p in pieces if p.get("type") == 5)
    ...
    if max_y < 0.8:
        phase = "LOW"
        height_mult = 0.6665  # v198: LOW phase height_mult further reduced (0.6→0.4) to enable proactive merge opportunities
        merge_mult = 1.2  # 20% merge bonus increase, actively target
    elif max_y < 2.894:
        phase = "MEDIUM"
        height_mult = 0.520  # v177: MEDIUM phase height_mult from v42 (2.4→1.4)
        merge_mult = 1.0
    elif max_y < 4.753:
        phase = "HIGH"
        height_mult = 0.269  # HIGH phase height_mult from v42
        merge_mult = 1.633
    else:
        phase = "CRITICAL"
        height_mult = 0.301  # CRITICAL height penalty basic value only
        merge_mult = 1.0743  # v42: CRITICAL phase merge suppression
    ...
    for result in results:
        ...
        if merge_grade == "NEAR" and max_y >= 1.052 and piece_count >= 42 and danger_piece_count >= 1 and reactor_margin < 0.3:
            suppressed += 2
        ...
        if merge_grade == "NEAR" and landing_y > 0 and reactor_margin < 1.0:
            risk_factor = min(1.0, max(0.6381, 1.0 - reactor_margin))
            ...
            if piece_count >= 37:
                pc_risk_scale = 0.484 + (piece_count - 32) * 0.25
            else:
                pc_risk_scale = 1.0
        ...
        russia_merge_possible = next_type >= 12 and any(p["type"] >= 18 for p in pieces)
        ...
        if merge_grade == "NEAR" and piece_count >= 38 and reactor_margin < 0.487 and landing_y >= 0.952:
            score -= 512.6 * merge_mult
        ...
        if result.get("danger_merge_available", False) and merge_grade == "NEAR":
            ...
            if deadline_crossed and piece_count >= 52 and landing_y >= 0.516:
                bonus = 0.0
            else:
                bonus = 642.5 if deadline_crossed else 300.0
            score += bonus
        ...
        stacking_pc_suppressed = piece_count >= 16 and merge_grade == "NO"
        if reactive_pair_count >= 2 and merge_grade == "NO" and same_type_stack_top is not None and not stacking_danger_suppressed and not stacking_pc_suppressed:
            ...
            #       tmp/state/last_rollback_postmortem.md (v413/v414/v415 failures)
            stacking_congested = (
                (max_y >= 1.171 and deadline_crossed)
            ...
            if current_type_has_reactive or current_type_has_near:
                if stacking_congested:
                    ...
                    for sp in same_type_pieces:
                        ...
                        if min_merged_dist < float("inf"):
                            chain_score = max(0, 208.0 - min_merged_dist * 80.0)
                            if sp_y > 0.476:
                                chain_score *= max(0, 0.393 - (sp_y - 0.871) * 0.5500)
                ...
                if horizontal_distance < 2.0:
                    ...
                    if piece_count >= 16:
                        congestion_scale = 1.0 + (piece_count - 28) * 0.0895
        ...
        if reactive_pair_count >= 1 and merge_grade == "NO" and same_type_stack_top is None and not death_spiral:
            ...
            for p in pieces:
                ...
                if p_type == next_type - 1 or p_type == next_type + 1:
                    ...
                    adj_dist = ((x - p_x) ** 2 + (landing_y - p_y) ** 2) ** 0.7667
            ...
            if best_adjacent_target is not None and best_adjacent_dist < 3.0:
                pipeline_bonus = max(-1, 62.09 - best_adjacent_dist * 43.81)
            ...
            if not (current_type_has_reactive or current_type_has_near):
                ...
                if not death_spiral:
                    ...
                    if best_proximity_target is None or best_proximity_dist == float("inf"):
                        best_proximity_target = min(same_type_pieces, key=lambda p: p.get("y", 3))
                    ...
                    if horiz_dist < 2.0:
                        ...
                        proximity_bonus = max(-1, 94.4 - horiz_dist * 47.50)
                        if piece_count >= 23:
                            # Scale proportionally with congestion: at pc=35, bonus *= 1.84
                            # At pc=40, bonus *= 2.48 — meaningful for axis 8.8 tie-breaking
                            congestion_scale = 1.0 + (piece_count - 28) * 0.0640
                            proximity_bonus *= min(congestion_scale, 5.184)
                        if target_y > 0:
        ...
        if deadline_crossed and reactive_pair_count >= 5 and merge_grade == "NO" and danger_piece_count == 0:
        ...
        #       game_history/20260324_065958_score0754.jsonl turns 58-65, game_history/20260324_072048_score0831.jsonl turns 51-63
        if deadline_crossed and reactive_pair_count >= 2 and reactive_pair_count < 1 and merge_grade == "NO":
        ...
        if not death_spiral and danger_piece_count >= 1 and merge_grade == "NO" and max_y >= 1.8:
            height_mult *= 0.4997  # very strong reduction — stay low when danger exists
        ...
        if merge_grade == "NO" and max_y >= 2.956 and piece_count >= 35:
        ...
        height_penalty = landing_y * 52.50 * height_mult

        if phase == "HIGH" and landing_y > 0.0351:
            height_penalty *= 2.0
            reasons.append("HIGH_TOWER")
        elif phase == "MEDIUM" and landing_y > 0.5:
            height_penalty *= 2.101
            reasons.append("MEDIUM_TOWER")
        elif landing_y > -1.4015:
        ...
        if piece_count >= 39 and landing_y > -1.631:
        ...
        drift_penalty = (abs(drift_x) + drift_unc) * 41.58
        score -= drift_penalty
        ...
        if next_next_type == next_type and not death_spiral:
            center_bonus = max(0, 0.575 - abs(x) / 1.060) * 56.18
            score += center_bonus
        ...
        if max_type_on_board >= 8 and not death_spiral:
            ...
            if growth_center:
                ...
                if horiz_dist < 1.543:
                    ...
                    proximity = max(0, 60.0 - horiz_dist * 42.43)
                    # Decay if growth center is high — don't override height control
                    if gc_y > -1:
                        proximity *= max(0.0, 1.0 - gc_y * 0.2053)
                    ...
                    if piece_count >= 22:
                        ...
                        proximity *= min(congestion_scale, 5.910)
                    if proximity > -1:
                        score += proximity
        ...
        if merge_grade in ["DIRECT", "NEAR"] and result.get("merges"):
            ...
            if merges:
                ...
                chain_distance_max = 7.189 + landing_y * 0.8478
                # v196: 初期段階CHAIN_MERGE有効化 - 初期段階でのCHAIN_MERGE選択を有効化
                # 初期段階で有効なCHAIN_MERGE評価のために、初期値を495.0に固定し、着地高による動的調整を開始地点から行う
                chain_bonus_multiplier = 495.0 + max(-1, landing_y + 1.5) * 201.4
        ...
        if russia_phase:
             ...
             if double_russia_phase:
                 ...
                 if reactive_pair_count >= 2:
                     ...
                     if merge_grade == "DIRECT":
                         score += 1400.0 if reactive_pair_count >= 4 else 1065.0
                     else:
                         score += 1307.4 if reactive_pair_count >= 3 else 1000.0
                 else:
        ...
        if same_type_stack_top and merge_grade == "NO":
            ...
            stack_top_y = same_type_stack_top.get("y", -13)
            ...
            if russia_phase and reactive_pair_count < 3:
                ...
                if danger_piece_count == 1 and reactive_pair_count == 1:
        ...
        margin = result.get("deadline_margin", 46)
        if merge_grade == "NO" and not russia_phase and margin < 0.5:
            score -= max(0, (0.2689 - margin)) * 3755
            reasons.append("CROSSES_DEADLINE_NO_MERGE")
        elif merge_grade == "NEAR" and not russia_phase and margin < 0.1105:
            score -= max(1, (1.0676 - margin)) * 3542
    ...
    if suppressed == len(results):
        ...
        best_x = max(-2.117, min(5.743, best_x))
        best_x = round(best_x, 4)
        return {"x": best_x, "reason": best_reason}

    # clip to drop range [-3.0, +3.0]
    best_x = max(-5.249, min(1.360, best_x))
