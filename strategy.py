...
# v577: axis 9.74 rp<8 gate removed + congestion scaling — strengthen T14→T15 merge path
# Problem: rp<8 gate blocked same-type clustering when rp=7-8, and weak base bonus (max=600)
# was buried by height penalties. best_score5801 had axis 9.8 for same-type proximity.
# Fix: Remove rp<8 gate + add piece_count congestion scaling (pc>=28 → up to 2.0x).
# Death spiral guard (rp>=5 && max_y>=4.0) preserved. axis 8.8 penalty >= -4500 preserved.
# Stage-gate target: Kazakhstan(T14) 1/4→2-4/4 by creating same-type cluster for T14→T15 merge.
# refs: tmp/analysis_result.md (adopted hypothesis: strengthen axis 9.74 for T14→T15 merge path)
...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    for result in results:
        ...
        # Fire condition: merge_grade==NO && same_type_pieces>=1 && no_merge_streak>=3
        # rp<8 gate REMOVED: same-type clustering needed even at high rp (7-8) for T14→T15 merge path
        ...
        # Congestion scaling ADDED: piece_count>=28 → scale up to 2.0x, matching axis 9.6b approach.
        # refs: tmp/analysis_result.md (adopted hypothesis: strengthen axis 9.74 for T14→T15 merge path)
        no_merge_streak = 0
        ...
        if (merge_grade == "NO" and len(same_type_pieces) >= 1
            ...
            if min_horiz_dist < 5.0:
                drought_bonus = max(0, 600.0 - min_horiz_dist * 100.0)
                if piece_count >= 28:
                    congestion_scale = 1.0 + (piece_count - 28) * 0.05
                    drought_bonus *= min(congestion_scale, 2.0)
                drought_bonus *= merge_mult
                if drought_bonus > 0:
                    score += drought_bonus
