...
workers=(chat_worker audio_worker radio_worker prediction_worker)

usage() {
	echo "Usage: $0 {all|chat_worker|audio_worker|radio_worker|prediction_worker}" >&2
}
...
all)
	...
	exit "$rc"
	;;
chat_worker|audio_worker|radio_worker|prediction_worker)
	reload_one "$target"
	;;
*)
	usage
