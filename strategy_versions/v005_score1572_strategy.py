...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- 2. 高度危機回避（max_y > 0.9、v012改善: 早期化と旗側変更）---
    # v012改善: 旗側を変更（低い側を旗側にする）
    if max_y > 0.9:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # 明らかに低い側にドロップ（差が0.2以上）
        if left_max_y > right_max_y + 0.2:
            # 右側が低い
            target_x = 2.8  # 壁ドロップ回避
            _flag_side = "right"  # v012改善: 旗側を変更
        ...
        elif right_max_y > left_max_y + 0.2:
            # 左側が低い
            target_x = -2.8  # 壁ドロップ回避
            _flag_side = "left"  # v012改善: 旗側を変更
            _consecutive_no_merge += 1
            _last_drop_x = target_x
            return {
                "x": target_x,
    ...
    # --- 3. 中程度危機回避（max_y > 0.7、v012改善: 早期化と旗側変更緩和）---
    # v012改善: 旗側max_y>0.8かつ反対側が0.2以上低い場合に旗側変更
    if max_y > 0.7:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        ...
        if _flag_side is not None:
            ...
            if _flag_change_cooldown > 0:
                ...
                # v012改善: 旗側max_yを緩くチェック（>0.8で旗側変更を検討）
                # 反対側が0.2以上低い場合に旗側を変更
                if _flag_side == "left" and left_max_y > 0.8:
                    if right_max_y < left_max_y - 0.2:
                        _flag_side = "right"
                        _flag_change_cooldown = 1  # v012改善: 1ターンは変更しない
    ...
    # --- 4. 旗側集約戦略（v012改善: 旗側max_y<0.8で緩和）---
    if _flag_side is not None and results:
        if next_type >= 9:
            ...
            # v012改善: 旗側max_yが0.8以上なら旗側に配置しない（緩和）
            # 旗側max_y<0.8かつ反対側が0.2以上低い場合のみ旗側を変更して配置
            if _flag_side == "left" and left_max_y >= 0.8:
                if right_max_y < 0.8 and right_max_y < left_max_y - 0.2:
                    _flag_side = "right"
                    _flag_change_cooldown = 1
                ...
                if left_max_y < 0.8 and left_max_y < right_max_y - 0.2:
                    _flag_side = "left"
                    _flag_change_cooldown = 1
                # 旗側が高すぎる場合は配置しない（次の戦略へ）
                if _flag_side == "right" and right_max_y >= 0.8:
                    pass
                else:
                    # 旗側（または変更後の旗側）に配置
                    pass

            # 旗側に配置（旗側のmax_yが0.8未満、または変更後）
            if _flag_side == "left":
                # 左側から（-3.0～-0.5）
                best_ev = -float("inf")
                best_x = None
