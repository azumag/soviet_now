...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- v022改善: 旗側決定の簡素化(type9+のmax_y基準)---
    if _flag_side is None:
        ...
        # 2. v022改善: type9+が1個以上あれば,type9+のmax_yが低い側を旗側にする
        if _flag_side is None:
    ...
    # --- v022改善: 旗側変更条件の厳格化(旗側max_y<1.0でない限り変更しない)---
    ...
    if mergeable_results:
        # v022改善: 高度危機時(max_y>1.5),高さ下げ効果のあるマージを優先
        if max_y > 1.5:
            left_max_y = calculate_side_max_y(pieces, "left")
            right_max_y = calculate_side_max_y(pieces, "right")
    ...
    # --- 2. 高度危機回避(v022改善: 危機時旗側変更とマージ優先)---
    if max_y > 1.5:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # v022改善: 高度危機時で旗側max_y>1.0なら,即時旗側変更して低い側を旗側にする
        if _flag_side is not None:
            flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
    ...
    if max_y > 1.0:
        ...
        if _flag_side is not None:
            ...
            # クールダウン期間中は旗側max_y>1.0なら即時反対側にドロップ
            if _flag_change_cooldown > 0:
                if flag_side_max_y > 1.0:
    ...
    # --- 4. 大型ピース旗側配置(v022改善: 旗側max_y>1.2であれば反対側配置)---
    if _flag_side is not None and next_type >= 9:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y

        # v022改善: 旗側max_y<1.2で旗側配置(1.0→1.2緩和)
        if flag_side_max_y < 1.2:
            ...
            # v022改善: 旗側max_y>1.2の場合,反対側に配置(旗側を避ける)
            if _flag_side == "left":
                target_x = 2.8
            else:
                target_x = -2.8
