...
# v417: v415成功実装復活版 - v416の失敗（score=576、v412の1345を大幅に下回る）を受けて、v416の根本的な問題点を特定。v416ではtype N-1が0個でもFUTURE_MERGEボーナスを計算するように変更したが、これは論理的に不自然（type N-1が存在しない場合、将来のマージは不可能なのにボーナスを与えている）。v473のbatch_summary.txtでFUTURE_MERGEが27.1%だがavg_score_delta=11.5と低い一方、NEAR_MERGEは8.5%だがavg_score_delta=28.0と高いことから、「FUTURE_MERGEが過剰に選択されているが、実際のスコア上昇につながっていない」ことを確認。v468(v415)のscore=1776とv465(v415)のscore=1955が成功していることから、v415の実装に戻すことが最も確実な改善。type N-1が0個でもボーナスを計算するv416のロジックを削除し、v415のtype N-1が1個以上の場合のみ適用する条件に戻す。v415の成功要素（FAR_MERGEボーナス300、HIGH_TOWERペナルティ1.0倍、MEDIUMフェーズheight_mult=1.8、先読みマージボーナスtype*10、クラスタ密度評価、連鎖マージ評価）を維持。
#   根本原因の特定:
#   - v416のtype N-1が0個でもボーナスを計算するロジックが誤り
#   - type N-1が0個の状況でFUTURE_MERGEボーナスが付与され、誤った位置選択を引き起こしている
#   - FUTURE_MERGEのavg_score_delta=11.5と低いのがボーナス計算の誤りを示している
#   - v468(v415)とv465(v415)が高いスコアを出していることから、v415の実装が正しい
#   改善策（v415成功実装復活）:
#   - v416のtype N-1が0個以上で適用するロジックを削除し、v415のtype N-1が1個以上で適用する条件に戻す
#   - v415の成功要素を維持: FAR_MERGEボーナス300、HIGH_TOWERペナルティ1.0倍、MEDIUMフェーズheight_mult=1.8、先読みマージボーナスtype*10、クラスタ密度評価、連鎖マージ評価
#   - v415の実装に完全復帰することで、v468の1776点やv465の1955点を上回ることを期待
#   核心的発見: v416のtype N-1が0個でもボーナスを計算するロジックは誤りで、FUTURE_MERGEが過剰に選択されているが実際のスコア上昇につながっていない。v415の実装に戻すことで、誤ったボーナス計算を修正し、v415の成功要素を活かせる。
#   成功基準: scoreがv415の成功値（1776-1955）を上回る、またはFUTURE_MERGEのavg_score_deltaが向上
#   失敗基準: scoreがv416の576以下、またはFUTURE_MERGEの誤った位置選択が改善されない
