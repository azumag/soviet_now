...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- v041改善: マージ戦略の超緊急時強化(マージ判定を危機回避より優先) ---
    mergeable_results = []
    for r in results:
    ...
    if mergeable_results:
        # v041改善: 致命的時マージ戦略(max_y>2.5ならscore>-30のマージを許容)
        if max_y > 2.5:
        ...
        # v041改善: 超致命的時マージ戦略(max_y>2.0ならscore>-20のマージを許容)
        if max_y > 2.0:
        ...
        # v041改善: 超緊急時マージ戦略(max_y>1.5ならscore>-12のマージを許容)
        if max_y > 1.5:
            valid_merges = [r for r in mergeable_results if r.get("score", 0) > -12]
            if valid_merges:
                best = max(valid_merges, key=lambda r: r.get("score", 0))
                x = best["x"]
                score = best.get("score", 0)
        ...
        # v041改善: 緊急時マージ戦略(max_y>1.0ならscore>-8のマージを許容)
        if max_y > 1.0:
    ...
    # --- v041改善: 旗側変更ロジックの複合化(反対側max_y<1.3かつ旗側max_y>反対側+0.5なら旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        ...
        # v041改善: 反対側max_y<1.3かつ旗側max_y>反対側+0.5なら旗側変更
        if opposite_side_max_y < 1.3 and flag_side_max_y > opposite_side_max_y + 0.5:
    ...
    # --- v041改善: 旗側max_y管理の複合化(旗側max_y>2.0かつ反対側max_y<1.3かつ旗側max_y>反対側+0.5なら旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
        ...
        if (
            flag_side_max_y > 2.0
            and opposite_side_max_y < 1.3
            and flag_side_max_y > opposite_side_max_y + 0.5
        ):
            _flag_side = "right" if _flag_side == "left" else "left"
            _flag_change_cooldown = 5
            target_x = 2.8 if _flag_side == "right" else -2.8
    ...
    # --- v040改善: 旗側max_y管理の強化(max_y>0.7かつ旗側max_y>1.5かつ旗側max_y>反対側なら旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
    ...
    # --- v040改善: クールダウン中旗側回避の無効化(マージ可能なら旗側回避をスキップ) ---
    if _flag_side is not None:
        ...
        # v040改善: max_y>0.7ならクールダウン中旗側回避を無効化
        if max_y <= 0.7:
    ...
    # --- v041改善: 高度危機回避の厳格化(max_y>1.6で発動) ---
    if max_y > 1.6:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
    ...
    # --- v040改善: 中程度危機回避の条件緩和(max_y>0.5で発動) ---
    if max_y > 0.5:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
    ...
    # --- v040改善: 大型ピース旗側配置(type7-12を旗側に配置) ---
    if _flag_side is not None and 7 <= next_type <= 12:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # v040改善: 旗側max_y>1.8なら旗側変更してから配置
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
        opposite_side_max_y = right_max_y if _flag_side == "left" else left_max_y

        if flag_side_max_y > 1.8 and opposite_side_max_y < 1.0:
            _flag_side = "right" if _flag_side == "left" else "left"
            _flag_change_cooldown = 5
    ...
    if next_next_type > 0 and next_next_type == next_type:
        ...
        # v039改善: 旗側max_y>1.5なら旗側にドロップしない
        if _flag_side is not None:
            flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
