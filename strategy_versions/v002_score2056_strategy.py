...
def determine_flag_side_from_distribution(pieces: list) -> str:
    """盤面のピース分布から旗側を決定する（v009改善：ピース数>15で使用）。

    大型ピース(type7+)の分布を基準に旗側を決定する。
...
def should_change_flag_side(
    ...
    """旗側を変更すべきか判定する（v009改善：旗側max_yチェック強化）。

    反対側が大幅に低い場合、かつ反対側のmax_yが1.0未満の場合のみ旗側を変更する。
...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # 旗側がまだ決まっていない場合、ピース分布から決定（ピース数>15で使用）
    if _flag_side is None and len(pieces) > 15:
    ...
    # 旗側再評価（v009追加: ピース数>15で旗側再チェック）
    if _flag_side is not None and len(pieces) > 15 and _flag_change_cooldown == 0:
        # 左右の大型ピース数を比較
        left_large = calculate_large_piece_count(pieces, "left", 7)
        right_large = calculate_large_piece_count(pieces, "right", 7)

        # 片側が圧倒的に多い場合、旗側を変更
        if _flag_side == "left" and right_large > left_large + 2:
            proposed = "right"
            if should_change_flag_side(pieces, _flag_side, proposed):
                _flag_side = proposed
                _flag_change_cooldown = 7
        elif _flag_side == "right" and left_large > right_large + 2:
            proposed = "left"
            if should_change_flag_side(pieces, _flag_side, proposed):
                _flag_side = proposed
                _flag_change_cooldown = 7
    ...
    # --- 3. 中程度危機回避（max_y > 1.0）---
    # v009改善: 旗側変更条件を厳格化（反対側が大幅に低い場合のみ）
    if max_y > 1.0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
            ...
            if _flag_change_cooldown > 0:
                ...
                if _flag_side == "left" and left_max_y > 1.0:
                    ...
                    if right_max_y < left_max_y - 0.5 and right_max_y < 1.0:
                        _flag_side = "right"
                        _flag_change_cooldown = 7  # 7ターンは変更しない
                        target_x = 2.8
                    else:
                        target_x = -2.8
    ...
    # --- 4. 旗側集約戦略（v009改善: type9+でも旗側max_y<1.0で配置）---
    if _flag_side is not None and results:
        if next_type >= 9:
            ...
            # type9は旗側固定、type10+は分散を考慮しない（v009改善）
            if _flag_side == "left":
                # 左側から（-3.0～-0.5）
                best_ev = -float("inf")
                best_x = None
