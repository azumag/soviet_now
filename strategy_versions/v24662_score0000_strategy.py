...
# Conditions: deadline_crossed && merge_grade=="NO" && |x|>=2.5
      ...
      # refs: tmp/analysis_result.md (Implementation Plan Change 4 fix)
...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # Prefer runner-provided state, but also support _decision_history because
    # some live paths do not inject no_merge_streak directly.
    ...
    if _history:
        ...
        for _h in reversed(_history[-10:]):
            if _h.get("merge_grade") == "NO":
                ...
                break
        no_merge_streak = max(no_merge_streak, _streak)
    ...
    def cascade_merge_value(result):
        """Score A-1 -> A immediate merges that can chain into an existing A."""
        if result.get("crosses_deadline", False):
            return 0.0, None
        grade = result.get("merge_grade", "NO")
        if grade not in ("DIRECT", "NEAR"):
            return 0.0, None

        best_value = 0.0
        best_label = None
        for merge in result.get("merges", []):
            merge_grade = merge.get("grade", "NO")
            if merge_grade not in ("DIRECT", "NEAR"):
                continue
            target = piece_by_id.get(merge.get("id"))
            if not target:
                continue
            target_x = float(merge.get("x", target.get("x", 0.0)) or 0.0)
            target_y = float(merge.get("y", target.get("y", 0.0)) or 0.0)
            for p in pieces:
                if p.get("id") == target.get("id") or p.get("type") != merged_type:
                    continue
                px = float(p.get("x", 0.0) or 0.0)
                py = float(p.get("y", 0.0) or 0.0)
                pr = float(p.get("r", 0.5) or 0.5)
                contact = max(0.1, pr * 2.0)
                dx = abs(px - target_x)
                dy = target_y - py
                dist = (dx * dx + dy * dy) ** 0.5
                # The valuable shape is the user's example: an A-1 above an A.
                # After A-1+A-1 creates A, it should immediately fall/contact
                # into the lower A. Keep this strict enough to avoid vague
                # "pipeline" proximity from overriding safety.
                if dy < -0.25 or dy > contact * 1.75:
                    continue
                if dx > contact * 0.95 or dist > contact * 1.55:
                    continue
                value = 1.0
                value += max(0.0, 1.0 - dx / max(contact * 0.95, 0.1))
                value += max(0.0, 1.0 - abs(dy - contact * 0.65) / max(contact, 0.1))
    ...
    def risk_top_value(result):
        return float(
            result.get(
                "risk_top_y_after_drop",
                result.get("top_y_after_drop", result.get("landing_y", 999.0)),
            )
            or 999.0
        )
    ...
    min_deadline_risk_top = min(
        (risk_top_value(r) for r in results),
        default=999.0,
    )
    ...
    merge_possible = any(r.get("merge_grade") in ("DIRECT", "NEAR") for r in active_results)
    ...
    cascade_values = [cascade_merge_value(r) for r in results]
    safe_direct_cascade_possible = any(
        cascade_values[idx][0] > 0.0 and cascade_values[idx][1] == "DIRECT"
        for idx, _ in active_indexed_results
    )
    deadline_safe_drop_exists = bool(safe_indexed_results)
    deadline_buffer_drop_exists = bool(buffered_indexed_results)
    min_active_deadline_risk_top = min(
        (
            r.get(
                "risk_top_y_after_drop",
    ...
    for idx, result in active_indexed_results:
        x = result["x"]
        landing_y = result.get("landing_y", 0)
        drift_x = result.get("drift_x", 0)
        drift_unc = result.get("drift_unc", 0)
        merge_grade = result.get("merge_grade", "NO")  # DIRECT/NEAR/FAR/NO
        cascade_value, cascade_label = cascade_values[idx]
        ...
        top_y_after_drop = risk_top_value(result)
        ...
        # when the board is dense or near the deadline, reducing piece count is
        # the safer move if the DIRECT candidate itself does not cross.
        if immediate_direct_required and merge_grade != "DIRECT":
            score -= 900000.0
            reasons.append("SAFE_DIRECT_MERGE_FORCED")

        if safe_direct_cascade_possible and not (
            merge_grade == "DIRECT" and cascade_value > 0.0
        ):
            score -= 950000.0
            reasons.append("SAFE_DIRECT_CASCADE_FORCED")
        ...
        if (
            ...
            and top_y_after_drop > deadline_buffer_y
            and not (immediate_direct_required and merge_grade == "DIRECT")
            and not (
                safe_direct_cascade_possible
                and merge_grade == "DIRECT"
                and cascade_value > 0.0
            )
        ):
            score -= 500000.0
            reasons.append("DEADLINE_HEADROOM_HARD_BLOCK")

        # If every candidate is already predicted to cross, DIRECT/NEAR merges
        # must not create an even taller vertical result. Choose the least bad
        # top edge instead.
        if (
            min_deadline_risk_top >= deadline_y
            and top_y_after_drop > min_deadline_risk_top + 0.15
        ):
            score -= 750000.0
        ...
        if cascade_value > 0.0:
            if cascade_label == "DIRECT":
                score += 2200.0 * merge_mult * cascade_value
                reasons.append("DIRECT_CASCADE_MERGE")
            else:
        ...
        effective_no_merge_streak = no_merge_streak
        if merge_grade == "NO" and len(same_type_pieces) >= 2 and reactive_pair_count >= 3:
            effective_no_merge_streak = max(effective_no_merge_streak, 3)
        if (
            effective_no_merge_streak >= 2
            and reactive_pair_count >= 3
            and max_y >= 1.5
            and merge_grade == "NO"
        ):
            drought_blocking_penalty = 0.0
            for rp in reactive_pairs:
                if isinstance(rp, (list, tuple)) and len(rp) >= 3:
                    pos1 = piece_pos_by_id.get(rp[0])
                    pos2 = piece_pos_by_id.get(rp[1])
                    if pos1 and pos2:
                        x1, y1 = pos1
                        x2, y2 = pos2
                        span_min = min(x1, x2) - 0.45
                        span_max = max(x1, x2) + 0.45
                        if span_min <= x <= span_max and landing_y >= min(y1, y2) - 0.1:
                            drought_blocking_penalty += 350.0
            if drought_blocking_penalty > 0.0:
                score -= min(drought_blocking_penalty, 1200.0)
                reasons.append("MERGE_DROUGHT_REACTIVE_CORRIDOR_BLOCK")
        ...
        if reactive_pair_count >= 3 and merge_grade == "NO":
            ...
            risk_excess = max(0.0, top_y_after_drop - min_deadline_risk_top)
            if risk_excess > 0.0:
                score -= risk_excess * 1500.0
                reasons.append("REACTIVE_PAIRS_NO_MERGE_RISK_TOP_PENALTY")
        ...
        # ロシア建国時でも mandatory_themes の deadline 原則を守るため、
        # russia_phase 例外は置かない。
        ...
        # vYYY: axis 9.17c — edge NO-MERGE penalty at deadline_crossed (merge_possible条件修正)
        # mandatory_themes第一条: 「デッドライン超出時はmergeできる場合に限る」
        # merge_possibleがfalseの時は配置禁止ではなくペナルティ抑制（候補除外ではなくペナルティで抑制）
        # Change 1 (-10000) がmerge_possible=true時のみ発動するため、axis 9.17cもmerge_possible条件を統一
        # refs: tmp/analysis_result.md (Implementation Plan Change 4)
        if deadline_crossed and merge_grade == "NO" and merge_possible and abs(x) >= 2.5:
            score -= 2500.0 * merge_mult
            reasons.append("EDGE_NO_MERGE_DEADLINE_9_17C")

        # vYYY: Change 3 - Edge Placement Suppression During Deadline Cross
        # mandatory_themes第一条の制度的補償: merge_possible=true時にdeadline超出でNO mergeすると大幅ペナルティ
        # merge_possible=falseの時はChange 1 (-10000)も発動しないため、axis 9.17cもmerge_possible条件を統一
        # Worst game T64-69: edge placements x=-3.0, x=1.6, x=-2.65 during deadline_crossed
        # Center-biased placement during deadline crossing reduces max_y growth.
        # Refs: tmp/analysis_result.md (Implementation Plan Change 3)
        if deadline_crossed and merge_grade == "NO" and merge_possible and abs(x) > 2.0:
            score -= 800.0
