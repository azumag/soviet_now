...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- v037改善: 旗側変更ロジックの強化(旗側max_y>反対側max_y+0.4かつ反対側max_y<1.4なら即時旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        ...
        # v037改善: 旗側max_y>反対側max_y+0.4かつ反対側max_y<1.4なら即時旗側変更
        if flag_side_max_y > opposite_side_max_y + 0.4 and opposite_side_max_y < 1.4:
    ...
    # --- v037改善: 旗側max_y管理の厳格化(旗側max_y>1.2なら即時旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
    ...
    # --- v037改善: 旗側max_y管理の強化(旗側max_y>1.0かつmax_y>0.7なら旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
    ...
    # --- v037改善: クールダウン中旗側回避のマージ優先化(マージ可能ならスキップ) ---
    if _flag_side is not None and not mergeable_results:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
    ...
    if _flag_side is not None and 7 <= next_type <= 12:
        ...
        # v037改善: 旗側max_y>1.2なら旗側変更してから配置
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
        opposite_side_max_y = right_max_y if _flag_side == "left" else left_max_y

        if flag_side_max_y > 1.2 and opposite_side_max_y < 0.9:
            _flag_side = "right" if _flag_side == "left" else "left"
            _flag_change_cooldown = 1
    ...
    # --- v037改善: シェイク戦略の発動条件緩和(無マージ3ターンで発動) ---
    _consecutive_no_merge += 1
