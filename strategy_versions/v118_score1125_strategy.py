...
def decide(game_state: dict, analysis: dict) -> dict:
    """v42のシンプル構造を維持しつつ、v84のHIGHフェーズ高度管理緩和を採用。"""
    ...
    if max_y < 0.8:
        ...
        phase = "HIGH"
        height_mult = 2.4  # v118: v42の2.6からv84の2.2の中間（v84のHIGHフェーズ高度管理緩和を採用）
        merge_mult = 1.0
    else:
        phase = "CRITICAL"
        height_mult = 1.0
    ...
    for result in results:
        ...
        if phase == "HIGH":
            height_penalty = (
                landing_y * 45.0 * height_mult
            )  # v118: v42の50.0からv84の高度管理緩和を採用
        else:
            height_penalty = (
                landing_y * 50.0 * height_mult
            )  # LOW/MEDIUM/CRITICALはv42のまま

        # HIGH_TOWERペナルティ（v42の2.0倍からv84の1.3倍の中間）
        if phase == "HIGH" and landing_y > 0.5:
            height_penalty *= 1.5  # v118: v42の2.0倍から緩和
