...
# refs: tmp/batch_summary.txt (merge率差分析), tmp/state/last_rollback_analysis.md
#
...
# 変更1: axis 8.7 reactive_pairs>=3 → >=2, max_y>=1.8, -3000.0 (拡張・強化)
# 変更2: axis 8.6 reactive_pairs>=2 → >=1, max_y>=2.0 → 1.8, -2000.0 → -3000.0 (拡張・強化)
# 変更3: axis 8.5 reactive_pairs>=2 → >=1, max_y>=2.0 → 1.8, +2000.0 → +3000.0 (拡張・強化)
# これにより危険域判定を max_y>=1.8 に統一し、reactive_pairs>=1 以上で即時併合のない選択を厳しく罰する。
# refs: tmp/state/last_rollback_postmortem.md, tmp/batch_summary.txt, game_history/20260316_112744_score0490.jsonl turns 60-68, game_history/20260316_112539_score0695.jsonl turns 60-68, game_history/20260316_041723_score2754.jsonl, game_history/20260316_042248_score2731.jsonl
#
...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    batch_summary: 高スコア群merge_rate=39.7% vs 低スコア群merge_rate=33.2%（6.5ポイント差）。併合率向上が必要。
    ワーストゲーム(score0917)終盤turns 52-53: max_y=2.83, merge_hits=1で即時併合機会不足により下振れ。
    ベストゲーム(score2348)終盤turns 96-113: max_y=3.37, merge_hits=4で即時併合を確実に捉えスコア稼ぎ。
    v250のmax_y>=1.8条件では、max_y=2.0未満の危険域で即時併合機会を見逃している問題を解消できていない。
    危険域判定をmax_y>=1.8からmax_y>=1.5へ緩和し、より早い段階から即時併合を優先する評価軸を追加・拡張。
    これによりmax_y>=1.5から即時併合優先を強制し、より広い範囲で併合機会を優先することでp25悪化を抑制。
    構造的変更（危険域判定閾値緩和・評価軸拡張）であり、数値微調整ではない。
    refs: tmp/batch_summary.txt (merge率差分析), tmp/state/last_rollback_analysis.md
    ...
    for result in results:
        ...
        # ----- evaluation axis 8.5: danger zone direct merge priority (NEW: reactive_pairs >=1 max_y>=1.5 dangerous situation prioritization) -----
        ...
        # v201 rollback教訓: 複雑な危険局面判定ロジックは禁止。reactive_pairsを活用したシンプルな改善を採用。
        # 単純に「max_y>=1.5かつreactive_pairs>=1かつDIRECT/NEARマージ」で強力なボーナスを与え、
        # 危険なHIGH_TOWER判断を上書きする評価軸を追加。
        # これによりreactive_pairs>=1がある危険域での即時併合機会を優先し、ワーストゲームのような「reactive_pairsがあるのにHIGH_TOWER」判断を回避。
        if max_y >= 1.5 and reactive_pair_count >= 1 and merge_grade in ["DIRECT", "NEAR"]:
            # max_y>=1.5の危険域でreactive_pairs>=1ある場合、DIRECT_MERGEを絶対優先
            # 危険なHIGH_TOWER判断を上書きする強力なボーナス
            score += 3000.0
            reasons.append("EXPANDED_DANGER_ZONE_DIRECT_MERGE_PRIORITY_FORCE")

        # ----- evaluation axis 8.6: danger zone no merge penalty (NEW: reactive_pairs>=1 max_y>=1.5 when no immediate merge) -----
        ...
        # last_rollback_postmortem: reactive_pairs>=3 + merge_available=false → HEIGHT_CONTROL over-selection
        # max_y>=1.5の危険域かつreactive_pairs>=1の場合、即時併合がない配置は極めて危険であり強力なペナルティを与える。
        ...
        # ----- evaluation axis 8.7: expanded danger zone absolute merge priority (NEW: reactive_pairs>=3 max_y>=1.5 super danger) -----
        # ワーストゲーム(score0490, score0695)終盤turns 60-68: reactive_pairs=5-7, merge_available=falseでHEIGHT_CONTROLが続きmax_y=3.3-3.4に悪化しゲームオーバー。
        # ベストゲーム(score2754, score2731)終盤max_y>=2.3-2.4, reactive_pairs>=2-3あってもDIRECT_MERGE選択で延命成功。
        # batch_summary: HEIGHT_CONTROL=18.2%(低スコア) vs 11.8%(高スコア)。危険域での即時併合機会取りこぼしを構造的に潰す。
        # max_y>=1.5の危険域かつreactive_pairs>=3の超危険域の場合、即時併合がない配置を極めて危険として扱う。
        # reactive_pairs>=3がある超危険域で即時併合を絶対優先し、p25悪化の主要因である「reactive_pairsがあるのに非併合」問題を構造的に潰す。
        if max_y >= 1.5 and reactive_pair_count >= 3 and merge_grade in ["DIRECT", "NEAR"]:
            # 超危険域でreactive_pairs>=3ある場合、即時併合を絶対優先
            # 危険なHEIGHT_CONTROL判断を上書きする強力なボーナス
            score += 3000.0
            reasons.append("EXPANDED_DANGER_ZONE_DIRECT_MERGE_PRIORITY_FORCE")
