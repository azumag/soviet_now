...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- 2. 高度危機回避（max_y > 1.5、v019改善: 旗側max_y>1.0で旗側即時変更）---
    if max_y > 1.5:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # v019改善: 旗側max_y>1.0なら旗側即時変更して低い側にドロップ
        if _flag_side is not None:
            flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
            ...
            if flag_side_max_y > 1.0:
                ...
                if left_max_y > right_max_y + 0.4:
                    ...
                    return {
                        "x": target_x,
                        "reason": f"高度危機回避(旗側即時変更 L_max={left_max_y:.1f} R_max={right_max_y:.1f}) x={target_x:.2f}",
                    }
    ...
    # --- 3. 中程度危機回避（max_y > 1.0、v019改善: 旗側max_y>1.0なら旗側回避）---
    if max_y > 1.0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        ...
        if _flag_side is not None:
            ...
            # v019改善: クールダウン期間中は旗側max_y>1.0なら旗側を回避
            if _flag_change_cooldown > 0:
                if flag_side_max_y > 1.0:
                    # 旗側が高い場合、反対側にドロップ
    ...
    # --- 4. 旗側集約戦略（v019改善: 旗側max_y<1.0で厳格化）---
    if _flag_side is not None and next_type >= 9:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # v019改善: 旗側max_y<1.0なら旗側に配置（v018: 0.8→1.0）
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
    ...
    # --- 9. フォールバック: 旗側側の中央（v019改善: 旗側max_y<1.0で厳格化）---
    _consecutive_no_merge += 1

    if _flag_side is not None:
        # v019改善: 旗側のmax_yをチェック、1.0未満なら旗側にドロップ
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
