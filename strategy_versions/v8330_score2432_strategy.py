...
# last_rollback_analysisのNext Improve Focus「anchor比で急激に悪化した局面を重点的に調べること。特にp25を落とした試合群の共通条件を抽出する。」に対応。
# refs: tmp/improve_brief.md, tmp/batch_summary.txt, tmp/state/last_rollback_analysis.md, advice.md,
#       game_history/20260321_071915_score0571.jsonl turns 69-76, game_history/20260321_070558_score2739.jsonl turns 103-110
...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    for result in results:
        ...
        # ----- evaluation axis 8.5: danger zone immediate merge priority (v274: 危険ピース数増強即時併合優先版)
         ...
         # v298: axis 8.5 removed - danger zone immediate merge priority
        ...
        if reactive_pair_count == 1 and merge_grade in ["DIRECT", "NEAR"]:
            # v297: reactive_pairs==1: 即時併合ボーナスを大幅に強化 (800.0 → 1500.0)
            score += 1500.0
            reasons.append("REACTIVE_IMMEDIATE_MERGE_PRIORITY")
        elif reactive_pair_count >= 2 and reactive_pair_count < 3 and merge_grade in ["DIRECT", "NEAR"]:
            # v297: reactive_pairs==2: 即時併合ボーナスを大幅に強化 (1300.0 → 2000.0)
            score += 2000.0
            reasons.append("REACTIVE_IMMEDIATE_MERGE_PRIORITY")
        elif reactive_pair_count >= 3 and merge_grade in ["DIRECT", "NEAR"]:
            # v297: reactive_pairs>=3: 即時併合ボーナスを大幅に強化 (1600.0 → 2500.0)
            # 即時併合機会を絶対優先
            score += 2500.0
            reasons.append("REACTIVE_IMMEDIATE_MERGE_PRIORITY")
        elif reactive_pair_count >= 1 and merge_grade == "NO" and danger_piece_count == 0:
            # v297: 即時併合不可で、危険ピースがない場合：戦略的配置ボーナスを最小限に抑制 (400.0 → 50.0)
            # 即時併合優先を絶対的に明確にする
            score += 50.0
            reasons.append("REACTIVE_STRATEGIC_PLACEMENT")
            # 戦略的配置の余地を最小限にするためheight_multを強抑制
            height_mult *= 0.3
        elif reactive_pair_count >= 1 and merge_grade == "NO" and danger_piece_count > 0:
            # v297: 即時併合不可で、危険ピースがある場合：戦略的配置ボーナスを最小限に抑制 (50.0 → 0.0)
            # 即時併合優先を絶対最優先にするためボーナスを完全に削除
            score += 0.0
            reasons.append("REACTIVE_STRATEGIC_PLACEMENT_DANGER")
            # 危険ピースがある場合、戦略的配置の余地を完全に抑制
            height_mult *= 0.4
        ...
        # ----- evaluation axis 9.5 (removed v298) current type stack merge priority -----
        # v298: 戦略的配置ボーナスを削除し、即時併合優先へシンプル化
