...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- v030改善: マージ戦略の最優先化(マージ判定を高度危機回避より優先) ---
    mergeable_results = []
    for r in results:
    ...
    if mergeable_results:
        # v030改善: 緊急時マージ戦略(max_y>2.0かつマージ可能なら全マージを許容)
        if max_y > 2.0:
            # v030新規: 全マージを許容
        ...
        # v030改善: 緊急時マージ戦略(max_y>1.5かつマージ可能ならscore>-20のマージを許容)
        if max_y > 1.5:
            # v030改善: スコア>-20のマージを許容(v029: -5→-20,大幅緩和)
        ...
        # v031改善: 緊急時マージ戦略(max_y>1.0かつマージ可能ならscore>-5のマージを許容)
        if max_y > 1.0:
            # v031改善: スコア>-5のマージを許容(v029復活)
    ...
    # --- v031改善: 旗側max_y管理の緩和(旗側max_y>1.5なら即時旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        ...
        # v031改善: 旗側max_y>1.5なら即時旗側変更(v030: 2.0→1.5)
        if flag_side_max_y > 1.5:
            _flag_side = "right" if _flag_side == "left" else "left"
            # v030改善: クールダウンを1ターンに短縮(v029: 2→1,超柔軟化)
            _flag_change_cooldown = 1
    ...
    # --- v030改善: 旗側max_y管理の強化(旗側max_y>1.5かつmax_y>1.0なら旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
        ...
        # v030改善: 旗側max_y>1.5かつmax_y>1.0なら旗側変更
        if (
            max_y > 1.0
    ...
    # --- v031改善: 旗側決定ロジックの簡素化(max_y>1.0で旗側max_y>反対側なら旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
        ...
        # v030改善: max_y>1.0で旗側max_y>反対側なら旗側変更
        if max_y > 1.0 and flag_side_max_y > opposite_side_max_y:
            _flag_side = "right" if _flag_side == "left" else "left"
            _flag_change_cooldown = 1

    # --- v031改善: 高度危機回避の早期化(max_y>1.5で発動) ---
    if max_y > 1.5:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # v031改善: 常により低い側にドロップ(v027維持)
        lower_side = "left" if left_max_y < right_max_y else "right"
        target_x = 2.8 if lower_side == "right" else -2.8

        # v030改善: 前回と同じ側なら反対側にドロップ
        if (target_x > 0 and _last_drop_x > 0) or (target_x < 0 and _last_drop_x < 0):
            target_x = -target_x
    ...
    # --- v029改善: 旗側max_y管理の超厳格化(旗側max_y>1.0なら即時旗側回避) ---
    if _flag_side is not None:
        ...
        # v028改善: 旗側max_y>1.0なら即時反対側にドロップ
    ...
    # --- v030改善: シェイク戦略の発動条件緩和(無マージ4ターンで発動) ---
    _consecutive_no_merge += 1
