...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    if max_y < 0.8:
        ...
        phase = "MEDIUM"
        height_mult = 2.4  # v150: v42の2.4を維持（MEDIUMフェーズの安定性確保）
        merge_mult = 1.0
    elif max_y < 3.0:
        phase = "HIGH"
        height_mult = (
            2.0  # v150: v149の2.2から2.0に緩和、v128の成功設定（1.8）に近づける
        )
        merge_mult = 1.0
    else:
        phase = "CRITICAL"
    ...
    for result in results:
        ...
        # === v150: reactive_pairs動的高度管理（閾値調整） ===

        # reactive_pairsに応じたheight_multiplier調整
        # v150: 閾値を調整してマージ優先をより積極的に適用
        # reactive_pairs >= 5: マージ優先（0.5倍）
        # reactive_pairs >= 2: バランス（1.0倍）
        # reactive_pairs < 2: 高度管理優先（1.2倍）
        if phase == "HIGH":
            if reactive_pairs_count >= 5:
                height_multiplier = height_mult * 0.5  # マージ優先（0.5倍に強化）
                reasons.append(f"RP{reactive_pairs_count}_MERGE")
            elif reactive_pairs_count >= 2:
        ...
        # v150: HIGHフェーズではHIGH_TOWERペナルティを削除し、reactive_pairs動的調整で代替
        # v150: MEDIUMフェーズではMEDIUM_TOWERペナルティを維持（v42の1.5倍）
        ...
        # 3. 左右バランス補正（v150: v148の設定を維持）
        # v150: v148の設定（HIGH=10.0, MEDIUM=15.0）を維持
        balance_strength = 20.0
        if phase == "HIGH":
            balance_strength = 10.0  # v150: v148の10.0を維持
        elif phase == "MEDIUM":
            balance_strength = 15.0  # v150: v148の15.0を維持
