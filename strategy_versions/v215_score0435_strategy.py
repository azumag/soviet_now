...
def decide(game_state: dict, analysis: dict) -> dict:
    """v208成功要素再導入・v128 HIGH構造融合版

    v213の失敗（スコア1350、MEDIUMフェーズheight_mult=1.8→スコア低下）を受けて、
    v208の成功要素（MEDIUMフェーズheight_mult=2.4、スコア1710）とv128のHIGHフェーズ構造（height_mult=1.8）を融合するブレイクスルーを実施。

    v213履歴分析で特定した問題:
    - MEDIUMフェーズheight_mult=1.8は盤面が高くなりすぎず、HIGHフェーズ期間が短い（max_y=2.8で終了）
    - HIGHフェーズでのマージ機会が不足（HIGH_TOWER発動率高）
    - v213の「v128完全回帰」はv128のMEDIUMフェーズheight_mult=1.8を再採用したが、v208の2.4の方が高スコア
    - v208の1710点 vs v213の1350点で、MEDIUMフェーズheight_multの違いが明確

    根本原因:
    - v213はv128の「完全回帰」を試みたが、v208のMEDIUMフェーズheight_mult=2.4が実際には成功していた
    - v128のHIGHフェーズ構造（height_mult=1.8）はv128成功時（スコア3689）の要素だが、MEDIUMフェーズのheight_multはv208の2.4が良い
    - 振り子パターン（2.4→1.8→2.4）を回避し、v208の成功要素とv128のHIGHフェーズ構造を融合する

    解決策（振り子パターン解消のブレイクスルー）:
    - v208の成功要素を再導入：MEDIUMフェーズheight_mult=2.4（v213の1.8から復帰）
    - v128のHIGHフェーズ構造を維持：height_mult=1.8、HIGH_TOWERペナルティ1.3倍
    - v128の成功構造を維持：固定マージボーナス（DIRECT=1200/NEAR=600/FAR=200）
    - v128のバランス補正強度（HIGH=40.0/MEDIUM=30.0/LOW=20.0）を維持
    - ドリフトペナルティ一律30.0を維持
    - nextNextが同じタイプなら中央寄せボーナス50.0を維持
    - MEDIUM_TOWERペナルティを完全削除（v213の設定を維持）
    ...
    # フェーズ判定（v214: v208/v128の閾値0.8/1.8/3.0を維持）
    if max_y < 0.8:
        phase = "LOW"
        height_mult = 1.0
        merge_mult = 1.2
        ...
        phase = "MEDIUM"
        height_mult = 2.4  # v214: v208の成功要素を再導入（v213の1.8から復帰）
        merge_mult = 1.0
    elif max_y < 3.0:
        phase = "HIGH"
        height_mult = 1.8  # v214: v128のHIGHフェーズ構造を維持
        merge_mult = 1.0
    else:
        phase = "CRITICAL"
    ...
    for result in results:
        ...
        # === v214: v208成功要素再導入・v128 HIGH構造融合 ===

        # 1. マージグレードによるスコア（v214: v128の固定値を維持）
        if merge_grade == "DIRECT":
            score += 1200.0 * merge_mult  # v214: v128の1200を維持
            reasons.append("DIRECT_MERGE")
        elif merge_grade == "NEAR":
            score += 600.0 * merge_mult  # v214: v128の600を維持
            reasons.append("NEAR_MERGE")
        elif merge_grade == "FAR":
            score += 200.0 * merge_mult  # v214: v128の200を維持
            reasons.append("FAR_MERGE")

        # 2. 高度によるペナルティ（v214: v128一律ルール）
        height_penalty = landing_y * 40.0 * height_mult

        # HIGH_TOWERペナルティ（v214: v128の設定を維持）
        if phase == "HIGH" and landing_y > 0.5:  # v214: v128の閾値0.5を維持
            height_penalty *= 1.3  # v214: v128の1.3倍を維持
        ...
        # 4. 左右バランス補正（v214: v128の値を維持）
        balance_strength = 20.0
        if phase == "HIGH":
            balance_strength = 40.0  # v214: v128の40.0を維持
        elif phase == "MEDIUM":
            balance_strength = 30.0  # v214: v128の30.0を維持
        ...
        # 5. nextNextが同じタイプなら中央寄せボーナス（v214: v128の一律50.0を維持）
