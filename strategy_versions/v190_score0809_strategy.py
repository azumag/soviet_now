...
# v190: v128構造維持・MEDIUMフェーズ高度管理強化版 - v189の失敗（スコア778、マージボーナス強化の振子パターン・MEDIUMフェーズ高度管理不足）を受けて、振り子パターンを第三の選択肢で解消するブレイクスルーを実施。（1）マージボーナス振り子の解消：v84/v189のマージボーナス強化（1500/800/300）がマージ判断を支配的にし、高度管理を無効化。マージボーナスをv128の値（1200/600/200）に戻し、補助的な位置付けに戻す。「追加も削除もせず」ではなく、v128の成功構造に戻すことで、振り子を解消。（2）MEDIUMフェーズ高度管理強化：v187の失敗（スコア540）はv128と同じ構造に復帰したにもかかわらず、MEDIUMフェーズheight_mult=2.4が不十分で盤面上昇速度が速かった（1ターンあたり0.16上昇）。MEDIUMフェーズheight_multを2.6に強化し、HIGHフェーズ到達を遅らせる。盤面上昇を抑制し、HIGHフェーズでheight_mult=1.8を機能させる。（3）v128の成功構造を維持：HIGHフェーズheight_mult=1.8、マージボーナス（1200/600/200）、TOWERペナルティ閾値0.5、HIGH_TOWER 1.3倍/MEDIUM_TOWER 1.5倍、バランス補正（HIGH=40.0/MEDIUM=30.0/LOW=20.0）、ドリフトペナルティ一律30.0を維持。（4）v42のMEDIUMフェーズ高度管理を採用：v42のMEDIUMフェーズheight_mult=2.4よりも強化し、2.6を採用。v42のHIGHフェーズheight_mult=2.6は盤面上昇を抑制するが、マージ機会を損なうため、v128のHIGHフェーズheight_mult=1.8を維持。（5）振り子パターン解消のバランス：v128のHIGHフェーズ構造（height_mult=1.8）とv42のMEDIUMフェーズ高度管理（height_mult=2.6）を統合。マージボーナスは補助的な位置付けに戻し、マージ予測を前提としないシンプルな構造を維持。コード量維持（約110行）。


def decide(game_state: dict, analysis: dict) -> dict:
    """v128構造維持・MEDIUMフェーズ高度管理強化版

    v189の失敗（スコア778、マージボーナス強化の振子パターン・MEDIUMフェーズ高度管理不足）を受けて、
    振り子パターンを第三の選択肢で解消するブレイクスルーを実施。

    v189履歴分析で確認した問題:
    - マージボーナス強化の振子パターン（v84/v189）
    - MEDIUMフェーズ高度管理不足（3ターンのみ）
    - MEDIUMフェーズでHIGH_LAYERが5回発動
    - HIGHフェーズマージ率20%と低い
    - HIGHフェーズでHIGH_TOWERが3回発動

    解決策:
    - マージボーナスをv128の値（1200/600/200）に戻し、補助的な位置付けに戻す
    - MEDIUMフェーズheight_multを2.6に強化し、HIGHフェーズ到達を遅らせる
    - v128のHIGHフェーズ構造（height_mult=1.8）を維持
    - マージ予測を前提としないシンプルな構造を維持
    """
    ...
    if max_y < 0.8:
        ...
        phase = "MEDIUM"
        height_mult = 2.6  # v190: v42の2.4を強化、HIGHフェーズ到達遅延
        merge_mult = 1.0
    elif max_y < 3.0:
        phase = "HIGH"
        height_mult = 1.8  # v190: v128の1.8を維持
        merge_mult = 1.0
    else:
        phase = "CRITICAL"
        height_mult = 1.0  # CRITICAL: height_multなし
        merge_mult = 0.6  # v190: v42の0.6を維持
    ...
    for result in results:
        ...
        # === v190: v128構造維持・MEDIUMフェーズ高度管理強化 ===

        # 1. マージグレードによるスコア（v190: v128の値を維持、補助的な位置付け）
        if merge_grade == "DIRECT":
            score += 1200.0 * merge_mult  # v190: v128の1200を維持
            reasons.append("DIRECT_MERGE")
        elif merge_grade == "NEAR":
            score += 600.0 * merge_mult  # v190: v128の600を維持
            reasons.append("NEAR_MERGE")
        elif merge_grade == "FAR":
            score += 200.0 * merge_mult  # v190: v128の200を維持
            reasons.append("FAR_MERGE")

        # 2. 高度によるペナルティ（v190: HIGHフェーズheight_mult=1.8を維持）
        height_penalty = landing_y * 50.0 * height_mult

        # HIGH_TOWERペナルティ（v190: v128の閾値0.5を維持）
        if phase == "HIGH" and landing_y > 0.5:
            height_penalty *= 1.3  # v190: v128の1.3倍を維持
            reasons.append("HIGH_TOWER")
        elif phase == "MEDIUM" and landing_y > 0.5:
            height_penalty *= 1.5  # v190: v128の1.5倍を維持
        ...
        # 3. ドリフトによるペナルティ（v190: v128の一律30.0を維持）
        drift_penalty = (abs(drift_x) + drift_unc) * 30.0
        score -= drift_penalty

        # 4. 左右バランス補正（v190: v128の左右カウントベースを維持）
        balance_strength = 20.0
        if phase == "HIGH":
            balance_strength = 40.0  # v190: v128の40.0を維持
        elif phase == "MEDIUM":
            balance_strength = 30.0  # v190: v128の30.0を維持
        ...
        # 5. nextNextが同じタイプなら中央寄せボーナス（v190: v128の一律50.0を維持）
