...
# v025: 危機回避ロジックの根本的改善と早期発動化(2026-02-25 最新)
...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- v025改善: クールダウン期間中の旗側回避緩和(0.8→0.6) ---
    if _flag_side is not None:
        ...
        # クールダウン期間中でも旗側max_y>0.6なら即時反対側にドロップ
        if flag_side_max_y > 0.6:
    ...
    # --- v025改善: 高度危機回避の根本的改善(常に低い側にドロップ) ---
    if max_y > 1.0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        ...
        # v025改善: 高い側が旗側の場合,旗側を変更して低い側にドロップ
    ...
    # --- v025改善: 旗側変更条件の緩和(反対側が0.3以上低い場合に旗側変更)---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y

        # 旗側max_y>0.6の場合,反対側が0.3以上低ければ旗側を変更
        if flag_side_max_y > 0.6:
    ...
    # --- 1. マージ可能なら最優先(v025改善: 旗側max_y>0.6なら旗側のマージを回避) ---
    mergeable_results = []
    for r in results:
    ...
    if mergeable_results:
        ...
        # v025改善: 高度危機時(max_y>1.5)は低スコアマージも許容
        if max_y > 1.5:
            # 全てのマージ候補から最高スコアを選択
    ...
    # --- v025改善: 大型ピース旗側配置(旗側max_y<0.7で旗側配置) ---
    if _flag_side is not None and next_type >= 9:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y

        # v025改善: 旗側max_y<0.7で旗側配置(v024: 0.5→0.7,緩和)
        if flag_side_max_y < 0.7:
            if _flag_side == "left":
                target_x = -2.8
            else:
                target_x = 2.8
            ...
            # v025改善: 旗側max_y>0.7の場合,反対側に配置(旗側を避ける)
            if _flag_side == "left":
                target_x = 2.8
            else:
                target_x = -2.8
