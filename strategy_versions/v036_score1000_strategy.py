...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- v040改善: マージ戦略の最優先化(マージ判定を危機回避より優先) ---
    mergeable_results = []
    for r in results:
    ...
    if mergeable_results:
        # v040改善: 致命的時マージ戦略(max_y>2.0ならscore>-25のマージを許容)
        if max_y > 2.0:
            valid_merges = [r for r in mergeable_results if r.get("score", 0) > -25]
            if valid_merges:
        ...
        # v040改善: 超緊急時マージ戦略(max_y>1.5ならscore>-15のマージを許容)
        if max_y > 1.5:
        ...
        # v040改善: 緊急時マージ戦略(max_y>1.0ならscore>-10のマージを許容)
        if max_y > 1.0:
    ...
    # --- v040改善: 旗側変更ロジックの複合化(反対側max_y<1.5かつ旗側max_y>反対側+0.5なら旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        ...
        # v040改善: 反対側max_y<1.5かつ旗側max_y>反対側+0.5なら旗側変更
    ...
    # --- v040改善: 旗側max_y管理の複合化(旗側max_y>2.0かつ反対側max_y<1.5かつ旗側max_y>反対側+0.5なら旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
    ...
    # --- v040改善: 高度危機回避の条件厳格化(max_y>1.5で発動) ---
    if max_y > 1.5:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
    ...
    # --- v037改善: 中程度危機回避の条件緩和(max_y>0.5で発動) ---
    if max_y > 0.5:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
    ...
    # --- v038改善: 大型ピース旗側配置(type7-12を旗側に配置) ---
    if _flag_side is not None and 7 <= next_type <= 12:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # v038改善: 旗側max_y>1.8なら旗側変更してから配置
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
        opposite_side_max_y = right_max_y if _flag_side == "left" else left_max_y
