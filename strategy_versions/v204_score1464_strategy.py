...
def decide(game_state: dict, analysis: dict) -> dict:
    """HIGH_TOWER緩和・v128構造維持版

    v202の失敗（スコア1107、NO_MERGEペナルティ逆効果・HIGH_TOWER50%発動）を受けて、
    振り子パターンを回避しつつHIGHフェーズでのマージ機会を確保するブレイクスルーを実施。

    v202履歴分析で確認した問題:
    - HIGHフェーズが8ターンしかない（v128ではもっと長かった）
    - HIGH_TOWERが50%発動（4/8ターン）、マージ優先が機能していない
    - NO_MERGEペナルティが逆効果：マージしようとしてもHIGH_TOWERが発動し、結局マージしない
    - マージグレードはNEARのみでDIRECT/FARがない
    - v201（スコア1431）もv128（スコア3689）より大幅に低く、v128設定だけでは不十分

    根本原因:
    - NO_MERGEペナルティは「マージ可能なら絶対にマージせよ」という強制だが、
      マージ可能な位置が高すぎる場合（HIGH_TOWER発動）、マージしようとしてもheight_penaltyが大きすぎて、
      結局マージしない方が良いという状況になる
    - v128と同じ設定でもv201のスコアが低いのは、HIGH_TOWERペナルティ（1.3倍）が強すぎて、
      マージ機会を損失している可能性がある
    - これは振り子パターン（NO_MERGEペナルティの追加・削除、HIGH_TOWER倍率の調整）に該当する

    解決策（振り子パターン解消のブレイクスルー）:
    - NO_MERGEペナルティを完全削除：振り子パターン（v201→v202の追加・削除）を回避
    - v128の成功構造（height_mult=1.8, TOWER閾値0.5）を維持しつつ、
      HIGH_TOWERペナルティを1.3倍から1.1倍に緩和：マージ機会を確保
    - v42のマージボーナス（DIRECT=1200/NEAR=600/FAR=200）を維持：
      マージグレードはマージボーナスの指標としてのみ使用
    - v128のバランス補正強度（HIGH=40.0/MEDIUM=30.0/LOW=20.0）を維持
    - ドリフトペナルティ一律30.0を維持
    - nextNextが同じタイプなら中央寄せボーナス50.0を維持
    - v128のシンプル構造（約110行）を維持
    - NO_MERGEペナルティという複雑な条件分岐を削除し、パラメータ調整（HIGH_TOWER倍率1.3→1.1）で対応
    """
    ...
    for result in results:
        ...
        # HIGH_TOWERペナルティ（v203: v128の閾値0.5を維持、倍率1.3→1.1に緩和）
        if phase == "HIGH" and landing_y > 0.5:  # v203: v128の閾値0.5を維持
            height_penalty *= 1.1  # v203: v128の1.3倍から1.1倍に緩和（マージ機会確保）
            reasons.append("HIGH_TOWER")
        elif phase == "MEDIUM" and landing_y > 0.5:  # v203: v42の閾値0.5を維持
            height_penalty *= 1.5  # v203: v42の1.5倍を維持
