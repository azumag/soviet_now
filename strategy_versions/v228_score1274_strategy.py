...
# v228: HIGH_TOWERペナルティ段階化版 - v227の失敗（avg=1120.8、stddev=251.0）を受けて、HIGH_TOWER振り子を解消。MEDIUM=1.3x、HIGH=1.5xの段階化でフェーズ適応制御を実現し、stddev削減を目指す。


def decide(game_state: dict, analysis: dict) -> dict:
    """v228: HIGH_TOWERペナルティ段階化版

    v227の失敗（スコアavg=1120.8、stddev=251.0、ばらつき依然として大きい）を受けて、
    HIGH_TOWERペナルティの振り子パターンを根本的に解消するブレイクスルーを実施。

    v227バッチ統計から特定した問題:
    - HIGH_TOWERペナルティ振り子パターン:
      v227はHIGHとMEDIUMで同じ1.5倍を適用し、「過度なマージ回避」と「急激な盤面上昇」の振り子を引き起こした
    - ベストゲーム（1474点、91ターン）vsワーストゲーム（742点、45ターン）:
      decision_reason分布のHIGH_TOWER使用率が高く（10.4%）、振り子パターンの証拠
    - v227の1.5倍一律は二律背反:
      HIGHフェーズでは緩すぎ（急激な盤面上昇）、MEDIUMフェーズでは強すぎ（過度なマージ回避）

    根本原因:
    - HIGH_TOWERペナルティは「あるかないか」の二択ではなく、「段階的に適応する」必要がある
    - v227はv218の1.5倍を再導入したが、HIGHとMEDIUMで同じ倍率を適用し、フェーズ間のバランスを崩した
    - MEDIUMフェーズ（0.8<max_y<1.8）では1.5倍は強すぎ、積極的なマージを阻害
    - HIGHフェーズ（1.8<max_y<3.0）では1.5倍は緩すぎ、急激な盤面上昇を防げない

    解決策（振り子パターン解消・ブレイクスルー）:
    - HIGH_TOWERペナルティ段階化:
      * MEDIUM=1.3倍（緩和）、HIGH=1.5倍（強化）の2段階化
      * フェーズに応じた適応的制御
    - フェーズ別インセンティブ設計:
      * MEDIUMフェーズではマージ機会を優先（1.3倍）
      * HIGHフェーズでは高度管理を優先（1.5倍）
    - v227の他の設定を維持:
      * height_mult（LOW=1.0、MEDIUM=1.6、HIGH=2.2、CRITICAL=1.0）
      * マージボーナス（DIRECT=1200/NEAR=600/FAR=200）
      * 2段階height_penalty係数（<4: 20.0、>=4: 50.0）
    - 段階的適応で振り子を解消:
      * 「過度なマージ回避」と「急激な盤面上昇」の両方を回避
      * フェーズごとのバランスを最適化
    """
    ...
    for result in results:
        ...
        # v228: HIGH_TOWERペナルティ段階化（振り子パターン解消）
        if phase == "HIGH" and landing_y > 0.5:
            # HIGHフェーズ：高度管理を優先（1.5倍で強化）
            height_penalty *= 1.5
            reasons.append("HIGH_TOWER")
        elif phase == "MEDIUM" and landing_y > 0.5:
            # MEDIUMフェーズ：マージ機会を優先（1.3倍に緩和）
            height_penalty *= 1.3
