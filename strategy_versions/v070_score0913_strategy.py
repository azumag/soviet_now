...
#
# v068: 高度危機回避抑制とマージ戦略再優先化(2026-02-26)
#       - 履歴分析(致命的): Turn 87 (max_y=2.59, merge_available=True, grade=NEAR)で「高度危機回避」優先されマージ見逃し
#       - 履歴分析(致命的): Turn 79-87で「高度危機回避」が9ターン中8ターン発動しmax_yが1.6→2.59に急増
#       - 履歴分析(致命的): type11がデッドライン(y=3.291)を超えゲームオーバー
#       - 履歴分析: 反応器ペアが常に0、type9+が左右散在(左側8個、右側6個)
#       - 履歴分析: type9距離7.3、type11距離7.4で到達不能
#       - v067の問題点: 高度危機回避(max_y>1.3)の閾値が緩すぎ、緊急時に発動しすぎ
#       - v067の問題点: 旗側変更クールダウン廃除により旗側変更が3回のみ(Turn 36, 71, 73)
#       - v067の問題点: 旗側max_y管理が緩すぎ(旗側max_y>1.0のみで即時旗側変更が発動せず)
#       - マージ戦略の絶対的最優先化: 緊急時マージ戦略の閾値を緩和(max_y>1.1でscore>-3許容)(v068新規)
#       - 高度危機回避条件厳格化: max_y>1.5で発動(v067: 1.3→1.5, 厳格化)(v068修正)
#       - 旗側max_y管理再厳格化: 旗側max_y>1.2で即時旗側変更(v067: 1.0→1.2, 再厳格化)(v068修正)
#       - 旗側max_y管理緊急化: 旗側max_y>0.7かつ反対側<0.4なら旗側変更(v067: 0.6→0.7, 0.4維持, 早期化)(v068修正)
#       - 旗側変更条件緩和: 反対側max_y<1.0かつ旗側max_y>反対側+0.5なら旗側変更(v067: 0.6→1.0, +0.4→+0.5, 再緩和)(v068修正)
#       - クールダウン復活: 旗側変更クールダウンを1ターンに設定(v068新規)
#       - 大型ピース旗側配置厳格化: 旗側max_y>0.9かつ反対側<0.4なら旗側変更(v067: 0.8→0.9, 厳格化)(v068修正)
#       - 中型ピース旗側集約: type5-6も旗側と反対側に配置(v068新規)
...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- v069改善: マージ戦略の絶対的最優先化(高度危機回避より前にチェック) ---
    mergeable_results = []
    for r in results:
    ...
    if mergeable_results:
        # v069改善: 緊急時マージ戦略(max_y>1.4ならscore>-8のマージを許容)
        if max_y > 1.4:  # v069新規
    ...
    # --- v069改善: 旗側max_y管理の早期化(旗側max_y>1.1で即時旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        ...
        if flag_side_max_y > 1.1 and opposite_side_max_y < 0.6:  # v069: 1.2→1.1, 早期化
    ...
    # --- v069改善: 旗側max_y管理の緊急化(旗側max_y>0.8かつ反対側<0.4なら旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
        ...
        if flag_side_max_y > 0.8 and opposite_side_max_y < 0.4:  # v069: 0.7→0.8, 早期化
            _flag_side = "right" if _flag_side == "left" else "left"
            _flag_change_cooldown = 1
            target_x = 2.8 if _flag_side == "right" else -2.8
            _consecutive_no_merge += 1
    ...
    if _flag_side is not None and _flag_change_cooldown == 0:
        ...
        # 旗側max_y>反対側max_y+0.5かつ反対側max_y<1.0なら即時旗側変更
        if flag_side_max_y > opposite_side_max_y + 0.5 and opposite_side_max_y < 1.0:
            _flag_side = "right" if _flag_side == "left" else "left"
            _flag_change_cooldown = 1
            target_x = 2.8 if _flag_side == "right" else -2.8
            _consecutive_no_merge += 1
    ...
    # --- v069改善: 高度危機回避の抑制(閾値厳格化と条件追加) ---
    # v069: 閾値を1.5→1.7に厳格化し、両側のmax_y差が0.5以上ある場合のみ発動
    if max_y > 1.7:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # 両側の差が十分大きい場合のみ発動
        if abs(left_max_y - right_max_y) > 0.5:
            lower_side = "left" if left_max_y < right_max_y else "right"
            target_x = 2.8 if lower_side == "right" else -2.8

            # v069改善: max_y>1.7で旗側max_y>1.5かつ反対側<0.5なら旗側変更を優先
    ...
    # --- 大型ピース旗側配置(旗側max_y>0.9かつ反対側<0.4なら旗側変更) ---
    if _flag_side is not None and 7 <= next_type <= 12:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # 旗側max_y>0.9かつ反対側<0.4なら旗側変更
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
        opposite_side_max_y = right_max_y if _flag_side == "left" else left_max_y

        if flag_side_max_y > 0.9 and opposite_side_max_y < 0.4:
            _flag_side = "right" if _flag_side == "left" else "left"
            _flag_change_cooldown = 1
    ...
    # --- v069改善: 中型ピース旗側配置(type5-6も旗側に配置) ---
    if (
        _flag_side is not None and 5 <= next_type <= 6
    ):  # v069修正: 反対側ではなく旗側に配置
        target_x = 2.8 if _flag_side == "right" else -2.8
        _last_drop_x = target_x
        return {
    ...
    # --- シェイク戦略の発動条件緩和(無マージ2ターンで発動) ---
    _consecutive_no_merge += 1
    if _consecutive_no_merge >= 2 and next_type <= 4:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
