...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    if max_y < 0.8:
        ...
        phase = "MEDIUM"
        height_mult = 2.4  # v157: v42の2.4を維持
        merge_mult = 1.0
    elif max_y < 3.0:
        phase = "HIGH"
        height_mult = 1.2  # v157: v128の1.8から1.2に大幅緩和、マージ優先を徹底
        merge_mult = 1.0
    else:
        phase = "CRITICAL"
        height_mult = 1.0  # CRITICAL: height_multなし
        merge_mult = 0.6  # v157: v42の0.6を維持
    ...
    for result in results:
        ...
        # === v157: HIGH_TOWERペナルティ削除・HIGHフェーズ高度管理大幅緩和 ===

        # 1. マージグレードによるスコア（v157: v42の強力な値を維持）
        ...
        # 2. 高度によるペナルティ（v157: 一律構造、HIGH_TOWERペナルティ完全削除）
        height_penalty = landing_y * 50.0 * height_mult

        # v157: HIGH_TOWERペナルティを完全削除（v152の1.0倍ではなく完全削除）
        # v157: 一律構造で頑健性を確保
        if phase == "MEDIUM" and landing_y > 0.5:
            height_penalty *= 1.5  # v157: v42の1.5倍を維持
        ...
        # 3. ドリフトによるペナルティ（v157: v42の一律30.0を維持）
        drift_penalty = (abs(drift_x) + drift_unc) * 30.0
        score -= drift_penalty

        # 4. 左右バランス補正（v157: v42の設定を維持）
        balance_strength = 20.0
        if phase == "HIGH":
            balance_strength = 40.0  # v157: v42の40.0を維持
        elif phase == "MEDIUM":
            balance_strength = 30.0  # v157: v42の30.0を維持
        ...
        # 5. nextNextが同じタイプなら中央寄せボーナス（v157: v42の一律50.0を維持）
