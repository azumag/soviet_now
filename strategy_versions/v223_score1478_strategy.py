...
def decide(game_state: dict, analysis: dict) -> dict:
    """連続的height_penalty係数導入・HIGHフェーズ緩和版

    v222の失敗（スコア2371、反応ペア閾値8が機能せず、max_y=2.93でCRITICAL直前）を受けて、
    閾値調整ではなく「連続的な高度管理」へブレイクスルーを実施。

    v222履歴分析で特定した問題:
    - 反応ペア数が全ターンで最大3（閾値8には一度も到達しない）
    - 常にLOW_REACTIVEモード：height_penalty=20.0（緩和）で固定
    - 盤面がmax_y=2.93まで上昇（CRITICAL閾値3.0の直前）
    - HIGHフェーズ（Turn 46-100）でmax_yが2.93まで上昇

    根本原因:
    - v222の閾値8は実際のゲームデータでは機能していないため、常にheight_penalty=20.0（緩和）で固定され、高度管理が効いていない
    - 閾値8→4→8のシャッフルパターンは、「ある閾値に合わせる」のではなく
      「実際のゲームで機能しない閾値を選んでいる」という堂々巡り
    - 反応ペアが実際には増えている（0個→1個→2個→3個）が、閾値8に到達しないため
      height_penalty係数が全く変化しない

    解決策（振り子パターン解消のブレイクスルー）:
    - 連続的なheight_penalty係数を導入：
      * 反応ペア0-3個: 20.0（緩和、積極的に積む、マージ機会を優先）
      * 反応ペア4-6個: 35.0（中程度の高度管理）
      * 反応ペア7個以上: 45.0（厳格な高度管理）
    - HIGHフェーズのheight_multを1.4に緩和：v218の1.6から緩和し、マージ機会を最大化
    - v218の成功要素を維持：マージボーナス1800/900/300、HIGH_TOWER削除
    - 閾値ではなく反応ペア数の実測値に応じた動的調整で、実際のゲームデータを直接反映させる
    - v218のシンプル構造（約135行）を維持：3段階で実装
    """
    ...
    if max_y < 0.8:
        ...
        phase = "MEDIUM"
        height_mult = 1.4  # v223: v218の1.4を維持
        merge_mult = 1.0
    elif max_y < 3.0:
        phase = "HIGH"
        height_mult = (
            1.4  # v223: v218の1.6から1.4に緩和（HIGHフェーズでのマージ機会を最大化）
        )
        merge_mult = 1.0
    else:
        phase = "CRITICAL"
        height_mult = 1.0  # CRITICAL: height_multなし
        merge_mult = 0.6  # v223: v128の0.6を維持
    ...
    for result in results:
        ...
        # === v223: 連続的height_penalty係数導入・HIGHフェーズ緩和 ===

        # 1. マージグレードによるスコア（v223: v218の強化値を維持）
        ...
        # 2. 高度によるペナルティ（v223: 連続的なheight_penalty係数）
        height_penalty = landing_y * height_penalty_coeff * height_mult

        # v223: HIGH_TOWERペナルティ完全削除（v218の成功要素を維持）
        if landing_y > 0.0:
        ...
        if phase == "HIGH":
            balance_strength = 40.0  # v223: v218の40.0を維持
        elif phase == "MEDIUM":
            balance_strength = 30.0  # v223: v218の30.0を維持
