...
def decide(game_state: dict, analysis: dict) -> dict:
    """v128成功設定を完全復帰し、予測を前提としない一律構造で頑健性を確保。v128は3689点を達成しており、これ以上の緩和や強化は不要。予測精度が低い状況下では、一律構造でv42の頑健性とバランス補正の安全装置機能を維持し、v128の高度管理緩和でマージ優先のバランスをとるのが最適。"""
    ...
    if max_y < 0.8:
        ...
        phase = "MEDIUM"
        height_mult = 2.4  # v153: v128の2.4を維持（MEDIUMフェーズの安定性確保）
        merge_mult = 1.0
    elif max_y < 3.0:
        phase = "HIGH"
        height_mult = 1.8  # v153: v128の1.8を復帰（HIGHフェーズ高度管理緩和）
        merge_mult = 1.0
    else:
        phase = "CRITICAL"
        height_mult = 1.0  # CRITICAL: height_multなし
        merge_mult = 0.6  # v153: v128の0.6を維持
    ...
    for result in results:
        ...
        # 1. マージグレードによるスコア（v153: v128の値を維持）
        ...
        # 2. 高度によるペナルティ（v153: v128の設定を維持）
        height_penalty = landing_y * 50.0 * height_mult

        # HIGH_TOWERペナルティ（v153: v128の1.3倍を復帰）
        if phase == "HIGH" and landing_y > 0.5:
            height_penalty *= 1.3  # v153: v128の1.3倍を復帰
            reasons.append("HIGH_TOWER")
        elif phase == "MEDIUM" and landing_y > 0.5:
            height_penalty *= 1.5  # v153: v128の1.5倍を維持
        ...
        # 3. ドリフトによるペナルティ（v153: v128の一律30.0を維持）
        drift_penalty = (abs(drift_x) + drift_unc) * 30.0
        score -= drift_penalty

        # 4. 左右バランス補正（v153: v128の設定を完全復帰）
        balance_strength = 20.0
        if phase == "HIGH":
            balance_strength = 40.0  # v153: v128の40.0を復帰
        elif phase == "MEDIUM":
            balance_strength = 30.0  # v153: v128の30.0を復帰
        ...
        # 5. nextNextが同じタイプなら中央寄せボーナス（v153: v128の一律50.0を復帰）
