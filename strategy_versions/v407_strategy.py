...
# v407: マージチェーン期待値係数低減版 - v343の失敗（score=1277、CHAIN_POTENTIAL支配45.8%）を受けて、根本的な問題を特定。v343はマージチェーン期待値の係数を強化（100.0/60.0）したが、これは逆効果であり、CHAIN_POTENTIALが45.8%と支配的になった。最新ゲーム（score=1277）の分析で、CHAIN_POTENTIALが22回使用されており、これは将来的な連鎖期待が強すぎて、現在のポジショニング判断を阻害していることを示している。v128のシンプル構造（height_mult=1.8固定）を維持しつつ、マージチェーン期待値の係数をv342のレベル（50.0/30.0）に戻すことで、CHAIN_POTENTIALの支配を抑制する。
#   根本原因の特定:
#   - v343はマージチェーン期待値の係数を強化（100.0/60.0）したが、これは逆効果
#   - 最新ゲームでCHAIN_POTENTIALが22回使用（45.8%の占有率）しており、支配的
#   - 将来的な連鎖期待が強すぎて、現在のポジショニング判断を阻害
#   - マージチェーン期待値ボーナスがマージボーナス（DIRECT=1500/NEAR=800/FAR=300）と競合し、適切な判断を妨げている
#   - v342（score=2495）の係数（50.0/30.0）が適切であったことが確認された
#   改善策（マージチェーン期待値係数低減）:
#   - マージチェーン期待値係数を低減:
#     * next_chain_expectation: 100.0 → 50.0 (半分)
#     * next_next_chain_expectation: 60.0 → 30.0 (半分)
#   - ノンマージのチェーンリリーフ係数も低減:
#     * next_chain_relief: 60.0 → 30.0 (半分)
#     * next_next_chain_relief: 40.0 → 20.0 (半分)
#   - v128のシンプル構造を維持:
#     * HIGHフェーズheight_mult=1.8固定
#     * マージボーナス強化 (DIRECT=1500/NEAR=800/FAR=300)
#     * 左右バランス補正（v128の設定を維持）
#     * HIGH_TOWERペナルティ: 1.3倍（v343と同じ）
#   核心的発見: マージチェーン期待値の係数が強すぎると、将来的な連鎖期待が現在のポジショニング判断を阻害する。v342の係数（50.0/30.0）が適切であり、CHAIN_POTENTIALの支配を抑制しつつ、将来的な連鎖マージの可能性を考慮できる。
#   成功基準: CHAIN_POTENTIAL占有率が20%以下、またはscoreがv343の1277以上
#   失敗基準: CHAIN_POTENTIAL占有率が30%以上、またはscoreがv343の1277未満


def decide(game_state: dict, analysis: dict) -> dict:
    """v128のシンプル構造に完全復帰し、マージチェーン期待値の係数を適正化。静的盤面ではheight_mult=1.8（固定）で安定した高度管理、動的盤面ではマージチェーン期待値を適正に考慮して将来の連鎖マージの可能性をスコアリングに反映。v343で係数を強化（100.0/60.0）したところCHAIN_POTENTIALが支配的（45.8%）になったため、係数を半減（50.0/30.0）してバランスを改善。"""
    ...
    if max_y < 0.8:
        ...
        phase = "HIGH"
        # v407: v128の1.8を固定値で使用（v342のheight_mult_modifier削除）
        height_mult = 1.8
        merge_mult = 1.0
    else:
        phase = "CRITICAL"
    ...
    for result in results:
        ...
        # === v407: v128復帰・マージチェーン期待値適正化 ===

        # 1. マージグレードによるスコア（v407: v341の強力な値を維持）
        ...
        # マージチェーン期待値ボーナス（v407: 係数を適正化）
        if merge_grade != "NO":
            # v407: 係数を適正化 (100.0/60.0 → 50.0/30.0 = 半分)
            # v343の係数が強すぎてCHAIN_POTENTIALが支配的（45.8%）になったため
            ...
            # v407: 係数を適正化 (60.0/40.0 → 30.0/20.0 = 半分)
        ...
        # 2. 高度によるペナルティ（v407: v128の固定値）
        height_penalty = landing_y * 50.0 * height_mult

        # HIGH_TOWERペナルティ（v407: v128の設定に復帰）
        if phase == "HIGH" and landing_y > 0.5:
            # v407: HIGH_TOWERペナルティ1.3倍を維持
            height_penalty *= 1.3
        ...
        # 3. ドリフトによるペナルティ（v407: v128の一律30.0を維持）
        drift_penalty = (abs(drift_x) + drift_unc) * 30.0
        score -= drift_penalty

        # 4. 左右バランス補正（v407: v128の設定を維持）
        ...
        # 5. nextNextが同じタイプなら中央寄せボーナス（v407: v128の設定を維持）
