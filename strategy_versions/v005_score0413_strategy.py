...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- 2. 高度危機回避（max_y > 1.5、v018改善: 旗側max_y>1.0で旗側を無視）---
    if max_y > 1.5:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # v018改善: 旗側max_y>1.0なら旗側を無視して低い側にドロップ
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y

        if flag_side_max_y > 1.0:
            # 明らかに低い側にドロップ（差が0.4以上）
            if left_max_y > right_max_y + 0.4:
                # 右側が低い
                target_x = 2.8  # 壁ドロップ回避
                if _flag_change_cooldown == 0:
                    _flag_side = "right"  # 旗側を変更
                    _flag_change_cooldown = 5  # クールダウン5ターン（v018: 振り子防止）
                _consecutive_no_merge += 1
                _last_drop_x = target_x
                return {
                    "x": target_x,
                    "reason": f"高度危機回避(旗側高い L_max={left_max_y:.1f} R_max={right_max_y:.1f}) x={target_x:.2f}",
                }
    ...
    # --- 3. 中程度危機回避（max_y > 1.0、v018改善: クールダウン中は旗側尊重）---
    if max_y > 1.0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        if _flag_side is not None:
            # v018改善: クールダウン期間中は旗側を尊重してドロップ
            flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y

            if _flag_change_cooldown > 0:
                # v018改善: クールダウン中は旗側max_y<1.0なら旗側にドロップ
                if _flag_side == "left" and left_max_y < 1.0:
                    target_x = -2.8
                elif _flag_side == "right" and right_max_y < 1.0:
                    target_x = 2.8
                else:
                    # クールダウン中だが旗側が高い場合、反対側にドロップ
                    target_x = 2.8 if _flag_side == "left" else -2.8
            else:
                # v018改善: 旗側max_y>=1.0かつ反対側が0.6以上低い場合に旗側変更
                if _flag_side == "left" and left_max_y >= 1.0:
                    if right_max_y < left_max_y - 0.6:
                        _flag_side = "right"
                        _flag_change_cooldown = (
                            5  # v018: クールダウン5ターン（振り子防止）
                        )
    ...
    # --- 4. 旗側集約戦略（v018改善: 旗側max_y<0.8で緩和）---
    if _flag_side is not None and next_type >= 9:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # v018改善: 旗側max_y<0.8なら旗側に配置（v017: 1.0→0.8）
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
        ...
        if flag_side_max_y < 0.8:
            ...
            # 旗側max_y>=0.8の場合、旗側変更を検討
            if _flag_side == "left" and left_max_y >= 0.8:
                if _flag_change_cooldown == 0 and right_max_y < left_max_y - 0.6:
                    _flag_side = "right"
                    _flag_change_cooldown = 5  # v018: クールダウン5ターン（振り子防止）
                    _last_drop_x = 2.8
                    return {
                        "x": 2.8,
                        "reason": f"旗側変更(左側高い L_max={left_max_y:.1f} R_max={right_max_y:.1f})",
                    }
            elif _flag_side == "right" and right_max_y >= 0.8:
                if _flag_change_cooldown == 0 and left_max_y < right_max_y - 0.6:
                    _flag_side = "left"
                    _flag_change_cooldown = 5  # v018: クールダウン5ターン（振り子防止）
                    _last_drop_x = -2.8
                    return {
                        "x": -2.8,
                        "reason": f"旗側変更(右側高い L_max={left_max_y:.1f} R_max={right_max_y:.1f})",
                    }

            # 旗側変更しない場合、反対側に配置（旗側max_y>=0.8）
            if _flag_side == "left":
                target_x = 2.8
            else:
                target_x = -2.8
    ...
    # --- 9. フォールバック: 旗側側の中央（v018改善: 旗側max_y<1.0なら旗側にドロップ）---
    _consecutive_no_merge += 1

    if _flag_side is not None:
        # v018改善: 旗側のmax_yをチェック、1.0未満なら旗側にドロップ
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
