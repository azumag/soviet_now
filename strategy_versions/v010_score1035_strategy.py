...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- v023改善: 旗側決定ロジックの簡素化(type9+が1個以上あればmax_yが低い側を旗側)---
    if _flag_side is None:
        ...
        # 2. v023改善: type9+が1個以上あれば,max_yが低い側を旗側にする
        if _flag_side is None:
    ...
    # --- v023改善: クールダウン期間中の旗側回避(旗側max_y>0.8なら即時反対側にドロップ)---
    if _flag_side is not None:
        ...
        if _flag_change_cooldown > 0 and flag_side_max_y > 0.8:
    ...
    # --- v023改善: 旗側変更条件の緩和(旗側max_y>0.8かつ反対側が0.5以上低い場合に旗側変更)---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y

        # 旗側max_y>0.8の場合,反対側が0.5以上低ければ旗側を変更
        if flag_side_max_y > 0.8:
            opposite_max_y = right_max_y if _flag_side == "left" else left_max_y
            if flag_side_max_y > opposite_max_y + 0.5:
                _flag_side = "right" if _flag_side == "left" else "left"
                _flag_change_cooldown = 3  # v023改善: 3ターン(5ターン→3ターン)

    # --- 1. マージ可能なら最優先(v023改善: 旗側max_y>0.8なら旗側のマージを回避)---
    mergeable_results = []
    for r in results:
    ...
    if mergeable_results:
        ...
        # v023改善: 高度危機時(max_y>1.5),高い側のマージを優先
        if max_y > 1.5:
            left_max_y = calculate_side_max_y(pieces, "left")
            right_max_y = calculate_side_max_y(pieces, "right")
            ...
            higher_merges = [
                r
                for r in mergeable_results
                if (higher_side == "left" and r["x"] < 0)
                or (higher_side == "right" and r["x"] > 0)
            ]

            if higher_merges:
                best = max(higher_merges, key=lambda r: r.get("score", 0))
            else:
                # マージ可能ならどれでも優先
            ...
            return {
                "x": x,
                "reason": f"マージ(高度危機) x={x:.2f} (score={score:.1f})",
            }
    ...
    if max_y > 1.5:
        ...
        # v023改善: 旗側max_y>0.8なら旗側を回避して低い側にドロップ
        if _flag_side is not None:
            flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
        ...
        if left_max_y > right_max_y + 0.6:
            ...
            return {
                "x": target_x,
                "reason": f"高度危機回避(左側高い) x={target_x:.2f}",
            }
            ...
            return {
                "x": target_x,
                "reason": f"高度危機回避(右側高い) x={target_x:.2f}",
            }
    ...
    if max_y > 1.0:
        ...
        if _flag_side is not None:
            ...
            # v023改善: 旗側max_y>0.8なら旗側を回避して反対側にドロップ
            if flag_side_max_y > 0.8:
                target_x = 2.8 if _flag_side == "left" else -2.8
            else:
                target_x = -2.8 if _flag_side == "left" else 2.8
    ...
    # --- 4. 大型ピース旗側配置(v023改善: 旗側max_y<0.8で旗側配置)---
    if _flag_side is not None and next_type >= 9:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y

        # v023改善: 旗側max_y<0.8で旗側配置(v022: 1.2→0.8)
        if flag_side_max_y < 0.8:
            if _flag_side == "left":
                target_x = -2.8
            else:
            ...
            # v023改善: 旗側max_y>0.8の場合,反対側に配置(旗側を避ける)
            if _flag_side == "left":
                target_x = 2.8
            else:
                target_x = -2.8
