...
def decide(game_state: dict, analysis: dict) -> dict:
    """HIGHフェーズでの高度管理を大幅緩和し、マージを強制的に優先"""
    ...
    # フェーズ判定（v94: v84の閾値0.8/1.8/3.0を維持）
    if max_y < 0.8:
        phase = "LOW"
        height_mult = 1.0
        merge_mult = 1.2
        ...
        phase = "MEDIUM"
        height_mult = 2.2  # v94: v84の2.2を維持
        merge_mult = 1.0
    elif max_y < 3.0:
        phase = "HIGH"
        height_mult = 2.2  # v94: v84の2.2を維持
    ...
    for result in results:
        ...
        # 1. マージグレードによるスコア（v94: v84の強化値を維持）
        if merge_grade == "DIRECT":
            score += 1500.0 * merge_mult  # v94: v84の1500を維持
            reasons.append("DIRECT_MERGE")
        elif merge_grade == "NEAR":
            score += 800.0 * merge_mult  # v94: v84の800を維持
            reasons.append("NEAR_MERGE")
        elif merge_grade == "FAR":
            score += 300.0 * merge_mult  # v94: v84の300を維持
            reasons.append("FAR_MERGE")

        # 2. 高度によるスコア（v94: HIGHフェーズで大幅緩和）
        if phase == "CRITICAL":
            ...
            # v94: HIGHフェーズで一律25.0に大幅緩和（35.0→25.0）
            # 予測ベースの条件分岐を一切排除し、一律で緩和してマージ機会を最大化
            height_multiplier = 25.0  # v94: v93の35.0から大幅緩和

            height_penalty = landing_y * height_mult * height_multiplier

            # v94: HIGH_TOWERペナルティ(1.3倍)を完全廃止
            # マージ機会を最大化するため、追加ペナルティなし
            if landing_y > 0.0:
            ...
            # 高盤面での追加ペナルティ（v94: v84の設定を維持）
            if phase == "MEDIUM" and landing_y > 0.5:
                height_penalty *= 1.5  # v94: v84の1.5倍を維持
        ...
        # 3. v94: NO_MERGEペナルティ（v92の強化値を維持）
        if phase == "HIGH" and merge_grade == "NO":
            score -= 200.0  # v94: v92の200を維持
            reasons.append("NO_MERGE_PENALTY")

        # 4. ドリフトによるペナルティ（v94: 一律で計算）
        drift_penalty = (abs(drift_x) + drift_unc) * 30.0
        score -= drift_penalty

        # 5. 左右バランス補正（v94: v84の設定を維持）
        balance_strength = 20.0
        if phase == "HIGH":
            balance_strength = 40.0  # v94: v84の40.0を維持
        elif phase == "MEDIUM":
            balance_strength = 30.0  # v94: v84の30.0を維持
        ...
        # 6. nextNextが同じタイプなら中央寄せボーナス（v94: v84の設定を維持）
        if next_next_type == next_type:
            center_bonus = max(0, 1.0 - abs(x) / 2.0) * 50.0
            score += center_bonus
            reasons.append("NEXT_SAME")

        # 7. v94: max_yに応じた動的調整（LOW_MERGEボーナス強化）
