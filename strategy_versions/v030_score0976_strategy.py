...
#
# v037: 旗側管理の根本的改善と緊急時マージ閾値の緩和(2026-02-25 最新)
#       - 履歴分析(致命的): ターン53でmerge_available=trueだが「高度危機回避」が優先されmax_y=2.17突破
#       - 履歴分析(致命的): ターン60でmerge_available=true(DIRECT!)だが「高度危機回避」が優先されmax_y=2.22突破
#       - 履歴分析(致命的): 最終盤面で旗側max_y=2.73に達するも旗側変更されず,type9+が左右散在
#       - 履歴分析(致命的): 旗側集約ロジックが不十分(type9+が左右散在)
#       - 履歴分析: マージ成功率19/30≈63%(max_y>1.0でマージ見逃し3回)
#       - 履歴分析: 旗側max_yが2.73に達するも旗側変更されず(旗側管理失敗)
#       - 履歴分析: スコア停滞期間が6回発生(3ターン以上score_delta=0)
#       - マージ戦略の超緊急時改善: max_y>2.0ならscore>-15のマージを許容(v037維持)
#       - マージ戦略の緊急時改善: max_y>1.5ならscore>-8のマージを許容(v037維持)
#       - マージ戦略の超緊急時改善: max_y>1.2ならscore>-5のマージを許容(v037改善: 1.0→1.2)
#       - マージ戦略の緊急時改善: max_y>1.0ならscore>-3のマージを許容(v037新規)
#       - 高度危機回避の早期化: max_y>1.2で発動(v037改善: 1.0→1.2,条件緩和)
#       - 旗側max_y管理の根本的改善: 多層的閾値導入(v037新規)
#       - 旗側max_y管理の第1層: 旗側max_y>1.5なら即時旗側変更(v037新規)
#       - 旗側max_y管理の第2層: 旗側max_y>1.3かつ反対側max_y<1.0なら即時旗側変更(v037新規)
#       - 旗側max_y管理の第3層: 旗側max_y>1.1かつmax_y>0.7なら旗側変更(v037新規)
#       - 旗側変更ロジックの強化: 旗側max_y>反対側max_y+0.4かつ反対側max_y<1.3なら即時旗側変更(v037改善: 0.3→0.4)
#       - 旗側変更クールダウンの短縮: 1ターン(v037改善: 2→1,柔軟性向上)
#       - クールダウン中旗側回避の改善: 反対側max_y>1.0なら即時旗側変更(v037新規)
#       - 中程度危機回避の条件緩和: max_y>0.6で発動(v037改善: 0.3→0.6,条件緩和)
#       - 大型ピース旗側配置の強化: 旗側max_y>1.1なら即時旗側変更してから配置(v037改善)
#       - シェイク戦略の発動条件緩和: 無マージ4ターンで発動(v037改善: 3→4,慎重化)
#       - 次のピース保護の改善: 旗側max_y>1.0なら旗側にドロップしない(v037改善)
...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    if mergeable_results:
        # v037改善: 致命的時マージ戦略(max_y>2.0ならscore>-15のマージを許容)
        if max_y > 2.0:
        ...
        # v037改善: 超緊急時マージ戦略(max_y>1.5ならscore>-8のマージを許容)
        if max_y > 1.5:
        ...
        # v037改善: 緊急時マージ戦略(max_y>1.2ならscore>-5のマージを許容)
        if max_y > 1.2:
        ...
        # v037改善: 準緊急時マージ戦略(max_y>1.0ならscore>-3のマージを許容)
        if max_y > 1.0:
            ...
            if valid_merges:
                ...
                return {
                    "x": x,
                    "reason": f"マージ(準緊急) x={x:.2f} (score={score:.1f})",
                }
    ...
    # --- v037改善: 旗側max_y管理の第1層(旗側max_y>1.5なら即時旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        ...
        if flag_side_max_y > 1.5:
            ...
            return {
                "x": target_x,
                "reason": f"旗側変更(超緊急) x={target_x:.2f} (旗側max_y={flag_side_max_y:.2f})",
            }

    # --- v037改善: 旗側max_y管理の第2層(旗側max_y>1.3かつ反対側max_y<1.0なら即時旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
        ...
        if flag_side_max_y > 1.3 and opposite_side_max_y < 1.0:
            _flag_side = "right" if _flag_side == "left" else "left"
            _flag_change_cooldown = 1
            target_x = 2.8 if _flag_side == "right" else -2.8
            _consecutive_no_merge += 1
    ...
    # --- v037改善: 旗側変更ロジックの強化(旗側max_y>反対側max_y+0.4かつ反対側max_y<1.3なら即時旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
        ...
        if flag_side_max_y > opposite_side_max_y + 0.4 and opposite_side_max_y < 1.3:
            _flag_side = "right" if _flag_side == "left" else "left"
            _flag_change_cooldown = 1
            target_x = 2.8 if _flag_side == "right" else -2.8
            _consecutive_no_merge += 1
    ...
    # --- v037改善: 旗側max_y管理の第3層(旗側max_y>1.1かつmax_y>0.7なら旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
    ...
    # --- v037改善: クールダウン中旗側回避の改善(反対側max_y>1.0なら即時旗側変更) ---
    if _flag_side is not None:
        ...
        # v037改善: 反対側max_y>1.0なら即時旗側変更
        if opposite_side_max_y > 1.0 and _flag_change_cooldown == 0:
            _flag_side = "right" if _flag_side == "left" else "left"
            _flag_change_cooldown = 1
            target_x = 2.8 if _flag_side == "right" else -2.8
            _consecutive_no_merge += 1
    ...
    # --- v037改善: 高度危機回避の早期化(max_y>1.2で発動) ---
    if max_y > 1.2:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
    ...
    # --- v037改善: 中程度危機回避の条件緩和(max_y>0.6で発動) ---
    if max_y > 0.6:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
    ...
    # --- v037改善: 大型ピース旗側配置(type7-12を旗側に配置) ---
    if _flag_side is not None and 7 <= next_type <= 12:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # v037改善: 旗側max_y>1.1なら即時旗側変更してから配置
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
        opposite_side_max_y = right_max_y if _flag_side == "left" else left_max_y

        if flag_side_max_y > 1.1 and opposite_side_max_y < 0.9:
            _flag_side = "right" if _flag_side == "left" else "left"
            _flag_change_cooldown = 1
    ...
    # --- v037改善: シェイク戦略の発動条件緩和(無マージ4ターンで発動) ---
    _consecutive_no_merge += 1
