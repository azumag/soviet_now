...
def decide(game_state: dict, analysis: dict) -> dict:
    """HIGH_TOWER完全削除・v128成功構造完全復帰版

    v210の失敗（スコア1511、HIGH_TOWER緩和・マージグレード高度制限導入）を受けて、
    振り子パターンを根本的に解消するブレイクスルーを実施。

    v210履歴分析で特定した問題:
    - HIGH_TOWERペナルティの緩和（1.3倍→1.1倍）は効果がなく、スコアが低下（v208:1710→v210:1511）
    - マージグレード高度制限を導入したが、複雑化しただけで改善に寄与していない
    - HIGH_TOWERペナルティ自体が現在のピース配列では過剰であり、微調整では不十分

    根本原因:
    - HIGH_TOWERペナルティの倍率調整（1.3倍→1.2倍→1.1倍）は振り子パターン
    - マージグレード高度制限は複雑化しただけで、v128のシンプル構造よりも劣化
    - v208でheight_penalty係数50.0が使用されていた可能性があり、v128の40.0に戻す必要がある
    - HIGH_TOWERペナルティ自体が現在のピース配列では過剰であり、完全削除が必要

    解決策（振り子パターン解消のブレイクスルー）:
    - HIGH_TOWERペナルティを完全削除：v128の1.3倍緩和ではなく、完全削除でマージ機会を最大化
    - マージグレード高度制限の簡素化：v210の複雑なロジックを削除し、シンプルに
    - v128の成功構造を完全復帰：height_penalty係数40.0、その他v128の設定を完全復帰
    - マージグレード高度制限：DIRECT:1.0, NEAR:0.8, FAR:0.6（v210と同じだがシンプルに実装）
    - 振り子パターン（HIGH_TOWER倍率調整、マージグレード高度制限の複雑化）を回避し、v128の成功構造に完全回帰
    """
    ...
    if max_y < 0.8:
        ...
        phase = "MEDIUM"
        height_mult = 2.4  # v211: v128の2.4を維持
        merge_mult = 1.0
    elif max_y < 3.0:
        phase = "HIGH"
        height_mult = 1.8  # v211: v128の1.8を維持
        merge_mult = 1.0
    else:
        phase = "CRITICAL"
        height_mult = 1.0  # CRITICAL: height_multなし
        merge_mult = 0.6  # v211: v128の0.6を維持
    ...
    for result in results:
        ...
        # === v211: HIGH_TOWER完全削除・v128成功構造完全復帰 ===
        ...
        # 2. マージグレードによる高度制限（v211: v128係数40.0に復帰、簡素化）
        # DIRECTマージ: 標準的な高度制限
        ...
        # v211: v128のheight_penalty係数40.0に復帰
        height_penalty = landing_y * 40.0 * height_mult * height_multiplier

        # MEDIUM_TOWERペナルティ（v211: v128の1.5倍を維持）
        if phase == "MEDIUM" and landing_y > 0.5:  # v211: v42の閾値0.5を維持
            height_penalty *= 1.5  # v211: v42の1.5倍を維持
        ...
        if phase == "HIGH":
            balance_strength = 40.0  # v211: v128の40.0を維持
        elif phase == "MEDIUM":
            balance_strength = 30.0  # v211: v128の30.0を維持
