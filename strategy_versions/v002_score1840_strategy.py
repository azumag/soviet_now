...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- 旗側早期決定（v015改善: type9+が1個以上あれば旗側決定）---
    if _flag_side is None:
        ...
        # 2. v015改善: type9+が1個以上あれば、max_yが低い側を旗側にする
        if _flag_side is None and len(pieces) >= 5:
            ...
            if left_9plus + right_9plus >= 1:
                # type9+が1個以上ある場合、max_yが低い側を旗側にする
    ...
    # --- 2. 高度危機回避（max_y > 1.0、v015維持）---
    # v015改善: 旗側変更閾値を0.5に統一
    if max_y > 1.0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
    ...
    if max_y > 0.7:
        ...
        if _flag_side is not None:
            ...
            if _flag_change_cooldown > 0:
                ...
                # v015改善: 旗側max_y>1.5かつ反対側が0.5以上低い場合に旗側変更（v014: 0.3→0.5）
                if _flag_side == "left" and left_max_y > 1.5:
                    if right_max_y < left_max_y - 0.5:
                        ...
                        # 旗側変更しない場合は旗側にドロップ
                        target_x = -2.8
                elif _flag_side == "right" and right_max_y > 1.5:
                    if left_max_y < right_max_y - 0.5:
                        _flag_side = "left"
                        _flag_change_cooldown = 5
                        # 旗側変更後は即時反対側（旗側）にドロップ
    ...
    # --- 4. 旗側集約戦略（v015改善: 旗側max_y<1.5で緩和、旗側固定強化）---
    if _flag_side is not None and next_type >= 9:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # v015改善: 旗側max_y<1.5なら旗側に配置（v014: 1.3→1.5）
        # 旗側max_y>=1.5かつ反対側が大幅に低い場合のみ旗側変更
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y

        if flag_side_max_y < 1.5:
            # 旗側max_y<1.5の場合、旗側に配置
            if _flag_side == "left":
                # 左側から（-3.0～-0.5）
                target_x = -2.8
            else:
    ...
    # v015改善: type9が続く場合、旗側を優先して左右分ける
    ...
    # --- 8. フォールバック: 旗側側の中央（v015改善: 旗側max_y>=1.5なら反対側）---
    _consecutive_no_merge += 1

    if _flag_side is not None:
        # v015改善: 旗側のmax_yをチェック、高すぎる場合は反対側にドロップ
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
