...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    if mergeable_results:
        ...
        if positive_merge_results:
            ...
            if _flag_side is not None:
                ...
                # v020改善: 旗側max_y>1.0なら旗側のマージを回避（統一閾値）
    ...
    # --- 2. 高度危機回避（max_y > 1.5、v020改善: 旗側max_y>1.0で旗側を無視して低い側にドロップ）---
    if max_y > 1.5:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # v020改善: 旗側max_y>1.0の場合、旗側を無視して低い側にドロップ（統一閾値）
        if _flag_side is not None:
            flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y

            if flag_side_max_y > 1.0:
                # v020改善: 明らかに低い側にドロップ（旗側を無視）
    ...
    # --- 3. 中程度危機回避（max_y > 1.0、v020改善: クールダウン期間中の旗側回避バグ修正）---
    if max_y > 1.0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        ...
        if _flag_side is not None:
            ...
            # v020改善: クールダウン期間中は旗側max_y>1.0なら即時反対側にドロップ（バグ修正）
            if _flag_change_cooldown > 0:
                if flag_side_max_y > 1.0:
                    # v020修正: 旗側を回避する場合、旗側と反対側にドロップ
                ...
                # v020改善: 旗側max_y>1.0なら旗側を回避（統一閾値）
                if flag_side_max_y > 1.0:
                    # v020修正: 旗側を回避する場合、旗側と反対側にドロップ
                    target_x = -2.8 if _flag_side == "left" else 2.8
                else:
    ...
    # --- 4. 旗側集約戦略（v020改善: 旗側max_y>1.0なら旗側に配置しない）---
    if _flag_side is not None and next_type >= 9:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # v020改善: 旗側max_y<1.0なら旗側に配置（統一閾値）
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y

        if flag_side_max_y < 1.0:
            # 旗側max_y<1.0の場合、旗側に配置
            if _flag_side == "left":
                # 左側から（-3.0～-0.5）
                target_x = -2.8
            else:
            ...
            # v020改善: 旗側max_y>1.0の場合、旗側変更を検討しつつ反対側にドロップ
            if _flag_side == "left" and left_max_y >= 1.0:
                if (
                    ...
                    return {
                        "x": 2.8,
                        "reason": f"旗側変更(極限条件 L_max={left_max_y:.1f} R_max={right_max_y:.1f})",
                    }
            ...
            # v020改善: 旗側max_y>1.0の場合、旗側に配置しない（反対側にドロップ）
            if _flag_side == "left":
                target_x = 2.8
            else:
                target_x = -2.8
    ...
    # --- 9. フォールバック: 旗側側の中央（v020改善: 旗側max_y>1.0なら反対側にドロップ）---
    _consecutive_no_merge += 1

    if _flag_side is not None:
        # v020改善: 旗側のmax_yをチェック、1.0未満なら旗側にドロップ（統一閾値）
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
