...
def decide(game_state: dict, analysis: dict) -> dict:
    """v42統合・MEDIUMフェーズ高度管理強化版

    v190の失敗（スコア809、MEDIUMフェーズ高度管理不十分・HIGHフェーズHIGH_TOWER 100%発動）を受けて、
    v42の成功した高度管理パラメータを採用し、MEDIUMフェーズでの高度管理を根本的に強化するブレイクスルーを実施。

    v190履歴分析で確認した問題:
    - MEDIUMフェーズでの高度管理不十分（盤面上昇速度0.109/ターン、v128の0.056/ターンの約2倍）
    - HIGHフェーズでHIGH_TOWERが100%発動（10ターン中10回）
    - TOWERペナルティ閾値0.5が緩すぎる
    - マージボーナス振子は解消済み（v190: 809 > v189: 778）

    解決策:
    - TOWERペナルティ閾値をv42の0.3に下げる（v190の0.5から）
    - MEDIUM_TOWERペナルティをv42の2.0倍に強化
    - バランス補正はv128の左右カウントベースを維持
    - v42とv128の成功要素を統合
    """
    ...
    # フェーズ判定（v191: v42の閾値0.8/1.8/3.0を維持）
    if max_y < 0.8:
        phase = "LOW"
        height_mult = 1.0
        merge_mult = 1.2
        ...
        phase = "MEDIUM"
        height_mult = 2.6  # v191: v190の2.6を維持
        merge_mult = 1.0
    elif max_y < 3.0:
        phase = "HIGH"
        height_mult = 1.8  # v191: v128の1.8を維持
        merge_mult = 1.0
    else:
        phase = "CRITICAL"
        height_mult = 1.0  # CRITICAL: height_multなし
        merge_mult = 0.6  # v191: v42の0.6を維持
    ...
    for result in results:
        ...
        # === v191: v42統合・MEDIUMフェーズ高度管理強化 ===

        # 1. マージグレードによるスコア（v191: v128の値を維持、補助的な位置付け）
        if merge_grade == "DIRECT":
            score += 1200.0 * merge_mult  # v191: v128の1200を維持
            reasons.append("DIRECT_MERGE")
        elif merge_grade == "NEAR":
            score += 600.0 * merge_mult  # v191: v128の600を維持
            reasons.append("NEAR_MERGE")
        elif merge_grade == "FAR":
            score += 200.0 * merge_mult  # v191: v128の200を維持
            reasons.append("FAR_MERGE")

        # 2. 高度によるペナルティ（v191: HIGHフェーズheight_mult=1.8を維持）
        height_penalty = landing_y * 50.0 * height_mult

        # TOWERペナルティ（v191: v42の閾値0.3を採用）
        if (
            phase == "HIGH" and landing_y > 0.3
        ):  # v191: v42の0.3を採用（v190の0.5から厳しく）
            height_penalty *= 1.3  # v191: v128の1.3倍を維持
            reasons.append("HIGH_TOWER")
        elif (
            phase == "MEDIUM" and landing_y > 0.3
        ):  # v191: v42の0.3を採用（v190の0.5から厳しく）
            height_penalty *= 2.0  # v191: v42の2.0倍を採用（v190の1.5倍から強化）
        ...
        # 3. ドリフトによるペナルティ（v191: v128の一律30.0を維持）
        drift_penalty = (abs(drift_x) + drift_unc) * 30.0
        score -= drift_penalty

        # 4. 左右バランス補正（v191: v128の左右カウントベースを維持）
        balance_strength = 20.0
        if phase == "HIGH":
            balance_strength = 40.0  # v191: v128の40.0を維持
        elif phase == "MEDIUM":
            balance_strength = 30.0  # v191: v128の30.0を維持
        ...
        # 5. nextNextが同じタイプなら中央寄せボーナス（v191: v42の一律50.0を維持）
