...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- 旗側早期決定（v016改善: ピース数>=3でtype9+のピース数が多い側を旗側にする）---
    if _flag_side is None:
        ...
        # 2. v016改善: ピース数>=3でtype9+のピース数が多い側を旗側にする
    ...
    # --- 2. 高度危機回避（max_y > 1.0、v016改善: 旗側max_y>1.0で旗側を無視）---
    if max_y > 1.0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # v016改善: 旗側max_y>1.0なら旗側を無視して低い側にドロップ
        ...
        if flag_side_max_y > 1.0:
            ...
            if left_max_y > right_max_y + 0.3:
                ...
                if _flag_change_cooldown == 0:
                    ...
                    _flag_change_cooldown = 3  # クールダウン3ターン
            ...
            if left_max_y > right_max_y + 0.5:
                ...
                if _flag_change_cooldown == 0:
                    _flag_side = "right"  # 旗側を変更
                    _flag_change_cooldown = 5  # クールダウン5ターン
                _consecutive_no_merge += 1
                _last_drop_x = target_x
                return {
                    "x": target_x,
    ...
    # --- 3. 中程度危機回避（max_y > 0.7、v016改善: 旗側max_y>=0.8で旗側変更）---
    if max_y > 0.7:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        if _flag_side is not None:
            # v016改善: 旗側max_y>=0.8で旗側変更を検討
            flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y

            # クールダウン期間中は旗側を変更しない
            if _flag_change_cooldown > 0:
                # v016改善: 旗側max_y>=0.8なら反対側にドロップ
                ...
                # v016改善: 旗側max_y>=0.8かつ反対側が0.5以上低い場合に旗側変更
                if _flag_side == "left" and left_max_y >= 0.8:
    ...
    # --- 4. 旗側集約戦略（v016改善: 旗側max_y<0.8で厳格化）---
    if _flag_side is not None and next_type >= 9:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # v016改善: 旗側max_y<0.8なら旗側に配置（v015: 1.5→0.8）
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
        ...
        if flag_side_max_y < 0.8:
            ...
            if _flag_side == "left" and left_max_y >= 0.8:
                if _flag_change_cooldown == 0 and right_max_y < left_max_y - 0.5:
                    _flag_side = "right"
                    _flag_change_cooldown = 3
                    _last_drop_x = 2.8
                    return {
                ...
                if _flag_change_cooldown == 0 and left_max_y < right_max_y - 0.5:
                    _flag_side = "left"
                    _flag_change_cooldown = 3
                    _last_drop_x = -2.8
                    return {
    ...
    # --- 5. type7-8の配置戦略（v016改善: 旗側と反対側に配置してバランス）---
    if _flag_side is not None and 7 <= next_type <= 8:
        # v016改善: type7-8は旗側と反対側に配置してバランスを取る
        target_x = 2.8 if _flag_side == "left" else -2.8
        _last_drop_x = target_x
        return {
            "x": target_x,
    ...
    # --- 6. シェイク戦略（v016改善: 無マージ3ターンで発動）---
    _consecutive_no_merge += 1
    if _consecutive_no_merge >= 3 and next_type <= 5:
        # 小ピースで下層を揺らす
    ...
    # --- 9. フォールバック: 旗側側の中央（v016改善: 旗側max_y>=0.8なら反対側）---
    _consecutive_no_merge += 1

    if _flag_side is not None:
        # v016改善: 旗側のmax_yをチェック、高すぎる場合は反対側にドロップ
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
