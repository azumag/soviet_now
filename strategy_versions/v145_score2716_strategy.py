...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    if max_y < 0.8:
        ...
        phase = "HIGH"
        height_mult = 1.8  # v145: v128の1.8を採用、HIGHフェーズ高度管理緩和でマージ優先
        merge_mult = 1.0
    else:
        phase = "CRITICAL"
    ...
    for result in results:
        ...
        # 2. 高度によるペナルティ（v145: 予測ベース緩和削除、一律で計算）
        height_penalty = landing_y * 50.0 * height_mult

        # v145: HIGH_TOWERペナルティ完全削除。MEDIUMフェーズでのみMEDIUM_TOWERペナルティを維持。
        # HIGHフェーズではheight_mult=1.8のみで高度管理し、マージを優先。
