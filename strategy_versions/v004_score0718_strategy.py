...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- 旗側早期決定（v017改善: ピース数>=3でtype9+のピース数が多い側を旗側にする）---
    if _flag_side is None:
        ...
        # 2. v017改善: ピース数>=3でtype9+のピース数が多い側を旗側にする
        if _flag_side is None and len(pieces) >= 3:
    ...
    # --- 2. 高度危機回避（max_y > 1.5、v017改善: 旗側max_y>1.0で旗側を無視）---
    if max_y > 1.5:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # v017改善: 旗側max_y>1.0なら旗側を無視して低い側にドロップ
        ...
        if flag_side_max_y > 1.0:
            ...
            if left_max_y > right_max_y + 0.4:
                ...
                if _flag_change_cooldown == 0:
                    ...
                    _flag_change_cooldown = 2  # クールダウン2ターン
    ...
    # --- 3. 中程度危機回避（max_y > 1.0、v017改善: 旗側max_y>=1.0で旗側変更）---
    if max_y > 1.0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        if _flag_side is not None:
            # v017改善: 旗側max_y>=1.0で旗側変更を検討
            flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y

            # クールダウン期間中は旗側を変更しない
            if _flag_change_cooldown > 0:
                # v017改善: 旗側max_y>=1.0なら反対側にドロップ
                ...
                # v017改善: 旗側max_y>=1.0かつ反対側が0.6以上低い場合に旗側変更
                if _flag_side == "left" and left_max_y >= 1.0:
    ...
    # --- 4. 旗側集約戦略（v017改善: 旗側max_y<1.0で緩和）---
    if _flag_side is not None and next_type >= 9:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # v017改善: 旗側max_y<1.0なら旗側に配置（v016: 0.8→1.0）
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
    ...
    # --- 9. フォールバック: 旗側側の中央（v017改善: 旗側max_y>=1.0なら反対側）---
    _consecutive_no_merge += 1

    if _flag_side is not None:
        # v017改善: 旗側のmax_yをチェック、高すぎる場合は反対側にドロップ
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        ...
        if _flag_side == "left" and left_max_y >= 1.0:
            ...
            # 旗側側にドロップ
            x = -1.5 if _flag_side == "left" else 1.5
    else:
        x = 0.0
