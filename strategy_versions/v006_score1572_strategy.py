...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- 旗側固定ロジック（v013改善: DIRECTマージがない場合、大型ピース分布から旗側決定）---
    if _flag_side is None:
        ...
        # 2. DIRECTマージがない場合、大型ピース分布から旗側決定
        if _flag_side is None and len(pieces) >= 10:
            left_large = calculate_large_piece_count(pieces, "left", min_type=8)
            right_large = calculate_large_piece_count(pieces, "right", min_type=8)

            if left_large > right_large + 2:
                _flag_side = "left"
            elif right_large > left_large + 2:
                _flag_side = "right"
            elif left_large > 0 or right_large > 0:
                # 差が小さい場合は、max_yが低い側を旗側にする
    ...
    # --- 2. 高度危機回避（max_y > 0.9、v013改善: 差閾値を0.2→0.3に緩和）---
    # v013改善: 差閾値を0.2から0.3に緩和して旗側変更の整合性確保
    if max_y > 0.9:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        ...
        if left_max_y > right_max_y + 0.3:
            ...
            _flag_change_cooldown = 3  # v013改善: クールダウン3ターン
    ...
    # --- 3. 中程度危機回避（max_y > 0.7、v013改善: 差閾値0.2→0.3、max_y閾値0.8→1.0）---
    if max_y > 0.7:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        ...
        if _flag_side is not None:
            ...
            if _flag_change_cooldown > 0:
                ...
                # v013改善: 旗側max_y>1.0かつ反対側が0.3以上低い場合に旗側変更（0.2→0.3）
    ...
    # --- 4. 旗側集約戦略（v013改善: 旗側max_y<1.0で緩和）---
    if _flag_side is not None and results:
        if next_type >= 9:
            ...
            # v013改善: 旗側max_y<1.0なら旗側に配置（v012: 0.8→1.0）
            # 旗側max_y>=1.0かつ反対側<0.7なら旗側を変更して配置
            if _flag_side == "left" and left_max_y >= 1.0:
                if right_max_y < 0.7 and right_max_y < left_max_y - 0.3:
                    _flag_side = "right"
                ...
                if left_max_y < 0.7 and left_max_y < right_max_y - 0.3:
                    _flag_side = "left"
                    _flag_change_cooldown = 3
                # 旗側が高すぎる場合は配置しない（次の戦略へ）
                if _flag_side == "right" and right_max_y >= 1.0:
                    pass
                else:
                    # 旗側（または変更後の旗側）に配置
                    pass

            # 旗側に配置（旗側のmax_yが1.0未満、または変更後）
            if _flag_side == "left":
                # 左側から（-3.0～-0.5）
                best_ev = -float("inf")
                best_x = None
    ...
    # --- 8. フォールバック: 旗側側の中央（v013改善: 旗側max_y>=1.0なら反対側）---
    _consecutive_no_merge += 1

    if _flag_side is not None:
        # v013改善: 旗側のmax_yをチェック、高すぎる場合は反対側にドロップ
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
