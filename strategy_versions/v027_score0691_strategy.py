...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- v035改善: 旗側max_y管理の厳格化(旗側max_y>1.2なら即時旗側変更) ---
    ...
    # --- v035改善: ゲームオーバー防止の早期化(max_y>1.8で発動) ---
    if max_y > 1.8:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
    ...
    # --- v035改善: 高度危機回避の優先度変更(高度危機回避をマージ戦略より優先) ---
    if max_y > 1.3:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
    ...
    if mergeable_results:
        # v035改善: 高度危機時マージ(1.3 < max_y <= 1.5, score>-5)
        if 1.3 < max_y <= 1.5:
        ...
        # v035改善: 超緊急時マージ(max_y>1.5ならscore>-5のマージを許容)
        if max_y > 1.5:
            valid_merges = [r for r in mergeable_results if r.get("score", 0) > -5]
            if valid_merges:
                best = max(valid_merges, key=lambda r: r.get("score", 0))
        ...
        # v035改善: 緊急時マージ(max_y>1.0ならscore>-3のマージを許容)
        if max_y > 1.0:
    ...
    # --- v035改善: 旗側max_y管理の強化(旗側max_y>1.0かつmax_y>0.7なら旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
    ...
    if _flag_side is not None:
        ...
        # v035改善: max_y>0.8ならクールダウン中旗側回避を無効化
        if max_y <= 0.8:
    ...
    # --- v035改善: 中程度危機回避の条件緩和(max_y>0.4で発動) ---
    if max_y > 0.4:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
    ...
    # --- v035改善: 大型ピース旗側配置(type7-12を旗側に配置) ---
