...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    if max_y < 0.8:
        ...
        phase = "MEDIUM"
        height_mult = 2.4  # v179: v42の2.4を維持
        merge_mult = 1.0
    elif max_y < 3.0:
        phase = "HIGH"
        height_mult = 1.2  # v179: v178の1.8から1.2に大幅緩和（v42の2.6の46%）
        merge_mult = 1.0
    else:
        phase = "CRITICAL"
        height_mult = 1.0  # CRITICAL: height_multなし
        merge_mult = 0.6  # v179: v42の0.6を維持
    ...
    for result in results:
        ...
        # === v179: HIGHフェーズ高度管理大幅緩和 ===

        # 1. マージグレードによるスコア（v179: v42の一律値を維持）
        if merge_grade == "DIRECT":
            score += 1200.0 * merge_mult  # v179: v42の1200を維持
            reasons.append("DIRECT_MERGE")
        elif merge_grade == "NEAR":
            score += 600.0 * merge_mult  # v179: v42の600を維持
            reasons.append("NEAR_MERGE")
        elif merge_grade == "FAR":
            score += 200.0 * merge_mult  # v179: v42の200を維持
            reasons.append("FAR_MERGE")

        # 2. 高度によるペナルティ（v179: v42の基本構造を維持）
        height_penalty = landing_y * 50.0 * height_mult

        # TOWERペナルティ（v179: HIGHフェーズでは大幅緩和）
        if phase == "HIGH" and landing_y > 0.5:
            height_penalty *= 1.1  # v179: v178の1.3倍から1.1倍に緩和
            reasons.append("HIGH_TOWER")
        elif phase == "MEDIUM" and landing_y > 0.5:
            height_penalty *= 1.5  # v179: v42の1.5倍を維持
        ...
        # 3. ドリフトによるペナルティ（v179: v42の一律30.0を維持）
        drift_penalty = (abs(drift_x) + drift_unc) * 30.0
        score -= drift_penalty

        # 4. 左右バランス補正（v179: v42の設定を維持）
        balance_strength = 20.0
        if phase == "HIGH":
            balance_strength = 40.0  # v179: v42の40.0を維持
        elif phase == "MEDIUM":
            balance_strength = 30.0  # v179: v42の30.0を維持
        ...
        # 5. nextNextが同じタイプなら中央寄せボーナス（v179: v42の一律50.0を維持）
