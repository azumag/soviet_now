...
#
# v030: マージ戦略の最優先化と旗側max_y管理の超厳格化(2026-02-25 最新)
#       - 履歴分析(致命的): Turn 60-62でmax_yが1.50→3.06に急増しゲームオーバー
#       - 履歴分析(致命的): Turn 62でmerge_available=true(NEAR)だが「高度危機回避」が優先されマージ見逃し
#       - 履歴分析(致命的): Turn 60でleft_max_y=2.56,right_max_y=1.50だがright側(x=2.8)にドロップ
#       - 履歴分析(致命的): Turn 60-62で連続して右側にドロップし,right_max_yが1.50→3.06に上昇
#       - 履歴分析(致命的): Turn 60-62で旗側max_y管理が機能していない(旗側変更が遅い)
#       - 履歴分析: マージ成功率16/22≈73%(マージ見逃し6回)
#       - 履歴分析: Turn 62でマージ見逃しが致命的(max_y=2.56→3.06)
#       - 履歴分析: Turn 60で「高度危機回避」が発動し,連続して同じ側にドロップし続けた
#       - 履歴分析: 最終盤面type10が右側に2個(id=81,y=1.507,id=85,y=3.053),旗側集約失敗
#       - マージ戦略の最優先化: マージ判定を高度危機回避より優先(v029維持)
#       - 緊急時マージ戦略: max_y>1.5かつマージ可能ならscore>-20のマージを許容(v030: -5→-20,緩和)
#       - 緊急時マージ戦略の改善: max_y>2.0かつマージ可能なら全マージを許容(v030新規)
#       - 高度危機回避の条件超厳格化: max_y>2.0で発動(v029: 1.5→2.0,超厳格化)
#       - 高度危機回避の改善: 同じ側に連続してドロップしない(前回と反対側にドロップ)
#       - 旗側max_y管理の超厳格化: 旗側max_y>2.0なら即時旗側変更(v029: 1.5→2.0,超厳格化)
#       - 旗側max_y管理の強化: 旗側max_y>1.5かつmax_y>1.0なら旗側変更(v030新規)
#       - 旗側決定ロジックの簡素化: max_y>1.0で旗側max_y>反対側なら旗側変更(v030改善)
#       - 旗側変更クールダウンの短縮: 1ターン(v029: 2→1,超柔軟化)
#       - 中程度危機回避の緩和: max_y>0.5で発動(v029: 0.6→0.5,緩和)
#       - シェイク戦略の発動条件緩和: 無マージ4ターンで発動(v029維持)
#       - nextNext保護の維持: 旗側max_yが高い場合,旗側にドロップしない(v029維持)
#
# v031: 高度危機回避の早期化と中程度危機回避の修正(2026-02-25 最新)
#       - 履歴分析(致命的): ターン60でmax_y=2.08に到達してから「高度危機回避」発動
#       - 履歴分析(致命的): ターン53-55で連続して「中程度危機回避」だがmax_yが0.5→0.86→0.87と増加
#       - 履歴分析(致命的): 中程度危機回避の「より高い側にドロップ」ロジックが問題
#       - 履歴分析(致命的): 連続して同じ側にドロップしmax_yが急増
#       - 履歴分析(致命的): ターン84でmax_y=2.90に到達しゲームオーバー
#       - 履歴分析: 最終盤面type10が右側(y=2.906),旗側集約失敗
#       - 高度危機回避の早期化: max_y>1.5で発動(v030: 2.0→1.5)
#       - 中程度危機回避の修正: より低い側にドロップ(v030の「より高い側」を修正)
#       - 旗側max_y管理の緩和: 旗側max_y>1.5で即時旗側変更(v030: 2.0→1.5)
#       - 旗側max_y管理の改善: 旗側max_y>1.2かつmax_y>0.8なら旗側変更(v029維持)
#       - マージ戦略の維持: max_y>2.0なら全マージ許容(v030維持)
#       - マージ戦略の維持: max_y>1.5ならscore>-20のマージ許容(v030維持)
#       - マージ戦略の改善: max_y>1.0ならscore>-5のマージ許容(v029復活)
#       - クールダウン期間中旗側回避の改善: max_y>1.2ならクールダウン中旗側回避を無効化(v029維持)
#       - 旗側変更クールダウンの維持: 1ターン(v030維持)
#       - 高度危機回避の改善: 同じ側に連続してドロップしない(v030維持)
#
# v032: 旗側集約の強化と大型ピース配置の改善(2026-02-25 最新)
#       - 履歴分析(致命的): Turn 49-57で旗側が頻繁に変更され、max_yが2.72に急増
#       - 履歴分析(致命的): Turn 81でtype7 y=2.801がデッドライン超過（旗側集約失敗）
#       - 履歴分析(致命的): 最終盤面でtype7-12が左右に散在し、旗側集約が機能していない
#       - 履歴分析(致命的): Turn 55-58でmax_y>2.0だがマージ判定が優先されず、旗側変更が発動
#       - 履歴分析: マージ成功率が低く、スコア停滞期間が長い
#       - 旗側決定ロジックの強化: type9+が出現した時点で旗側を固定にする(v032新規)
#       - 旗側変更クールダウンの改善: 旗側固定後は2ターンクールダウン(v032新規)
#       - マージ戦略の強化: max_y>1.5ならマージ判定を絶対最優先にする(v032改善)
#       - 大型ピース旗側配置の改善: type7-12を旗側に配置(v032新規)
#       - nextNext保護の改善: nextNextのtypeが同じなら旗側max_yが低い側に配置(v032改善)
#       - 旗側max_y管理の改善: 旗側max_y>1.5なら即時旗側変更(v032維持)
...
_flag_fixed = False  # 旗側固定フラグ(v032新規)
...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- v032改善: 旗側決定ロジックの強化(type9+が出現した時点で旗側を固定) ---
    ...
    # --- v032改善: マージ戦略の強化(max_y>1.5ならマージ判定を絶対最優先) ---
    mergeable_results = []
    for r in results:
    ...
    if mergeable_results:
        # v032改善: max_y>1.5ならマージ判定を絶対最優先にする
        if max_y > 1.5:
            # v030改善: 全マージを許容
        ...
        # v029改善: max_y>1.0かつマージ可能ならscore>-5のマージを許容
        if max_y > 1.0:
            # v029改善: スコア>-5のマージを許容(v028: -10→-5)
    ...
    if _flag_side is not None and _flag_change_cooldown == 0:
        ...
        if max_y > 0.8 and flag_side_max_y > opposite_side_max_y:
            ...
            # v032改善: 旗側固定後は2ターンクールダウン
            _flag_change_cooldown = 2

    # --- v032改善: 旗側max_y管理の改善(旗側max_y>1.5なら即時旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y

        # v029改善: 旗側max_y>1.5なら即時旗側変更(v028: 1.0→1.5)
    ...
    # --- v029改善: 旗側max_y管理の強化(旗側max_y>1.2かつmax_y>0.8なら旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
        ...
        # v029改善: 旗側max_y>1.2かつmax_y>0.8なら旗側変更
        if (
            max_y > 0.8
    ...
    # --- v028改善: クールダウン中旗側回避ロジックのバグ修正 ---
    if _flag_side is not None:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # v028改善: 旗側max_y>1.0かつ反対側が低い場合のみ旗側回避
        if _flag_side == "left":
        ...
        # v029改善: max_y>1.2ならクールダウン中旗側回避を無効化
        if max_y <= 1.2:
    ...
    # --- v029改善: 高度危機回避の条件厳格化(max_y>1.5で発動) ---
    if max_y > 1.5:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
    ...
    # --- v029改善: 中程度危機回避の修正(より低い側にドロップ) ---
    if max_y > 0.6:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # v031改善: より低い側にドロップ(v030の「より高い側」を修正)
        lower_side = "left" if left_max_y < right_max_y else "right"
        target_x = 2.8 if lower_side == "right" else -2.8
    ...
    # --- v032改善: 大型ピース旗側配置の改善(type7-12を旗側に配置) ---
    ...
    # --- v029改善: シェイク戦略の発動条件緩和(無マージ4ターンで発動) ---
    _consecutive_no_merge += 1
    ...
    # --- v032改善: nextNext保護の改善(nextNextのtypeが同じなら旗側max_yが低い側に配置) ---
    ...
    if next_next_type > 0 and next_next_type == next_type:
        ...
        # v032改善: nextNextのtypeが同じなら旗側max_yが低い側に配置
