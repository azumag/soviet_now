...
#       - マージ戦略の緊急時改善: max_y>1.0ならscore>-8のマージを許容(v039改善: -8→-12)
...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- v039改善: マージ戦略の最優先化(マージ判定を危機回避より優先) ---
    mergeable_results = []
    for r in results:
    ...
    if mergeable_results:
        # v039改善: 致命的時マージ戦略(max_y>2.0ならscore>-20のマージを許容)
        if max_y > 2.0:
        ...
        # v039改善: 超緊急時マージ戦略(max_y>1.5ならscore>-12のマージを許容)
        if max_y > 1.5:
        ...
        # v039改善: 緊急時マージ戦略(max_y>1.0ならscore>-12のマージを許容)
        if max_y > 1.0:
            valid_merges = [r for r in mergeable_results if r.get("score", 0) > -12]
            if valid_merges:
                best = max(valid_merges, key=lambda r: r.get("score", 0))
    ...
    # --- v039改善: 旗側変更ロジックの複合化(反対側max_y<1.5かつ旗側max_y>反対側+0.5なら旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        ...
        # v039改善: 反対側max_y<1.5かつ旗側max_y>反対側+0.5なら旗側変更
    ...
    # --- v039改善: 旗側max_y管理の複合化(旗側max_y>2.0かつ反対側max_y<1.5かつ旗側max_y>反対側+0.5なら旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
    ...
    # --- v039改善: 旗側max_y管理の強化(max_y>0.7かつ旗側max_y>1.5かつ旗側max_y>反対側なら旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
    ...
    # --- v039改善: クールダウン中旗側回避の無効化(マージ可能なら旗側回避をスキップ) ---
    if _flag_side is not None:
        ...
        # v039改善: max_y>0.7ならクールダウン中旗側回避を無効化
        if max_y <= 0.7:
    ...
    # --- v039改善: 高度危機回避の早期化(max_y>1.3で発動) ---
    if max_y > 1.3:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
