...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    if max_y < 0.8:
        ...
        phase = "MEDIUM"
        height_mult = 2.4  # v152: v42の2.4を維持（MEDIUMフェーズの安定性確保）
        merge_mult = 1.0
    elif max_y < 3.0:
        phase = "HIGH"
        height_mult = 1.6  # v152: v128の1.8から1.6に緩和、マージ機会を確保
        merge_mult = 1.0
    else:
        phase = "CRITICAL"
        height_mult = 1.0  # CRITICAL: height_multなし
        merge_mult = 0.6  # v152: v42の0.6を維持
    ...
    for result in results:
        ...
        # === v152: v128ベース・バランス補正強化・高度管理緩和 ===

        # 1. マージグレードによるスコア（v152: v42の値を維持）
        ...
        # 2. 高度によるペナルティ（v152: 高度管理緩和）
        height_penalty = landing_y * 50.0 * height_mult

        # HIGH_TOWERペナルティ（v152: v128の1.3倍から1.0倍に削除）
        if phase == "HIGH" and landing_y > 0.5:
            height_penalty *= 1.0  # v152: 1.0倍に削除、高い位置でのマージを回避しない
            reasons.append("HIGH_TOWER")
        elif phase == "MEDIUM" and landing_y > 0.5:
            height_penalty *= 1.5  # v152: v42の1.5倍を維持
            reasons.append("MEDIUM_TOWER")
        elif (
            landing_y > 0.3
        ):  # v152: HIGH_LAYER閾値を0.5から0.3に下げ、マージ機会を確保
            reasons.append("HIGH_LAYER")

        score -= height_penalty

        # 3. ドリフトによるペナルティ（v152: v42の一律30.0を維持）
        drift_penalty = (abs(drift_x) + drift_unc) * 30.0
        score -= drift_penalty

        # 4. 左右バランス補正（v152: バランス補正強化）
        # v152: HIGHフェーズのbalance_strengthをv128の40.0から50.0に強化
        # 予測精度が低い状況下では、バランス補正を強めることで予測ミスの影響を最小限に抑え、
        # マージ可能な位置を回避しない安全装置として機能
        balance_strength = 20.0
        if phase == "HIGH":
            balance_strength = 50.0  # v152: v128の40.0から50.0に強化
        elif phase == "MEDIUM":
            balance_strength = 40.0  # v152: v128の30.0から40.0に強化
        ...
        # 5. nextNextが同じタイプなら中央寄せボーナス（v152: v128の50.0から60.0に強化）
        # v152: 盤面の左右不均衡を是正するために強化
