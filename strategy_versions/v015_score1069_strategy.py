...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- v028改善: 高度危機回避の早期化(max_y>1.0で発動) ---
    if max_y > 1.0:
    ...
    if mergeable_results:
        # v028改善: 高度危機時(max_y>1.0)はマージ優先(低スコアも許容)
        if max_y > 1.0:
            # v028改善: スコア>-10のマージを許容
            ...
            if valid_merges:
                ...
                return {
                    "x": x,
                    "reason": f"マージ(危機優先) x={x:.2f} (score={score:.1f})",
                }
    ...
    if _flag_side is not None and _flag_change_cooldown == 0:
        ...
        if max_y > 0.8 and flag_side_max_y > opposite_side_max_y:
            ...
            # v028改善: クールダウンを3ターンに短縮
            _flag_change_cooldown = 3

    # --- v028改善: 中程度危機回避の条件緩和(max_y>0.6で発動) ---
    if max_y > 0.6:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
