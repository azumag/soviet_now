...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    if max_y < 0.8:
        ...
        phase = "MEDIUM"
        height_mult = 2.4  # v163: v128/v42の2.4を維持、HIGHフェーズ到達遅延戦略
        merge_mult = 1.0
    elif max_y < 3.0:
        phase = "HIGH"
        height_mult = 2.6  # v163: v42の2.6に復帰、高度管理を強化
        merge_mult = 1.0
    else:
        phase = "CRITICAL"
        height_mult = 1.0  # CRITICAL: height_multなし
        merge_mult = 0.6  # v163: v42の0.6を維持
    ...
    for result in results:
        ...
        # === v163: v42頑健性復帰・v128構造維持 ===

        # 1. マージグレードによるスコア（v163: v42の強力な値を維持）
        ...
        # 2. 高度によるペナルティ（v163: v42の高度管理構造を完全復帰）
        height_penalty = landing_y * 50.0 * height_mult

        # HIGH_TOWERペナルティ（v163: v42の2.0倍に復帰、振り子解消）
        if phase == "HIGH" and landing_y > 0.5:
            height_penalty *= 2.0  # v163: v42の2.0倍に復帰
            reasons.append("HIGH_TOWER")
        elif phase == "MEDIUM" and landing_y > 0.5:
            height_penalty *= 1.5  # v163: v42の1.5倍を維持
        ...
        # 3. ドリフトによるペナルティ（v163: v42の一律30.0を維持）
        drift_penalty = (abs(drift_x) + drift_unc) * 30.0
        score -= drift_penalty

        # 4. 左右バランス補正（v163: v42の設定を維持）
        balance_strength = 20.0
        if phase == "HIGH":
            balance_strength = 40.0  # v163: v42の40.0を維持
        elif phase == "MEDIUM":
            balance_strength = 30.0  # v163: v42の30.0を維持
        ...
        # 5. nextNextが同じタイプなら中央寄せボーナス（v163: v42の一律50.0を維持）
