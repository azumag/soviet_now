...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- 旗側固定ロジック（v014改善: type9+を使用し、早期旗側決定）---
    if _flag_side is None:
        ...
        # 2. v014改善: type9+の分布から旗側決定（type8+→type9+）
        # v014改善: ピース数>=5で早期旗側決定（10→5）
        if _flag_side is None and len(pieces) >= 5:
            ...
            if left_9plus > right_9plus + 2:
                _flag_side = "left"
            elif right_9plus > left_9plus + 2:
                _flag_side = "right"
            elif left_9plus > 0 or right_9plus > 0:
    ...
    # --- 2. 高度危機回避（max_y > 1.0、v014改善: 0.9→1.0）---
    # v014改善: 旗側変更閾値を0.3に統一
    if max_y > 1.0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
    ...
    # --- 3. 中程度危機回避（max_y > 0.7、v014改善: max_y閾値1.0→1.3）---
    if max_y > 0.7:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        ...
        if _flag_side is not None:
            ...
            if _flag_change_cooldown > 0:
                ...
                # v014改善: 旗側max_y>1.3かつ反対側が0.3以上低い場合に旗側変更（1.0→1.3）
    ...
    # --- 4. 旗側集約戦略（v014改善: 旗側max_y<1.3で緩和、ロジック簡素化）---
    if _flag_side is not None and next_type >= 9:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # v014改善: 旗側max_y<1.3なら旗側に配置（v013: 1.0→1.3）
        # 旗側max_y>=1.3かつ反対側<0.8なら旗側を変更して配置
        if _flag_side == "left" and left_max_y >= 1.3:
            if right_max_y < 0.8 and right_max_y < left_max_y - 0.3:
                _flag_side = "right"
                _flag_change_cooldown = 3
            ...
            if left_max_y < 0.8 and left_max_y < right_max_y - 0.3:
                _flag_side = "left"
                _flag_change_cooldown = 3
            # 旗側が高すぎる場合は配置しない（次の戦略へ）
    ...
    # --- 8. フォールバック: 旗側側の中央（v014改善: 旗側max_y>=1.3なら反対側）---
    _consecutive_no_merge += 1

    if _flag_side is not None:
        # v014改善: 旗側のmax_yをチェック、高すぎる場合は反対側にドロップ
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
