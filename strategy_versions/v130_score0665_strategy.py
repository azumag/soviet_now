...
def decide(game_state: dict, analysis: dict) -> dict:
    """v42のシンプル構造を完全復帰し、振り子パターンを解消。確実に成功した構造に戻る。"""
    ...
    if max_y < 0.8:
        ...
        phase = "MEDIUM"
        height_mult = 2.4  # v130: v42の2.4を完全復帰
        merge_mult = 1.0
    elif max_y < 3.0:
        phase = "HIGH"
        height_mult = 2.6  # v130: v42の2.6に完全復帰
        merge_mult = 1.0
    else:
        phase = "CRITICAL"
        height_mult = 1.0  # CRITICAL: height_multなし
        merge_mult = 0.6  # v130: v42の0.6を維持
    ...
    for result in results:
        ...
        # 1. マージグレードによるスコア（v130: v42の値を完全復帰）
        ...
        # 2. 高度によるペナルティ（v130: v42の計算方法を完全復帰）
        height_penalty = landing_y * 50.0 * height_mult

        # HIGH_TOWERペナルティ（v130: v42の2.0倍を完全復帰）
        if phase == "HIGH" and landing_y > 0.5:
            height_penalty *= 2.0  # v130: v42の2.0倍を完全復帰
        ...
        # 3. ドリフトによるペナルティ（v130: v42の一律30.0を完全復帰）
        drift_penalty = (abs(drift_x) + drift_unc) * 30.0
        score -= drift_penalty

        # 4. 左右バランス補正（v130: v42の設定を完全復帰）
        ...
        # 5. nextNextが同じタイプなら中央寄せボーナス（v130: v42の一律50.0を完全復帰）
