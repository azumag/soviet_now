...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    if mergeable_results:
        # 緊急時マージ戦略(max_y>1.2かつマージ可能ならscore>0のマージを許容)
        if max_y > 1.2:
            valid_merges = [r for r in mergeable_results if r.get("score", 0) > 0]
            if valid_merges:
    ...
    # --- v031改善: 高度危機回避の早期化(max_y>1.2で最優先発動) ---
    if max_y > 1.2:
    ...
    # --- v031改善: 旗側変更ロジックの緩和(max_y>0.5で旗側max_y>反対側なら旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
        ...
        # v031改善: max_y>0.5で旗側max_y>反対側なら旗側変更(v029: 0.8→0.5)
        if max_y > 0.5 and flag_side_max_y > opposite_side_max_y:
            _flag_side = "right" if _flag_side == "left" else "left"
            # v029改善: クールダウンを2ターンに短縮
            _flag_change_cooldown = 2

    # --- v031改善: クールダウン中旗側回避の強化(旗側max_y>1.2なら即時反対側にドロップ) ---
    if _flag_side is not None:
        ...
        # v031改善: 旗側max_y>1.2なら即時反対側にドロップ(v029: 1.0→1.2)
        if flag_side_max_y > 1.2:
            # 反対側にドロップ
    ...
    # --- v031改善: 中程度危機回避の条件緩和(max_y>0.7で発動) ---
    if max_y > 0.7:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
