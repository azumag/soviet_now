...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- 3. 中程度危機回避（max_y > 1.0）---
    # v010改善: 旗側max_y厳格チェック、旗側>1.0かつ反対側<0.8なら旗側変更
    if max_y > 1.0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        ...
        if _flag_side is not None:
            ...
            if _flag_change_cooldown > 0:
                ...
                # v010改善: 旗側max_yを厳格にチェック（>1.0で旗側変更を検討）
                # 反対側が0.8未満かつ反対側が大幅に低い場合のみ旗側を変更
                if _flag_side == "left" and left_max_y > 1.0:
                    if right_max_y < 0.8 and right_max_y < left_max_y - 0.5:
                        _flag_side = "right"
                        _flag_change_cooldown = 5  # 5ターンは変更しない
                        # v010改善: 旗側変更後は即時反対側（旗側）にドロップ
                        target_x = 2.8
                    else:
                        # 旗側変更しない場合は旗側にドロップ
                        target_x = -2.8
                elif _flag_side == "right" and right_max_y > 1.0:
                    if left_max_y < 0.8 and left_max_y < right_max_y - 0.5:
                        _flag_side = "left"
                        _flag_change_cooldown = 5  # 5ターンは変更しない
                        # v010改善: 旗側変更後は即時反対側（旗側）にドロップ
                        target_x = -2.8
    ...
    # --- 4. 旗側集約戦略（v010改善: 旗側max_y<0.8で厳格化）---
    if _flag_side is not None and results:
        if next_type >= 9:
            ...
            # v010改善: 旗側max_yが0.8以上なら旗側に配置しない
            # 旗側max_y<0.8かつ反対側が大幅に低い場合のみ旗側を変更して配置
