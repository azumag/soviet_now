...
#       - 履歴分析: type9+が左右に散在（左3、右6） - 旗側集約完全失敗
...
#       - 旗側変更クールダウン: 5ターンに戻す（v017: 2ターン→5ターン、振り子防止）
...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- 旗側早期決定（v018改善: type9+が1個以上あれば即時旗側決定）---
    if _flag_side is None:
        ...
        # 2. v018改善: type9+が1個以上あれば即時旗側決定
        if _flag_side is None:
            left_9plus = calculate_large_piece_count(pieces, "left", min_type=9)
            right_9plus = calculate_large_piece_count(pieces, "right", min_type=9)

            if left_9plus + right_9plus >= 1:
                # type9+のピース数が多い側を旗側にする
                if left_9plus > right_9plus:
                    _flag_side = "left"
                elif right_9plus > left_9plus:
                    _flag_side = "right"
                else:
                    # ピース数が同じ場合はmax_yが低い側を旗側にする
                    left_max_y = calculate_side_max_y(pieces, "left")
                    right_max_y = calculate_side_max_y(pieces, "right")
                    if left_max_y < right_max_y:
                        _flag_side = "left"
                    elif right_max_y < left_max_y:
                        _flag_side = "right"

            # 3. type9+がない場合、type8+の分布から旗側決定
            if _flag_side is None:
                left_8plus = calculate_large_piece_count(pieces, "left", min_type=8)
                right_8plus = calculate_large_piece_count(pieces, "right", min_type=8)

                if left_8plus + right_8plus >= 1:
                    if left_8plus > right_8plus:
                        _flag_side = "left"
                    elif right_8plus > left_8plus:
                        _flag_side = "right"
                    else:
                        # ピース数が同じ場合はmax_yが低い側を旗側にする
                        left_max_y = calculate_side_max_y(pieces, "left")
    ...
    # --- v021改善: 旗側変更後の旗側max_y即時チェック ---
    if (
        _flag_side is not None and _flag_change_cooldown == 6
    ):  # 前ターンで旗側を変更した場合
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y

        # v021改善: 旗側変更後の旗側max_yが1.0を超える場合、即時旗側回避
        if flag_side_max_y > 1.0:
            # 明らかに低い側にドロップ（旗側を無視）
            if left_max_y > right_max_y + 0.3:
                target_x = 2.8
                _flag_side = "right"
                _flag_change_cooldown = 7
                _consecutive_no_merge += 1
                _last_drop_x = target_x
                return {
                    "x": target_x,
                    "reason": f"旗側変更後回避(L_max={left_max_y:.1f} R_max={right_max_y:.1f}) x={target_x:.2f}",
                }
    ...
    if mergeable_results:
        ...
        if positive_merge_results:
            # v019改善: 旗側max_yチェックと旗側フィルタリング
            if _flag_side is not None:
                left_max_y = calculate_side_max_y(pieces, "left")
                right_max_y = calculate_side_max_y(pieces, "right")
                flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y

                # v021改善: 旗側max_y>0.8なら旗側のマージを回避（統一閾値の厳格化）
    ...
    # --- 2. 高度危機回避（max_y > 1.5、v021改善: 旗側max_y>1.0で旗側を無視して低い側にドロップ）---
    if max_y > 1.5:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # v021改善: 旗側max_y>1.0の場合、旗側を無視して低い側にドロップ（統一閾値）
        if _flag_side is not None:
            flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y

            if flag_side_max_y > 1.0:
                # v021改善: 明らかに低い側にドロップ（旗側を無視）
                if left_max_y > right_max_y + 0.4:
                    # 右側が低い
                    target_x = 2.8  # 壁ドロップ回避
                    # v019改善: 旗側を即時変更（クールダウン無視）
                    _flag_side = "right"
                    _flag_change_cooldown = (
                        7  # v019: クールダウン7ターン（振り子徹底防止）
                    )
                    _consecutive_no_merge += 1
                    _last_drop_x = target_x
                    return {
                        "x": target_x,
                        "reason": f"高度危機回避(旗側無視 L_max={left_max_y:.1f} R_max={right_max_y:.1f}) x={target_x:.2f}",
                    }
                elif right_max_y > left_max_y + 0.4:
                    # 左側が低い
                    target_x = -2.8  # 壁ドロップ回避
                    # v019改善: 旗側を即時変更（クールダウン無視）
                    _flag_side = "left"
        ...
        # 旗側max_y<=1.0の場合、旗側を考慮
        # 明らかに低い側にドロップ（差が0.6以上）
        if left_max_y > right_max_y + 0.6:
            # 右側が低い
            target_x = 2.8  # 壁ドロップ回避
            if _flag_change_cooldown == 0:
                _flag_side = "right"  # 旗側を変更
                _flag_change_cooldown = 7  # v019: クールダウン7ターン（振り子徹底防止）
            _consecutive_no_merge += 1
            _last_drop_x = target_x
            return {
                "x": target_x,
        ...
        elif right_max_y > left_max_y + 0.6:
            # 左側が低い
            target_x = -2.8  # 壁ドロップ回避
            if _flag_change_cooldown == 0:
                _flag_side = "left"  # 旗側を変更
                _flag_change_cooldown = 7  # v019: クールダウン7ターン（振り子徹底防止）
            _consecutive_no_merge += 1
            _last_drop_x = target_x
            return {
    ...
    # --- 3. 中程度危機回避（max_y > 1.0、v021改善: クールダウン期間中の旗側回避改善）---
    if max_y > 1.0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        ...
        if _flag_side is not None:
            ...
            # v021改善: クールダウン期間中は旗側max_y>1.0なら即時反対側にドロップ（v020のバグ修正）
            if _flag_change_cooldown > 0:
                if flag_side_max_y > 1.0:
                    # v021修正: 旗側を回避する場合、旗側と反対側にドロップ
                    target_x = -2.8 if _flag_side == "left" else 2.8
                    _consecutive_no_merge += 1
                    _last_drop_x = target_x
                    return {
                        "x": target_x,
                        "reason": f"中程度危機回避(クールダウン中旗側回避) x={target_x:.2f}",
                    }
                else:
                    # 旗側max_y<1.0なら旗側にドロップ
                    target_x = -2.8 if _flag_side == "left" else 2.8
            else:
                # v021改善: 旗側max_y>1.0なら旗側を回避（統一閾値）
                if flag_side_max_y > 1.0:
                    # v021修正: 旗側を回避する場合、旗側と反対側にドロップ
                    target_x = -2.8 if _flag_side == "left" else 2.8
                else:
    ...
    # --- 4. 旗側集約戦略（v021改善: 旗側max_y>0.8なら旗側に配置しない）---
    if _flag_side is not None and next_type >= 9:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")

        # v021改善: 旗側max_y<0.8なら旗側に配置（統一閾値の厳格化）
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y

        if flag_side_max_y < 0.8:
            # 旗側max_y<0.8の場合、旗側に配置
            if _flag_side == "left":
                # 左側から（-3.0～-0.5）
                target_x = -2.8
            else:
            ...
            return {
                "x": target_x,
                "reason": f"大型ピース旗側 x={target_x:.2f} (旗側max_y={flag_side_max_y:.1f})",
            }
        else:
            # v021改善: 旗側max_y>0.8の場合、旗側変更を検討しつつ反対側にドロップ
            if _flag_side == "left" and left_max_y >= 0.8:
                if (
                    _flag_change_cooldown == 0
                    and right_max_y < left_max_y - 0.8
                    and right_max_y < 0.6
                ):
                    _flag_side = "right"
                    _flag_change_cooldown = (
                    ...
                    return {
                        "x": 2.8,
                        "reason": f"旗側変更(L_max={left_max_y:.1f} R_max={right_max_y:.1f})",
                    }
            elif _flag_side == "right" and right_max_y >= 0.8:
                if (
                    _flag_change_cooldown == 0
                    and left_max_y < right_max_y - 0.8
                    and left_max_y < 0.6
                ):
                    _flag_side = "left"
                    _flag_change_cooldown = (
            ...
            # v021改善: 旗側max_y>0.8の場合、旗側に配置しない（反対側にドロップ）
            if _flag_side == "left":
                target_x = 2.8
            else:
                target_x = -2.8
    ...
    # --- 5. type7-8の配置戦略（v017改善: 旗側と反対側に配置してバランス）---
    if _flag_side is not None and 7 <= next_type <= 8:
        # v017改善: type7-8は旗側と反対側に配置してバランスを取る
        target_x = 2.8 if _flag_side == "left" else -2.8
        _last_drop_x = target_x
        return {
            "x": target_x,
    ...
    # --- 9. フォールバック: 旗側側の中央（v021改善: 旗側max_y>0.8なら反対側にドロップ）---
    _consecutive_no_merge += 1

    if _flag_side is not None:
        # v021改善: 旗側のmax_yをチェック、0.8未満なら旗側にドロップ（統一閾値の厳格化）
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
