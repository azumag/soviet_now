...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    # --- v037改善: マージ戦略の最優先化(マージ判定を危機回避より優先) ---
    mergeable_results = []
    for r in results:
    ...
    if mergeable_results:
        # v037改善: 致命的時マージ戦略(max_y>2.0ならscore>-20のマージを許容)
        if max_y > 2.0:
        ...
        # v037改善: 超緊急時マージ戦略(max_y>1.5ならscore>-12のマージを許容)
        if max_y > 1.5:
        ...
        # v037改善: 緊急時マージ戦略(max_y>1.0ならscore>-8のマージを許容)
        if max_y > 1.0:
    ...
    # --- v038改善: 旗側変更ロジックの厳格化(旗側max_y>反対側max_y+0.5かつ反対側max_y<1.2なら即時旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        ...
        # v038改善: 旗側max_y>反対側max_y+0.5かつ反対側max_y<1.2なら即時旗側変更
        if flag_side_max_y > opposite_side_max_y + 0.5 and opposite_side_max_y < 1.2:
            _flag_side = "right" if _flag_side == "left" else "left"
            _flag_change_cooldown = 5  # v038: 5ターンに延長
    ...
    # --- v038改善: 旗側max_y管理の厳格化(旗側max_y>1.8なら即時旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y

        if flag_side_max_y > 1.8:
            _flag_side = "right" if _flag_side == "left" else "left"
            _flag_change_cooldown = 5  # v038: 5ターンに延長
            _consecutive_no_merge += 1
            target_x = 2.8 if _flag_side == "right" else -2.8
    ...
    # --- v038改善: 旗側max_y管理の強化(旗側max_y>1.5かつmax_y>0.7なら旗側変更) ---
    if _flag_side is not None and _flag_change_cooldown == 0:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
        flag_side_max_y = left_max_y if _flag_side == "left" else right_max_y
    ...
    # --- v037改善: クールダウン中旗側回避の無効化(マージ可能なら旗側回避をスキップ) ---
    if _flag_side is not None:
        ...
        # v038改善: クールダウン中でも旗側max_y>2.0なら即時旗側変更
        if flag_side_max_y > 2.0:
            _flag_side = "right" if _flag_side == "left" else "left"
        ...
        # v037改善: max_y>0.7ならクールダウン中旗側回避を無効化
        if max_y <= 0.7:
    ...
    # --- v037改善: 高度危機回避の条件厳格化(max_y>1.3で発動) ---
    if max_y > 1.3:
        left_max_y = calculate_side_max_y(pieces, "left")
        right_max_y = calculate_side_max_y(pieces, "right")
