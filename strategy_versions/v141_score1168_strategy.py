...
def decide(game_state: dict, analysis: dict) -> dict:
    ...
    if max_y < 0.8:
        ...
        phase = "MEDIUM"
        height_mult = 2.4  # v141: v42の2.4を維持（MEDIUMフェーズの安定性確保）
        merge_mult = 1.0
    elif max_y < 3.0:
        phase = "HIGH"
        height_mult = (
            2.2  # v141: v42の2.6とv128の1.8の中間値（頑健性とマージ機会のバランス）
        )
        merge_mult = 1.2  # v141: v128の1.2を維持（HIGHフェーズでマージ優先）
    else:
        phase = "CRITICAL"
        height_mult = 1.0  # CRITICAL: height_multなし
        merge_mult = 0.6  # v141: v42の0.6を維持
    ...
    for result in results:
        ...
        # === v141: v42のシンプル一律構造維持・HIGHフェーズ高度管理緩和 ===

        # 1. マージグレードによるスコア（v141: v42の値を維持）
        ...
        # 2. 高度によるペナルティ（v141: 反応器状態連動なし、一律で計算）
        height_penalty = landing_y * 50.0 * height_mult

        # 高盤面での追加ペナルティ（v141: v42とv128の中間値）
        if phase == "HIGH" and landing_y > 0.5:
            height_penalty *= 1.6  # v141: v42の2.0倍とv128の1.3倍の中間値
            reasons.append("HIGH_TOWER")
        elif phase == "MEDIUM" and landing_y > 0.5:
            height_penalty *= 1.5  # v141: v42の1.5倍を維持
        ...
        # 3. ドリフトによるペナルティ（v141: v42の一律30.0を維持）
        drift_penalty = (abs(drift_x) + drift_unc) * 30.0
        score -= drift_penalty

        # 4. 左右バランス補正（v141: v42の設定を維持）
        balance_strength = 20.0
        if phase == "HIGH":
            balance_strength = 40.0  # v141: v42の40.0を維持
        elif phase == "MEDIUM":
            balance_strength = 30.0  # v141: v42の30.0を維持
        ...
        # 5. nextNextが同じタイプなら中央寄せボーナス（v141: v42の一律50.0を維持）
