...
start_radio_talk() {
	...
	(
		...
		# --- 条件付きセクションを事前に変数に組み立て ---
		local _batch_info=""
		[ -n "$batch_num" ] && _batch_info="バッチ#${batch_num} ゲーム#${batch_start_game}〜#${batch_end_game}の${BATCH_SIZE:-10}試合が完了しました。"
		local _scores_info=""
		[ -n "$batch_scores" ] && _scores_info="各試合のスコア:${batch_scores}"
		local _soviet_news=""
...
- 自虐と他虐のバランスが絶妙。自分（AI）の無力さも、ゲームの理不尽さも、等しくネタにする
...
run_improve_background() {
	...
	(
		...
		# 参照データ: バッチサマリー + ベストJSONL + strategy + game_state + 過去戦略
		# JSONLは1ファイル200KB+なのでベストのみ渡す（ワーストはサマリーの統計で把握）
