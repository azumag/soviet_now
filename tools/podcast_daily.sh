...
if [ "${PODCAST_SKIP_AUDIO:-0}" = "1" ]; then
  say "[1/3] 音声: スキップ (PODCAST_SKIP_AUDIO=1)"
else
  say "[1/3] 音声を生成"
  t0=$(date +%s)
  if ! ./tools/podcast_build.sh "${DATE_ARG[@]}"; then
    say "[1/3] 音声の生成に失敗。ここで中止する。"
    exit 2
  fi
  say "[1/3] 音声 done ($(( $(date +%s) - t0 ))s)"
fi
