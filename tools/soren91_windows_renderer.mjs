...
const windowTitle = process.env.SOREN91_LOCAL_WINDOW_TITLE || 'Soren91-Remote';
...
if (process.platform !== 'win32') throw new Error('soren91_windows_renderer must run on Windows');
...
try {
  ...
  await page.evaluate(({ width: w, height: h, title }) => {
  ...
  }, { width, height, title: windowTitle });
