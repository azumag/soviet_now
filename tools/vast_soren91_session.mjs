...
export function defaults(env = process.env) {
  return {
    ...
    maxDph: Number(env.SOREN91_VAST_MAX_DPH || 0.02),
...
export function parseArgs(argv, env = process.env) {
  ...
  const takesValue = new Map([
    ['--market', 'market'], ['--max-dph', 'maxDph'], ['--max-inet-up-cost', 'maxInetUpCost'],
...
export function validateOptions(options) {
  ...
  if (!(options.maxDph > 0 && options.maxDph <= 0.02)) throw new Error('maxDph must be <= 0.02');
...
export function buildSearchQuery(options) {
  return [
    ...
    'inet_down>=100', 'inet_up>=20', `inet_up_cost<=${options.maxInetUpCost}`, `dph<=${options.maxDph}`,
  ].join(' ');
}
...
export function estimateSessionCost(offer, options) {
  const hours = options.sessionSec / 3600;
  ...
  const network = Number(offer.inet_up_cost || 0) * egressGb;
  const storage = Number(offer.storage_cost || 0) * options.diskGb * (options.instanceMaxAgeSec / (30 * 86400));
  return { total: compute + network + storage, compute, network, storage, egressGb, dph };
}
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const plan = { execute: options.execute, offerId: offer.id ?? offer.ask_contract_id, gpu: offer.gpu_name, cost: estimateSessionCost(offer, options), limits: { maxDph: options.maxDph, maxInetUpCost: options.maxInetUpCost, sessionSec: options.sessionSec, instanceMaxAgeSec: options.instanceMaxAgeSec } };
