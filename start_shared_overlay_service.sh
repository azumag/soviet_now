...
NODE_PID=$!
# Keep the common overlay at the BOTTOM of the X stack and the game windows
# above it.  The overlay is a fullscreen Chromium window, so whichever window
# is mapped last ends up on top: a service that starts while a game window is
# already mapped covers that game.  On 2026-09-15 the soren91 corner started
# the overlay over the live sorengame window and the broadcast showed only the
# rails plus the blank game region for four minutes.  Games own the game
# rectangle; the shared overlay only fills what no game window covers, so the
# common rails stay visible around a region-sized presenter and a fullscreen
# game page stays fully visible (it carries its own rails while it is up).
(
	while :; do
		for wid in $(DISPLAY="$DISP" xdotool search --name "^Shared overlay" 2>/dev/null); do
			DISPLAY="$DISP" wmctrl -i -r "$wid" -b add,below 2>/dev/null || true
		done
...
STACK_PID=$!
printf '%s\n' "$XVFB_PID" "$WM_PID" "$NODE_PID" "$STACK_PID" >"$PIDFILE"
cleanup() {
	for pid in "$NODE_PID" "${XVFB_PID:-}" "${WM_PID:-}" "$STACK_PID"; do
