...
// HTML/CSS に追記（XFO 除去・透明化ルール）を行うため、キャッシュを無効化する。
// 特に CSS は静的ハッシュ付き URL で長期間キャッシュされるため、追記ルールが
// ライブ Chrome に届かない問題を防ぐ。
const NO_CACHE_HEADERS = {
  'cache-control': 'no-store, no-cache, must-revalidate',
  'pragma': 'no-cache',
};

// Next.js の静的 CSS はハッシュ付き URL でブラウザに長期間キャッシュされる。
// プロキシで CSS に透明化ルールを追記しても、ライブ Chrome が古いキャッシュを
// 読むと反映されない（CSS が空/未適用に見える）。HTML の <link> href に
// クエリを付与してキャッシュキーを変え、必ずプロキシ経由の新しい CSS を取得させる。
function bustCssLinkHref(html) {
  return String(html ?? '').replace(
    /(<link\b[^>]*rel=["']stylesheet["'][^>]*href=["'])([^"']+\.css)(["'])/gi,
    (m, pre, href, post) => `${pre}${href}${href.includes('?') ? '&' : '?'}twica=1${post}`,
  );
}
...
export async function startTwicaOverlayProxy({
  ...
  const server = http.createServer((req, res) => {
    ...
    // ブラウザは br/gzip 圧縮を要求する。上流（Cloudflare）が圧縮して返すと、
    // プロキシはデコードせずに書き換えるため HTML/CSS が壊れる。
    // 常に平文で取得してから書き換え・中継する。
    ...
    const preq = client.request(
      ...
      (pres) => {
        ...
        try {
          res.writeHead(pres.statusCode, { ...outHeaders, ...(injectTransparentCss ? NO_CACHE_HEADERS : {}) });
        } catch (e) {
        ...
        if (injectTransparentCss
          ...
          // HTML の <link rel="stylesheet"> にキャッシュバスターを付与する。
          // Next.js のチャンクド配信を壊さないよう、先頭バッファ方式で
          // <link rel="stylesheet" を含む部分まで書き換え、以降は pipe する。
          let htmlBuf = Buffer.alloc(0);
          let htmlRewritten = false;
          pres.on('data', (chunk) => {
            if (htmlRewritten) {
              res.write(chunk);
              return;
            }
            htmlBuf = Buffer.concat([htmlBuf, chunk]);
            const s = htmlBuf.toString('utf8');
            // stylesheet link を含む断片が揃ったら書き換え
            if (s.includes('rel="stylesheet"') || s.includes("rel='stylesheet'") || s.includes('</head>') || htmlBuf.length > 262144) {
              res.write(Buffer.from(bustCssLinkHref(s), 'utf8'));
              htmlBuf = Buffer.alloc(0);
              htmlRewritten = true;
            }
          });
          pres.on('end', () => {
            if (htmlBuf.length) res.write(Buffer.from(bustCssLinkHref(htmlBuf.toString('utf8')), 'utf8'));
            res.end();
          });
          pres.on('error', (e) => {
            log?.warn?.(`twica proxy html error: ${e.message}`);
            try { res.end(); } catch (_) {}
          });
          return;
        }
