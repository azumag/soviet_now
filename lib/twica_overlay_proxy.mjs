      ...
      (pres) => {
        ...
        if (injectTransparentCss
          ...
          pres.on('data', (chunk) => {
            ...
            const headEnd = headBuf.indexOf('<head>');
            if (headEnd === -1) {
              // <head> がまだ揃わない。巨大化を防ぐ上限（64KB）で打ち切る。
              if (headBuf.length > 65536) {
                res.write(headBuf);
                headBuf = Buffer.alloc(0);
                injected = true;
                log?.warn?.('twica css inject: <head> not found within 64KB, skipping injection');
              }
              return;
            }
            const headStart = headEnd + '<head>'.length;
            const headPart = headBuf.subarray(0, headStart);
            const rest = headBuf.subarray(headStart);
            res.write(headPart);
            res.write(Buffer.from(TRANSPARENT_CSS, 'utf8'));
            injected = true;
