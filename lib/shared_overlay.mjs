...
export async function waitForSharedOverlayFrames(page, config, options = {}) {
  ...
  const check = ({ frames: expectedFrames }) => {
    ...
    for (const expected of expectedFrames) {
      const frame = document.getElementById(expected.elementId);
      if (!frame || frame.dataset.sorenOverlayRegion !== expected.region) return false;
      try {
...
function healthPayload(config, server) {
  ...
  return {
    ...
    ready: Boolean(runtime.browserReady && runtime.layoutReady && runtime.overlayReady),
