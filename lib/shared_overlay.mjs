...
const SHARED_OVERLAY_REQUIRED_SURFACE_ROUTES = Object.freeze({
...
export function assertSharedOverlayContract(direct) {
  ...
  for (const [key, route] of Object.entries(SHARED_OVERLAY_REQUIRED_SURFACE_ROUTES)) {
    const surface = surfaces.find((item) => item?.key === key);
    if (!surfaceKeys.has(key) || surface?.route !== route) problems.push(`surface.${key}`);
  }
  if (surfaces.some((item) => SHARED_OVERLAY_LEGACY_SURFACE_KEYS.has(item?.key))) {
    problems.push('legacy-surfaces');
  }
  if (surfaces.some((item) => !SHARED_OVERLAY_OPTIONAL_SURFACE_ROUTES.has(item?.route)
      && !Object.values(SHARED_OVERLAY_REQUIRED_SURFACE_ROUTES).includes(item?.route))) {
...
function healthPayload(config, server) {
  ...
  return {
    ...
    ready: Boolean(runtime.browserReady && runtime.layoutReady),
...
export function setSharedOverlayBrowserReady(server, ready, viewport = null) {
  ...
  server.sharedOverlayRuntime.browserReady = Boolean(ready);
  if (viewport && typeof viewport === 'object') {
    server.sharedOverlayRuntime.viewport = viewport;
    server.sharedOverlayRuntime.layoutReady = Boolean(
      Number(viewport.innerWidth) === SHARED_OVERLAY_LAYOUT.width
      && Number(viewport.innerHeight) === SHARED_OVERLAY_LAYOUT.height,
    );
  } else if (!ready) {
