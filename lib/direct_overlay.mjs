...
function overlayInstaller(surfaces) {
  const install = () => {
    ...
    for (const item of surfaces) {
      ...
      if (item.pollMs > 0) {
        frame.srcdoc = '<!doctype html><style>html,body{margin:0;background:transparent}</style>';
      } else {
      ...
      if (item.pollMs > 0) {
        ...
        const refresh = async () => {
          try {
            ...
            // Retry on the next poll; probe IO must never block game rendering.
          }
        };
        void refresh();
        setInterval(refresh, item.pollMs);
      }
    }
  };
