...
import { rewriteTwicaOverlayUrl } from './twica_overlay_proxy.mjs';
...
export function loadDirectOverlayConfig(env = process.env, platform = process.platform) {
  ...
  const improveStateFile = resolveRepoFile(
    env.IMPROVE_STATE_FILE,
    'tmp/state/improve_state.json',
  );
  const improveLogFile = resolveRepoFile(
    env.IMPROVE_AI_LOG_FILE,
    'tmp/debug/improve_ai.log',
  );
  const topOverrideFile = resolveRepoFile(
    env.BROADCAST_TOP_OVERRIDE_FILE,
    'tmp/state/broadcast_top_override.json',
  );
  const twicaUpstreamUrl = env.SOREN_DIRECT_TWICA_OVERLAY_URL || '';
  const twicaProxyPort = parseInt(env.SOREN_DIRECT_TWICA_PROXY_PORT || '18080', 10);
  // TwiCa は X-Frame-Options: SAMEORIGIN を返すため、iframe 直埋めは Chromium に
  // 拒否される。同一オリジンのローカルプロキシ（XFO/CSP 除去・WebSocket 中継）経由で
  // 表示するため、srcUrl はプロキシ URL へ書き換え、元 URL は upstreamUrl に残す。
  const twicaOverlayUrl = twicaUpstreamUrl
    ? rewriteTwicaOverlayUrl(twicaUpstreamUrl, twicaProxyPort)
    : '';
  ...
  const twicaSurface = surface(
    'twica',
    '',
    {
      title: 'TwiCa overlay (external)',
      srcUrl: twicaOverlayUrl,
      upstreamUrl: twicaUpstreamUrl,
      // 透過背景の外部オーバーレイを最前面に配置する。
      // Xvfb の WebGL を黒く覆わないよう、ゲーム領域と独立した最前面 z-index を使う。
      style: {
  ...
  const flagNames = {
    ...
    twica: 'SOREN_DIRECT_TWICA_OVERLAY_ENABLED',
  };
  ...
  // improve も同様に broadcast 側へ統合するため、dashboard では全画面
  // surface を張らない (改善中もゲーム画面を覆わない)。
  ...
  const candidates = useBroadcastSurface
    ? [...broadcastSurfaces, twicaSurface, ...definitions.filter((item) => ['wildcard', 'avsync'].includes(item.key))]
    : [...definitions, twicaSurface];
...
// Place any browser game inside the same persistent broadcast game area used
// by sorengame. The surrounding status rails remain independent of the game,
// so future games only need to provide container/canvas selectors.
export async function installDirectGameStage(page, config, options = {}) {
  const stage = config?.stage || config;
  if (!stage?.enabled) return { stageMode: 'fullscreen' };
  const payload = {
    stage,
    containerSelector: options.containerSelector || '#unity-container',
    canvasSelector: options.canvasSelector || '#unity-canvas',
    drawBufferWidth: Number(options.drawBufferWidth || 0),
    drawBufferHeight: Number(options.drawBufferHeight || 0),
  };
  return page.evaluate(({ stage, containerSelector, canvasSelector, drawBufferWidth, drawBufferHeight }) => {
    ...
    Object.assign(stageElement.style, {
      position: 'fixed', inset: '0', width: `${stage.outputWidth}px`,
      height: `${stage.outputHeight}px`, overflow: 'hidden', pointerEvents: 'none',
      zIndex: '2147483620', background: '#050914',
    });
    const panel = (style) => {
      const element = document.createElement('div');
      element.className = 'soren-stage-placeholder';
      Object.assign(element.style, { position: 'absolute', boxSizing: 'border-box', ...style, opacity: '0' });
    ...
    if (!container || !canvas) throw new Error('game stage container/canvas not found');
    Object.assign(container.style, {
      position: 'fixed', left: `${stage.gameLeft}px`, top: `${stage.gameTop}px`,
      width: `${stage.gameWidth}px`, height: `${stage.gameHeight}px`,
      transform: 'none', overflow: 'hidden', zIndex: '2147483621',
      boxShadow: '0 0 0 1px rgba(148, 205, 255, .28)',
    });
    if (drawBufferWidth > 0 && drawBufferHeight > 0) {
      canvas.width = drawBufferWidth;
      canvas.height = drawBufferHeight;
    }
...
// Keep the blue stage backing hidden until the inline rails have rendered.
export async function revealStagePanels(page, config) {
  await page.evaluate(() => {
    for (const element of document.querySelectorAll('.soren-stage-placeholder')) {
...
// Public game pages must not fetch VM-local overlay routes: Chromium will show
// a Local Network Access prompt on the live surface. Embed only the broadcast
// rails as srcdoc; their state is supplied through window.__sorenBroadcastState.
export async function installInlineDirectBroadcastOverlay(page, config) {
  const surfaces = (config?.surfaces || []).filter((item) => item.region && item.htmlFile);
  if (!config?.enabled || !config?.broadcast || surfaces.length === 0) return false;
  const payload = surfaces.map((item) => ({
    key: item.key,
    elementId: item.elementId,
    title: item.title,
    region: item.region,
    style: item.style,
    html: fs.readFileSync(item.htmlFile, 'utf8'),
  }));
  await page.evaluate((items) => {
    for (const item of items) {
      ...
      frame.dataset.sorenOverlayRegion = item.region;
      frame.dataset.sorenNeutralTheme = '1';
      Object.assign(frame.style, {
        position: 'fixed', border: '0', margin: '0', padding: '0',
        background: 'transparent', pointerEvents: 'none', ...item.style,
      });
...
function overlayInstaller(surfaces) {
  ...
  const install = () => {
    ...
    for (const item of surfaces) {
      ...
      // 外部URL（TwiCa 等）は二重バッファにすると片方の CSS ロード失敗で
      // 素の HTML（スタイルなし・左上配置）が visible 側に表示され得るため、
      // バッファは作らず1フレームのみで直接 src を設定する。
      if (item.srcUrl) {
      ...
      const frames = [0, 1].map(makeFrame);
      let activeIndex = 0;
      let lastHtml = '';
      let pendingHtml = '';
      let generation = 0;
      const refresh = async () => {
        // 外部URLオーバーレイはポーリング不要（iframe 自身がロード済み）。
