...
export const DIRECT_OVERLAY_ROUTES = Object.freeze({
  broadcast: '/__soren_overlay/broadcast',
...
export function loadDirectOverlayConfig(env = process.env, platform = process.platform) {
  ...
  const broadcast = surface(
    ...
    {
      title: 'Soren direct broadcast overlay',
      pollMs: 60000,
      style: {
        inset: '0', width: `${stage.outputWidth || 1280}px`,
  ...
  const flagNames = {
    broadcast: 'SOREN_DIRECT_BROADCAST_OVERLAY_ENABLED',
  ...
  const candidates = useBroadcastSurface
    ? [broadcast, ...definitions.filter((item) => ['improve', 'wildcard', 'avsync'].includes(item.key))]
    : definitions;
  const surfaces = candidates.filter((item) => enabledValue(env[flagNames[item.key]]));
  const event = definitions[0];
  const broadcastEnabled = surfaces.some((item) => item.key === 'broadcast');
  return {
