...
_radio_theme_recent_match_mode() {
	local theme_body="$1"
	local history_bodies_file="${2:-${PAST_RADIO_THEME_BODIES:-$TMP_HISTORY_DIR/past_radio_theme_bodies.txt}}"
	local history_keys_file="${3:-${PAST_RADIO_THEME_KEYS:-$TMP_HISTORY_DIR/.past_radio_themes.txt}}"
...
_radio_mark_theme_used() {
	...
	local keep="${PAST_RADIO_THEME_HISTORY_KEEP:-160}"
	local theme_key=""
	theme_key=$(_radio_theme_key_from_body "$theme_body")
	[ -n "$theme_key" ] && echo "$theme_key" >>"$past_keys_file"
	echo "$theme_body" >>"$past_bodies_file"
	tail -"${keep}" "$past_keys_file" >"${past_keys_file}.tmp" && mv "${past_keys_file}.tmp" "$past_keys_file"
	tail -"${keep}" "$past_bodies_file" >"${past_bodies_file}.tmp" && mv "${past_bodies_file}.tmp" "$past_bodies_file"
}
...
_pick_radio_theme() {
	...
	if [ -f "$ELOOP_LIB_DIR/data/radio_themes.txt" ]; then
		while IFS= read -r _line || [ -n "$_line" ]; do
			...
			local t_key
			t_key=$(_radio_theme_key_from_body "$_line")
			[ -n "$t_key" ] || t_key="$_line"
	...
	for t in "${themes[@]}"; do
		[ -z "$t" ] && continue
		local match_mode=""
		match_mode=$(_radio_theme_recent_match_mode "$t" "$past_theme_bodies_file" "$past_themes_file")
