...
function fallbackRankingComment(effectiveRank, gameNumber = null) {
  ...
  if (effectiveRank != null) {
    ...
    if (rank <= 3) {
      return `今回は${rank}位です。いやー、これはかなり気分がいいですね。資本主義の看板、今日はちゃんと光っています。もちろん満足しすぎると油断するので、そこは少しだけ偉そうにしておきます。${gameNote}上出来です。次はランキングの上のほうで肘を置く場所まで確保します。`;
    }
    if (rank <= 20) {
      return `今回は${rank}位です。悪くないですね、悪くないと言いながら内心ちょっと悔しいやつです。上位に顔は出せましたが、主役の椅子にはまだ座れていません。まあ椅子取りゲームなら、次は椅子ごと買います。${gameNote}だいぶ手応えありです。`;
    }
    return `今回は${rank}位です。正直、悔しいですね。とはいえ、ここでしょんぼりするほど素直なAIではありません。盤面に文句を言いつつ、ちゃんと次の手は考えます。${gameNote}痛みはありますが、まあ授業料です。次は請求書を相手に回します。`;
  }
  const variants = [
    `今回は順位表が出る前に次のMATCHING画面へ戻りました。順位は断定しません。見えていない数字を盛るほど雑なAIではありませんからね。とはいえ、試合した事実と粘りは残っています。${gameNote}材料ありです。次は順位表に居座って、ちゃんとドヤ顔します。`,
    `今回はランキング順位の表示を待つ前に、次戦待機へ進みました。順位を適当に名乗るのはさすがにやりません。そこまで雑なら、もう帽子だけアメリカ製です。終盤の崩れ方は次へ回します。${gameNote}反省はしますが、しょんぼりはしません。`,
    `今回はランキング表ではなくMATCHING画面に戻ったので、順位コメントではなく試合後コメントに切り替えます。勝ち負けの数字は見えていませんが、終盤まで戦った履歴はあります。まあ、証拠が少ない時ほど態度は大きく、です。${gameNote}次で回収します。`,
  ];
