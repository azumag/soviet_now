...
let chosen = normal;
let hold = false;
// Height-first revision: observed deaths are stack-top overruns near the
// deadline, so once the board is already tall, prefer the root candidate
// that adds the least new stack height within the same immediate risk tier.
const dangerLine = 2.3;
const boardTop = pieces.reduce((top, p) => Math.max(top, p.y + p.r), FLOOR);
if (boardTop >= dangerLine) {
let guard = null;
const rootAdded = Math.max(0, normal.peak - boardTop);
for (const candidateX of candidates(pieces, current, garbage.columns)) {
const move = evaluate(pieces, current, candidateX, garbage);
if (move.risk > normal.risk) continue;
const added = Math.max(0, move.peak - boardTop);
if (guard === null
|| added < guard.added - 1e-9
|| (added <= guard.added + 1e-9 && move.value > guard.move.value)) {
guard = { move, added };
}
}
if (guard && Math.abs(guard.move.x - normal.x) >= 0.02 && guard.added < rootAdded - 0.02) {
chosen = {
...normal,
...guard.move,
pathRisk: normal.pathRisk,
minClearance: normal.minClearance,
depth: normal.depth,
expandedNodes: normal.expandedNodes,
};
}
}
...
const prefix = hold ? 'HOLD: ' : '';
const mode = chosen.merges ? 'reachable-merge' : 'structured-stack';
const reason = prefix + mode + '; risk=' + chosen.risk + '; pathRisk=' + chosen.pathRisk
