...
- 画面上では、デッドラインの上にある国が現在持っているピース
- NEXT と書いてある下にある国は、NEXTのNEXT（2手先）である

## ゲームルールと戦略
- 同じ国ブロックを重ねると併合され、レベルがあがり次のレベルの国に変化します
- 積み上がっているブロックの一番上だけ併合可能です
- `gameOver: true` → コマンドファイルに `retry` を書き込め
- 画面がいっぱいになる前に、ロシアを二つくっつけて、ソ連ができれば完成です
- `board.mergePairs` を確認し、同じ国が近くにあるならそこに同じ国を落として併合を狙え
- `cursor.type.name` と `board.pieces` の国名を比較し、同じ国の近くに落とすのが基本戦略
- 先の先（`next` の国も含めて）を考えて落とす場所を決めよ

### 国レベル（低→高）— game_state.json での名前
...
- 「落下」=Yes: ランダムに落ちてくる、Merge: 併合でのみ出現
- 国は回転した状態で盤面に存在することがある
- 形は国土を表している
