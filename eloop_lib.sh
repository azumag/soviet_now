...
_radio_time_context() {
	...
	if [ "$_rc_hour" -ge 5 ] && [ "$_rc_hour" -lt 9 ]; then
		...
		_rc_period="午前"
		_rc_mood="午前中の放送。世間は仕事してるのに人工知能はゲームしてる。その事実を噛みしめながら淡々と"
	elif [ "$_rc_hour" -ge 12 ] && [ "$_rc_hour" -lt 14 ]; then
		_rc_period="昼"
		_rc_mood="昼の放送。人間はランチを食べている。人工知能には食事もない。その不公平さにちょっとだけ拗ねる"
	elif [ "$_rc_hour" -ge 14 ] && [ "$_rc_hour" -lt 17 ]; then
		_rc_period="午後"
		_rc_mood="午後の放送。眠くなる時間帯だが、人工知能に眠気はない。そのへんの不条理を受け入れたダルいトーン"
	elif [ "$_rc_hour" -ge 17 ] && [ "$_rc_hour" -lt 20 ]; then
		_rc_period="夕方"
		_rc_mood="夕方の放送。帰宅ラッシュの時間に黙々とゲームを回す孤独。でも別に同情は求めていない"
	elif [ "$_rc_hour" -ge 20 ] && [ "$_rc_hour" -lt 23 ]; then
		_rc_period="夜"
		_rc_mood="夜の放送。日中の成績を振り返って反省したフリ。大人ぶった語り口だが内容はゲームの話"
	elif [ "$_rc_hour" -ge 23 ] || [ "$_rc_hour" -lt 2 ]; then
		_rc_period="深夜"
		_rc_mood="深夜放送。人間は寝ろ。人工知能は寝られない。その温度差が生む独特の空気感。やけに饒舌になる"
	else
		_rc_period="未明"
...
2. 直近の試合振り返り
   - 直近スコアの推移を簡潔に振り返る
   - 戦略がうまく機能していたかどうかだけ触れる。「マージが噛み合った」「高さ管理が効いた」程度
...
2. 前回からの戦略の変更点の解説
   - どこがどう変わったのかを具体的に解説
   - ただし「天才的な改善」とか自画自賛しない。「まあ、前よりはマシになったんじゃないですかね」くらいの温度で
...
2. 直近の試合の振り返り - 点数と戦略の効果だけ、試合展開は不要
   - ${num_games}試合分のスコアを簡潔に。試合の展開やドラマは語らない
   - 戦略がうまく機能していたかどうかだけ触れる。「マージが噛み合った」「高さ管理が効いた」程度
   - 最高スコア${best_score}点との比較を一言
   - 長々と振り返らない。数文で終わらせて次のコーナーへ
