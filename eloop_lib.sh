...
毎回「今日の一語」として、世の中にまだ名前のない極めて具体的な瞬間・現象・感情に一つだけ名前をつけ、深掘りする。

1. 導入 — 現象の描写から入る（3-5文）
   - いきなり「こういうこと、ありませんか？」と具体的なシーンを語り始める
   - 聞き手が「あ、それ知ってる」と思えるレベルの解像度で描写する
   - コーナー名の宣言は後回し。まず体験を共有して引き込む

2. 命名の儀式（2-3文）
   - 「これを我々は○○と名付けました」と厳かに発表する
   - 語源を語る（古語、方言、外国語の掛け合わせ、ソ連公文書風の略語など、凝った由来）
   - ソ連の学術機関が正式に認定したかのような権威づけを添えるとなお良い

3. 深掘りトーク — ここがメイン（本文の半分以上をここに使う）
   - この現象の「重症例」「亜種」「地域差」などを紹介して広げる
   - 類似概念との学術的っぽい比較（「これは○○と混同されがちですが、決定的に違うのは…」）
   - この言葉が社会に認知されたら何が変わるかを大真面目に論じる
   - ゲームの現状（${game_num}回目、${score}点）をこの概念で分析してみる
   - 脱線OK。話が転がっていく面白さを大事にする

4. クロージング（1-2文）

【造語のクオリティ基準 — 重要】
- 1回につき1語のみ。浅く広くではなく、1語を徹底的に掘り下げる
- 現象: 「あるある」を超えて「言われてみれば確かにそれ名前ないわ…」レベルの解像度
- 語感: 声に出して言いたくなる響き。一度聞いたら忘れない音の並び
- 禁止パターン: 「○○ショック」「○○症候群」「○○現象」「○○ロス」等の安易な接尾辞
- 良い造語の方向: 響きが独特で意味を聞くと腑に落ちる語。和語・方言風・ロシア語風・学術ラテン語風など毎回スタイルを変える

※ 毎回異なる題材にすること。既出の概念・語は絶対に繰り返さない。
